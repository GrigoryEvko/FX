/-! # Sketch/Wave9 — Raw-aware Term architectural prototype.

Ported verbatim from `lean-fx/LeanFX/Sketch/Wave9.lean` (commit
`0dd256a7`).  Standalone proof-of-concept that the raw-aware Term
encoding works zero-axiom for var/unit/lam/app + arrow types.

In lean-fx-2 this file IS the architecture-of-record's PoC.  The
production design in `LeanFX2/Term.lean` extrapolates from this
prototype to all 28 ctors.

Open work: extend this sketch with `appPi` + `refl rawWitness`
demonstrations to cover the dep cases that broke lean-fx's bridge.

---

(original prototype below — content unchanged from lean-fx)
...
namespace LeanFX2.Sketch.Wave9
