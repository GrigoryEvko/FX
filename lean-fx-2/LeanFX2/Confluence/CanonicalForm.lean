...
/-- An `eitherCode`-headed source and an `equivCode`-headed target
are not convertible.  Either type code versus equivalence type
code — distinct canonical type-code heads at the raw level. -/
theorem Conv.eitherCode_ne_equivCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqEquivCode

/-- An `eitherCode`-headed source and an `equivCompose`-headed
target are not convertible.  Either type code versus equivalence
composition — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_equivCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqEquivCompose

/-- An `eitherCode`-headed source and an `equivIntro`-headed
target are not convertible.  Either type code versus equivalence
introduction witness — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_equivIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqEquivIntro

/-- An `eitherCode`-headed source and a `glueIntro`-headed target
are not convertible.  Either type code versus cubical glue
introduction — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_glueIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqGlueIntro

/-- An `eitherCode`-headed source and an `idCode`-headed target are
not convertible.  Either type code versus identity-type code
(ternary at the raw level) — distinct canonical heads. -/
theorem Conv.eitherCode_ne_idCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqIdCode

/-- An `eitherCode`-headed source and an `idStrictRefl`-headed
target are not convertible.  Either type code versus strict-identity
reflexivity witness — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_idStrictRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqStrictRefl

/-- An `eitherCode`-headed source and an `intervalJoin`-headed
target are not convertible.  Either type code versus interval-join
operation — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqIntervalJoin

/-- An `eitherCode`-headed source and an `intervalMeet`-headed
target are not convertible.  Either type code versus interval-meet
operation — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqIntervalMeet

/-- An `eitherCode`-headed source and an `intervalOpp`-headed
target are not convertible.  Either type code versus interval-negation
operation — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
...
/-- An `eitherCode`-headed source and a `productCode`-headed target
are not convertible.  Either type code versus product-type code —
distinct canonical type-code heads at the raw level. -/
theorem Conv.eitherCode_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {productFirstCode productSecondCode : RawTerm scope}
    {sourceTerm : Term context sourceType
      (RawTerm.eitherCode firstCode secondCode : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.productCode productFirstCode productSecondCode : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  nomatch joinEqEither.symm.trans joinEqProductCode

/-- An `eitherCode`-headed source and a `recordIntro`-headed target
are not convertible.  Either type code versus record introduction
— distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqRecordIntro

/-- An `eitherCode`-headed source and a `refineIntro`-headed target
are not convertible.  Either type code versus refinement-type
introduction — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqRefineIntro

/-- An `eitherCode`-headed source and a `sessionRecv`-headed target
are not convertible.  Either type code versus session-receive
operation — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqSessionRecv

/-- An `eitherCode`-headed source and a `sessionSend`-headed target
are not convertible.  Either type code versus session-send
operation — distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqSessionSend

/-- An `eitherCode`-headed source and a `sigmaTyCode`-headed target
are not convertible.  Either type code versus Σ-type code —
distinct canonical type-code heads at the raw level. -/
theorem Conv.eitherCode_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqSigmaTyCode

/-- An `eitherCode`-headed source and a `subsume`-headed target are
not convertible.  Either type code versus modal subsumption —
distinct canonical heads at the raw level. -/
theorem Conv.eitherCode_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqSubsume

/-- An `eitherCode`-headed source and a `sumCode`-headed target are
not convertible.  Either type code versus sum-type code — distinct
canonical type-code heads at the raw level. -/
theorem Conv.eitherCode_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
  ...
  nomatch joinEqEither.symm.trans joinEqSumCode

/-- An `eitherCode`-headed source and a `transpFill`-headed target
are not convertible.  Either type code versus transport-fill
operation — distinct canonical heads at the raw level.  Ternary
target — three trailing slots in the inversion tuple. -/
theorem Conv.eitherCode_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
...
/-- An `equivCode`-headed source and an `equivCompose`-headed target
are not convertible.  Equivalence type code versus equivalence
composition operation — distinct canonical heads at the raw level.
Opens the equivCode row's compound-target sweep. -/
theorem Conv.equivCode_ne_equivCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {firstEquiv secondEquiv : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqEquivCompose, _, _⟩ :=
    RawStep.parStar.equivCompose_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqEquivCompose

/-- An `equivCode`-headed source and an `equivIntro`-headed target
are not convertible.  Equivalence type code versus equivalence
introduction witness — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_equivIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {forwardMap backwardMap : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqEquivIntro, _, _⟩ :=
    RawStep.parStar.equivIntro_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqEquivIntro

/-- An `equivCode`-headed source and a `glueIntro`-headed target are
not convertible.  Equivalence type code versus cubical glue
introduction — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_glueIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {baseCarrier glueCarrier : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqGlueIntro, _, _⟩ :=
    RawStep.parStar.glueIntro_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqGlueIntro

/-- An `equivCode`-headed source and an `idCode`-headed target are
not convertible.  Equivalence type code versus identity-type code
(ternary at the raw level) — distinct canonical type-code heads. -/
theorem Conv.equivCode_ne_idCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {typeCode leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqIdCode, _, _, _⟩ :=
    RawStep.parStar.idCode_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqIdCode

/-- An `equivCode`-headed source and an `idStrictRefl`-headed target
are not convertible.  Equivalence type code versus strict-identity
reflexivity witness — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_idStrictRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {witness : RawTerm scope}
  ...
  obtain ⟨_, joinEqStrictRefl, _⟩ :=
    RawStep.parStar.idStrictRefl_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqStrictRefl

/-- An `equivCode`-headed source and an `intervalJoin`-headed target
are not convertible.  Equivalence type code versus interval-join
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalJoin, _, _⟩ :=
    RawStep.parStar.intervalJoin_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqIntervalJoin

/-- An `equivCode`-headed source and an `intervalMeet`-headed target
are not convertible.  Equivalence type code versus interval-meet
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqIntervalMeet

/-- An `equivCode`-headed source and an `intervalOpp`-headed target
are not convertible.  Equivalence type code versus interval-negation
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqIntervalOpp

/-- An `equivCode`-headed source and a `listCode`-headed target are
not convertible.  Equivalence type code versus list-type code —
distinct canonical type-code heads at the raw level. -/
theorem Conv.equivCode_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqListCode

/-- An `equivCode`-headed source and a `modIntro`-headed target are
not convertible.  Equivalence type code versus modality introduction
— distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqModIntro, _⟩ :=
    RawStep.parStar.modIntro_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqModIntro

/-- An `equivCode`-headed source and an `oeqFunext`-headed target are
not convertible.  Equivalence type code versus observational-equality
functional-extensionality witness — distinct canonical heads at the
raw level. -/
theorem Conv.equivCode_ne_oeqFunext
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {pointwiseProof : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqFunext, _⟩ :=
    RawStep.parStar.oeqFunext_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqOeqFunext

/-- An `equivCode`-headed source and an `oeqJ`-headed target are not
convertible.  Equivalence type code versus observational-equality
eliminator (J) — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_oeqJ
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {baseCase witness : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqJ, _, _⟩ :=
    RawStep.parStar.oeqJ_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqOeqJ

/-- An `equivCode`-headed source and an `oeqRefl`-headed target are
not convertible.  Equivalence type code versus observational-equality
reflexivity — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {witnessTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqOeqRefl

/-- An `equivCode`-headed source and an `oeqTrans`-headed target are
not convertible.  Equivalence type code versus observational-equality
transitivity — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {firstProof secondProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqOeqTrans

/-- An `equivCode`-headed source and an `optionCode`-headed target
are not convertible.  Equivalence type code versus option-type code
— distinct canonical type-code heads at the raw level. -/
theorem Conv.equivCode_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqOptionCode

/-- An `equivCode`-headed source and a `pathCompose`-headed target
are not convertible.  Equivalence type code versus path-composition
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {leftPath rightPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqPathCompose

/-- An `equivCode`-headed source and a `piTyCode`-headed target are
not convertible.  Equivalence type code versus Π-type code —
distinct canonical type-code heads at the raw level. -/
theorem Conv.equivCode_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {domainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTyCode, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqPiTyCode

/-- An `equivCode`-headed source and a `productCode`-headed target
are not convertible.  Equivalence type code versus product-type code
— distinct canonical type-code heads at the raw level.  Target
binders renamed to disambiguate from the source's `firstCode
secondCode`. -/
theorem Conv.equivCode_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {productFirstCode productSecondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProductCode, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqProductCode

/-- An `equivCode`-headed source and a `recordIntro`-headed target
are not convertible.  Equivalence type code versus record
introduction — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecordIntro, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqRecordIntro

/-- An `equivCode`-headed source and a `refineIntro`-headed target
are not convertible.  Equivalence type code versus refinement-type
introduction — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqRefineIntro

/-- An `equivCode`-headed source and a `sessionRecv`-headed target
are not convertible.  Equivalence type code versus session-receive
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqSessionRecv

/-- An `equivCode`-headed source and a `sessionSend`-headed target
are not convertible.  Equivalence type code versus session-send
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqSessionSend

/-- An `equivCode`-headed source and a `sigmaTyCode`-headed target
are not convertible.  Equivalence type code versus Σ-type code —
distinct canonical type-code heads at the raw level.  Target binders
renamed to disambiguate from the source's `firstCode secondCode`. -/
theorem Conv.equivCode_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {sigmaDomainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqSigmaTyCode

/-- An `equivCode`-headed source and a `subsume`-headed target are
not convertible.  Equivalence type code versus modal subsumption —
distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {innerSubsumed : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqSubsume

/-- An `equivCode`-headed source and a `sumCode`-headed target are
not convertible.  Equivalence type code versus sum-type code —
distinct canonical type-code heads at the raw level.  Target
binders renamed to disambiguate from the source's `firstCode
secondCode`. -/
theorem Conv.equivCode_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {sumFirstCode sumSecondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSumCode, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqSumCode

/-- An `equivCode`-headed source and a `transpFill`-headed target
are not convertible.  Equivalence type code versus transport-fill
operation — distinct canonical heads at the raw level.  Ternary
target — three trailing slots in the inversion tuple. -/
theorem Conv.equivCode_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {pathTerm intervalTerm sourceRawTerm : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqEquiv.symm.trans joinEqTranspFill

/-- An `equivCode`-headed source and a `uaToEquiv`-headed target
are not convertible.  Equivalence type code versus univalence-to-
equivalence bridge — distinct canonical heads at the raw level. -/
theorem Conv.equivCode_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstCode secondCode : RawTerm scope}
    {proofTerm : RawTerm scope}
...
/-- An `idCode`-headed source and an `oeqRefl`-headed target are
not convertible.  Identity-type code versus observational-equality
reflexivity witness — distinct canonical heads. -/
theorem Conv.idCode_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {witness : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqOeqRefl

/-- An `idCode`-headed source and an `oeqTrans`-headed target are
not convertible.  Identity-type code versus observational-equality
transitivity composition — distinct canonical heads. -/
theorem Conv.idCode_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {firstProof secondProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqOeqTrans

/-- An `idCode`-headed source and an `optionCode`-headed target are
not convertible.  Identity-type code versus option-type code —
distinct type-former codes at the raw level. -/
theorem Conv.idCode_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqOptionCode

/-- An `idCode`-headed source and a `pathCompose`-headed target are
not convertible.  Identity-type code (HOTT, set-truncated) versus
cubical path composition — distinct equality-fragment heads. -/
theorem Conv.idCode_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {leftPath rightPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqPathCompose

/-- An `idCode`-headed source and a `piTyCode`-headed target are
not convertible.  Identity-type code versus Π-type code; piTyCode
introduces a binder-shape codomain at `scope + 1`. -/
theorem Conv.idCode_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {domainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTyCode, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqPiTyCode

/-- An `idCode`-headed source and a `productCode`-headed target are
not convertible.  Identity-type code versus non-dependent product
type code — distinct type-former codes at the raw level. -/
theorem Conv.idCode_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProductCode, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqProductCode

/-- An `idCode`-headed source and a `recordIntro`-headed target are
not convertible.  Identity-type code versus record-value
introduction — distinct canonical heads at the raw level. -/
theorem Conv.idCode_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecord, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqRecord

/-- An `idCode`-headed source and a `refineIntro`-headed target are
not convertible.  Identity-type code versus refinement-type
introduction (value + predicate proof) — distinct canonical heads. -/
theorem Conv.idCode_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqRefineIntro

/-- An `idCode`-headed source and a `sessionRecv`-headed target are
not convertible.  Identity-type code versus session-receive
operation — distinct canonical heads at the raw level. -/
theorem Conv.idCode_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqSessionRecv

/-- An `idCode`-headed source and a `sessionSend`-headed target are
not convertible.  Identity-type code versus session-send operation
— distinct canonical heads at the raw level. -/
theorem Conv.idCode_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqSessionSend

/-- An `idCode`-headed source and a `sigmaTyCode`-headed target are
not convertible.  Identity-type code versus Σ-type code; sigmaTyCode
introduces a binder-shape codomain at `scope + 1`. -/
theorem Conv.idCode_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {firstCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqSigmaTyCode

/-- An `idCode`-headed source and a `subsume`-headed target are not
convertible.  Identity-type code versus modal subsumption —
distinct canonical heads at the raw level. -/
theorem Conv.idCode_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqSubsume

/-- An `idCode`-headed source and a `sumCode`-headed target are not
convertible.  Identity-type code versus sum-type code — distinct
type-former codes at the raw level. -/
theorem Conv.idCode_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSumCode, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqSumCode

/-- An `idCode`-headed source and a `transpFill`-headed target are
not convertible.  Identity-type code versus cubical transport-fill
— ternary target, 7-tuple inversion. -/
theorem Conv.idCode_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {pathTerm intervalTerm sourceRawTerm : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqTranspFill

/-- An `idCode`-headed source and a `uaToEquiv`-headed target are
not convertible.  Identity-type code versus univalence-to-
equivalence bridge — distinct canonical heads at the raw level. -/
theorem Conv.idCode_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {proofTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqUaToEquiv, _⟩ :=
    RawStep.parStar.uaToEquiv_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqUaToEquiv

/-- An `idCode`-headed source and a `universeCode`-headed target are
not convertible.  Identity-type code versus universe-type code —
nullary target, direct equality inversion; closes the `idCode` row. -/
theorem Conv.idCode_ne_universeCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {typeCode leftCode rightCode : RawTerm scope}
    {innerLevel : Nat}
  ...
  have joinEqUniv : joinRaw = RawTerm.universeCode innerLevel :=
    RawStep.parStar.universeCode_inv targetToJoin
  nomatch joinEqId.symm.trans joinEqUniv
...
/-- A `glueIntro`-headed source and an `idCode`-headed target are not
convertible.  Cubical glue introduction versus HoTT identity-type code
— distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_idCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {typeCode leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqIdCode, _, _, _⟩ :=
    RawStep.parStar.idCode_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqIdCode

/-- A `glueIntro`-headed source and an `idStrictRefl`-headed target are
not convertible.  Cubical glue introduction versus strict-mode identity
reflexivity — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_idStrictRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {witnessTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIdStrictRefl, _⟩ :=
    RawStep.parStar.idStrictRefl_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqIdStrictRefl

/-- A `glueIntro`-headed source and an `intervalJoin`-headed target
are not convertible.  Cubical glue introduction versus interval-join
(lattice max) — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalJoin, _, _⟩ :=
    RawStep.parStar.intervalJoin_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqIntervalJoin

/-- A `glueIntro`-headed source and an `intervalMeet`-headed target
are not convertible.  Cubical glue introduction versus interval-meet
(lattice min) — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqIntervalMeet

/-- A `glueIntro`-headed source and an `intervalOpp`-headed target are
not convertible.  Cubical glue introduction versus interval-opposite
(involution) — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqIntervalOpp

/-- A `glueIntro`-headed source and a `listCode`-headed target are
not convertible.  Cubical glue introduction versus list-type code
— distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqListCode

/-- A `glueIntro`-headed source and a `modIntro`-headed target are
not convertible.  Cubical glue introduction versus modality
introduction — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqModIntro, _⟩ :=
    RawStep.parStar.modIntro_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqModIntro

/-- A `glueIntro`-headed source and an `oeqFunext`-headed target are
not convertible.  Cubical glue introduction versus observational
function extensionality witness — distinct canonical heads at the
raw level. -/
theorem Conv.glueIntro_ne_oeqFunext
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {pointwiseProof : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqFunext, _⟩ :=
    RawStep.parStar.oeqFunext_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqOeqFunext

/-- A `glueIntro`-headed source and an `oeqJ`-headed target are not
convertible.  Cubical glue introduction versus observational-equality
eliminator (J) — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_oeqJ
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {baseCase witness : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqJ, _, _⟩ :=
    RawStep.parStar.oeqJ_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqOeqJ

/-- A `glueIntro`-headed source and an `oeqRefl`-headed target are not
convertible.  Cubical glue introduction versus observational-equality
reflexivity — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {witnessTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqOeqRefl

/-- A `glueIntro`-headed source and an `oeqTrans`-headed target are not
convertible.  Cubical glue introduction versus observational-equality
transitivity — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {firstProof secondProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqOeqTrans

/-- A `glueIntro`-headed source and an `optionCode`-headed target are
not convertible.  Cubical glue introduction versus option-type code
— distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqOptionCode

/-- A `glueIntro`-headed source and a `pathCompose`-headed target are
not convertible.  Cubical glue introduction versus path composition
— distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {leftPath rightPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqPathCompose

/-- A `glueIntro`-headed source and a `piTyCode`-headed target are not
convertible.  Cubical glue introduction versus Π-type code — distinct
canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {domainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTyCode, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqPiTyCode

/-- A `glueIntro`-headed source and a `productCode`-headed target are
not convertible.  Cubical glue introduction versus product-type code
— distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProductCode, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqProductCode

/-- A `glueIntro`-headed source and a `recordIntro`-headed target are
not convertible.  Cubical glue introduction versus record introduction
— distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecordIntro, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqRecordIntro

/-- A `glueIntro`-headed source and a `refineIntro`-headed target are
not convertible.  Cubical glue introduction versus refinement-type
introduction — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqGlue.symm.trans joinEqRefineIntro

/-- A `glueIntro`-headed source and a `sessionRecv`-headed target are
not convertible.  Cubical glue introduction versus session-receive
operation — distinct canonical heads at the raw level. -/
theorem Conv.glueIntro_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {baseValue partialValue : RawTerm scope}
    {channel : RawTerm scope}
...
/-- An `equivCompose`-headed source and an `equivIntro`-headed
target are not convertible.  Equivalence composition versus
equivalence introduction witness — distinct canonical heads at the
raw level.  Opens equivCompose's compound-target sweep. -/
theorem Conv.equivCompose_ne_equivIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {forwardMap backwardMap : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqEquivIntro, _, _⟩ :=
    RawStep.parStar.equivIntro_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqEquivIntro

/-- An `equivCompose`-headed source and a `glueIntro`-headed target
are not convertible.  Equivalence composition versus cubical glue
introduction — distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_glueIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {baseCarrier glueCarrier : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqGlueIntro, _, _⟩ :=
    RawStep.parStar.glueIntro_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqGlueIntro

/-- An `equivCompose`-headed source and an `idCode`-headed target
are not convertible.  Equivalence composition versus identity-type
code — distinct canonical heads at the raw level.  Ternary target
expansion in the inversion tuple. -/
theorem Conv.equivCompose_ne_idCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {typeCode leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqIdCode, _, _, _⟩ :=
    RawStep.parStar.idCode_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqIdCode

/-- An `equivCompose`-headed source and an `idStrictRefl`-headed
target are not convertible.  Equivalence composition versus strict-
identity reflexivity witness — distinct canonical heads at the raw
level. -/
theorem Conv.equivCompose_ne_idStrictRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {witness : RawTerm scope}
  ...
  obtain ⟨_, joinEqStrictRefl, _⟩ :=
    RawStep.parStar.idStrictRefl_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqStrictRefl

/-- An `equivCompose`-headed source and an `intervalJoin`-headed
target are not convertible.  Equivalence composition versus
interval-join operation — distinct canonical heads at the raw
level. -/
theorem Conv.equivCompose_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalJoin, _, _⟩ :=
    RawStep.parStar.intervalJoin_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqIntervalJoin

/-- An `equivCompose`-headed source and an `intervalMeet`-headed
target are not convertible.  Equivalence composition versus
interval-meet operation — distinct canonical heads at the raw
level. -/
theorem Conv.equivCompose_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqIntervalMeet

/-- An `equivCompose`-headed source and an `intervalOpp`-headed
target are not convertible.  Equivalence composition versus
interval-negation operation — distinct canonical heads at the raw
level. -/
theorem Conv.equivCompose_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqIntervalOpp

/-- An `equivCompose`-headed source and a `listCode`-headed target
are not convertible.  Equivalence composition versus list-type
code — distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqListCode

/-- An `equivCompose`-headed source and a `modIntro`-headed target
are not convertible.  Equivalence composition versus modality
introduction — distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {innerTerm : RawTerm scope}
...
/-- An `equivCompose`-headed source and a `refineIntro`-headed target
are not convertible.  Equivalence composition versus refinement-type
introduction — distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqRefineIntro

/-- An `equivCompose`-headed source and a `sessionRecv`-headed target
are not convertible.  Equivalence composition versus session-receive
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqSessionRecv

/-- An `equivCompose`-headed source and a `sessionSend`-headed target
are not convertible.  Equivalence composition versus session-send
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqSessionSend

/-- An `equivCompose`-headed source and a `sigmaTyCode`-headed target
are not convertible.  Equivalence composition versus Σ-type code —
distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {sigmaDomainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqSigmaTyCode

/-- An `equivCompose`-headed source and a `subsume`-headed target are
not convertible.  Equivalence composition versus modal subsumption —
distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {innerSubsumed : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqSubsume

/-- An `equivCompose`-headed source and a `sumCode`-headed target are
not convertible.  Equivalence composition versus sum-type code —
distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {sumFirstCode sumSecondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSumCode, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqSumCode

/-- An `equivCompose`-headed source and a `transpFill`-headed target
are not convertible.  Equivalence composition versus transport-fill
operation — ternary target, three trailing slots in inversion tuple. -/
theorem Conv.equivCompose_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {pathTerm intervalTerm sourceRawTerm : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqTranspFill

/-- An `equivCompose`-headed source and a `uaToEquiv`-headed target
are not convertible.  Equivalence composition versus univalence-to-
equivalence bridge — distinct canonical heads at the raw level. -/
theorem Conv.equivCompose_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {proofTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqUaToEquiv, _⟩ :=
    RawStep.parStar.uaToEquiv_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqUaToEquiv

/-- An `equivCompose`-headed source and a `universeCode`-headed target
are not convertible.  Equivalence composition versus universe-type
code at a specific level — distinct canonical type-code heads at the
raw level.  Nullary target — direct equality inversion closes the
equivCompose source row. -/
theorem Conv.equivCompose_ne_universeCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {firstEquiv secondEquiv : RawTerm scope}
    {innerLevel : Nat}
  ...
  have joinEqUniv : joinRaw = RawTerm.universeCode innerLevel :=
    RawStep.parStar.universeCode_inv targetToJoin
  nomatch joinEqCompose.symm.trans joinEqUniv
...
/-- An `idStrictRefl`-headed source and an `oeqTrans`-headed target
are not convertible.  Strict-identity reflexivity versus
observational-equality transitivity composition. -/
theorem Conv.idStrictRefl_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {firstProof secondProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqOeqTrans

/-- An `idStrictRefl`-headed source and an `optionCode`-headed target
are not convertible.  Strict-identity reflexivity versus option-
type code — distinct canonical heads. -/
theorem Conv.idStrictRefl_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqOptionCode

/-- An `idStrictRefl`-headed source and a `pathCompose`-headed target
are not convertible.  Strict-identity reflexivity (set-truncated)
versus cubical path composition — distinct equality strata. -/
theorem Conv.idStrictRefl_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {leftPath rightPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqPathCompose

/-- An `idStrictRefl`-headed source and a `piTyCode`-headed target
are not convertible.  Strict-identity reflexivity versus Π-type
code (binder-shape codomain at `scope + 1`). -/
theorem Conv.idStrictRefl_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {domainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTyCode, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqPiTyCode

/-- An `idStrictRefl`-headed source and a `productCode`-headed target
are not convertible.  Strict-identity reflexivity versus non-
dependent product type code. -/
theorem Conv.idStrictRefl_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProductCode, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqProductCode

/-- An `idStrictRefl`-headed source and a `recordIntro`-headed target
are not convertible.  Strict-identity reflexivity versus record-
value introduction — distinct canonical heads. -/
theorem Conv.idStrictRefl_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecord, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqRecord

/-- An `idStrictRefl`-headed source and a `refineIntro`-headed target
are not convertible.  Strict-identity reflexivity versus
refinement-type introduction (value + predicate proof). -/
theorem Conv.idStrictRefl_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqRefineIntro

/-- An `idStrictRefl`-headed source and a `sessionRecv`-headed target
are not convertible.  Strict-identity reflexivity versus session-
receive operation — distinct canonical heads. -/
theorem Conv.idStrictRefl_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqSessionRecv

/-- An `idStrictRefl`-headed source and a `sessionSend`-headed target
are not convertible.  Strict-identity reflexivity versus session-
send operation — distinct canonical heads. -/
theorem Conv.idStrictRefl_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqSessionSend

/-- An `idStrictRefl`-headed source and a `sigmaTyCode`-headed target
are not convertible.  Strict-identity reflexivity versus Σ-type
code (binder-shape codomain). -/
theorem Conv.idStrictRefl_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {firstCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqSigmaTyCode

/-- An `idStrictRefl`-headed source and a `subsume`-headed target are
not convertible.  Strict-identity reflexivity versus modal
subsumption. -/
theorem Conv.idStrictRefl_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqSubsume

/-- An `idStrictRefl`-headed source and a `sumCode`-headed target are
not convertible.  Strict-identity reflexivity versus sum-type
code. -/
theorem Conv.idStrictRefl_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {leftCodeSum rightCodeSum : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSumCode, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqSumCode

/-- An `idStrictRefl`-headed source and a `transpFill`-headed target
are not convertible.  Strict-identity reflexivity versus cubical
transport-fill — ternary target, 7-tuple inversion. -/
theorem Conv.idStrictRefl_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {pathTerm intervalTerm sourceRawTerm : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqTranspFill

/-- An `idStrictRefl`-headed source and a `uaToEquiv`-headed target
are not convertible.  Strict-identity reflexivity versus
univalence-to-equivalence bridge. -/
theorem Conv.idStrictRefl_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {proofTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqUaToEquiv, _⟩ :=
    RawStep.parStar.uaToEquiv_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqUaToEquiv

/-- An `idStrictRefl`-headed source and a `universeCode`-headed target
are not convertible.  Strict-identity reflexivity versus universe-
type code — nullary target, direct equality inversion; closes the
`idStrictRefl` source row. -/
theorem Conv.idStrictRefl_ne_universeCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {witnessTerm : RawTerm scope}
    {innerLevel : Nat}
  ...
  have joinEqUniv : joinRaw = RawTerm.universeCode innerLevel :=
    RawStep.parStar.universeCode_inv targetToJoin
  nomatch joinEqIdStrictRefl.symm.trans joinEqUniv
...
/-- An `oeqFunext`-headed source and a `refineIntro`-headed target
are not convertible. -/
theorem Conv.oeqFunext_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqRefineIntro

/-- An `oeqFunext`-headed source and a `sessionRecv`-headed target are
not convertible. -/
theorem Conv.oeqFunext_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqSessionRecv

/-- An `oeqFunext`-headed source and a `sessionSend`-headed target are
not convertible. -/
theorem Conv.oeqFunext_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqSessionSend

/-- An `oeqFunext`-headed source and a `sigmaTyCode`-headed target are
not convertible. -/
theorem Conv.oeqFunext_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {firstCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqSigmaTyCode

/-- An `oeqFunext`-headed source and a `subsume`-headed target are not
convertible. -/
theorem Conv.oeqFunext_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqSubsume

/-- An `oeqFunext`-headed source and a `sumCode`-headed target are not
convertible. -/
theorem Conv.oeqFunext_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {leftCodeSum rightCodeSum : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSumCode, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqSumCode

/-- An `oeqFunext`-headed source and a `transpFill`-headed target are
not convertible.  Ternary target, 7-tuple inversion. -/
theorem Conv.oeqFunext_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {pathTerm intervalTerm sourceRawTerm : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqTranspFill

/-- An `oeqFunext`-headed source and a `uaToEquiv`-headed target are
not convertible. -/
theorem Conv.oeqFunext_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {proofTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqUaToEquiv, _⟩ :=
    RawStep.parStar.uaToEquiv_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqUaToEquiv

/-- An `oeqFunext`-headed source and a `universeCode`-headed target
are not convertible.  Nullary target, direct equality inversion;
closes the `oeqFunext` source row. -/
theorem Conv.oeqFunext_ne_universeCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {pointwiseEquality : RawTerm scope}
    {innerLevel : Nat}
  ...
  have joinEqUniv : joinRaw = RawTerm.universeCode innerLevel :=
    RawStep.parStar.universeCode_inv targetToJoin
  nomatch joinEqOeqFunext.symm.trans joinEqUniv
...
/-- An `equivIntro`-headed source and a `glueIntro`-headed target are
not convertible.  Equivalence introducer versus glue introducer —
distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_glueIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {boundaryWitness interiorWitness : RawTerm scope}
    {sourceTerm : Term context sourceType
      (RawTerm.equivIntro forwardFn backwardFn : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.glueIntro boundaryWitness interiorWitness : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  obtain ⟨_, _, joinEqGlueIntro, _, _⟩ :=
    RawStep.parStar.glueIntro_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqGlueIntro

/-- An `equivIntro`-headed source and an `idCode`-headed target are
not convertible.  Equivalence introducer versus HOTT identity-type
code — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_idCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {typeCode leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqIdCode, _, _, _⟩ :=
    RawStep.parStar.idCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqIdCode

/-- An `equivIntro`-headed source and an `idStrictRefl`-headed target
are not convertible.  Equivalence introducer versus strict-mode
identity reflexivity — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_idStrictRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {witnessTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIdStrictRefl, _⟩ :=
    RawStep.parStar.idStrictRefl_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqIdStrictRefl

/-- An `equivIntro`-headed source and an `intervalJoin`-headed target
are not convertible.  Equivalence introducer versus cubical interval
join (lattice maximum) — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalJoin, _, _⟩ :=
    RawStep.parStar.intervalJoin_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqIntervalJoin

/-- An `equivIntro`-headed source and an `intervalMeet`-headed target
are not convertible.  Equivalence introducer versus cubical interval
meet (lattice minimum) — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqIntervalMeet

/-- An `equivIntro`-headed source and an `intervalOpp`-headed target
are not convertible.  Equivalence introducer versus cubical interval
opposite (involution) — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqIntervalOpp

/-- An `equivIntro`-headed source and a `listCode`-headed target are
not convertible.  Equivalence introducer versus list-type code —
distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqListCode

/-- An `equivIntro`-headed source and a `modIntro`-headed target are
not convertible.  Equivalence introducer versus modality introduction
— distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqModIntro, _⟩ :=
    RawStep.parStar.modIntro_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqModIntro

/-- An `equivIntro`-headed source and an `oeqFunext`-headed target are
not convertible.  Equivalence introducer versus observational function
extensionality witness — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_oeqFunext
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {pointwiseProof : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqFunext, _⟩ :=
    RawStep.parStar.oeqFunext_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqOeqFunext

/-- An `equivIntro`-headed source and an `oeqJ`-headed target are not
convertible.  Equivalence introducer versus observational-equality
eliminator (J) — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_oeqJ
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {baseCase witness : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqJ, _, _⟩ :=
    RawStep.parStar.oeqJ_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqOeqJ

/-- An `equivIntro`-headed source and an `oeqRefl`-headed target are
not convertible.  Equivalence introducer versus observational-equality
reflexivity — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {witnessTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqOeqRefl

/-- An `equivIntro`-headed source and an `oeqTrans`-headed target are
not convertible.  Equivalence introducer versus observational-equality
transitivity — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {firstProof secondProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqOeqTrans

/-- An `equivIntro`-headed source and an `optionCode`-headed target
are not convertible.  Equivalence introducer versus option-type code
— distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqOptionCode

/-- An `equivIntro`-headed source and a `pathCompose`-headed target
are not convertible.  Equivalence introducer versus path composition
— distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {leftPath rightPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqPathCompose

/-- An `equivIntro`-headed source and a `piTyCode`-headed target are
not convertible.  Equivalence introducer versus Π-type code —
distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {domainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTyCode, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqPiTyCode

/-- An `equivIntro`-headed source and a `productCode`-headed target
are not convertible.  Equivalence introducer versus product-type code
— distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProductCode, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqProductCode

/-- An `equivIntro`-headed source and a `recordIntro`-headed target
are not convertible.  Equivalence introducer versus record introduction
— distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecordIntro, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqRecordIntro

/-- An `equivIntro`-headed source and a `refineIntro`-headed target
are not convertible.  Equivalence introducer versus refinement-type
introduction — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqRefineIntro

/-- An `equivIntro`-headed source and a `sessionRecv`-headed target
are not convertible.  Equivalence introducer versus session-receive
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqSessionRecv

/-- An `equivIntro`-headed source and a `sessionSend`-headed target
are not convertible.  Equivalence introducer versus session-send
operation — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqSessionSend

/-- An `equivIntro`-headed source and a `sigmaTyCode`-headed target
are not convertible.  Equivalence introducer versus Σ-type code —
distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {sigmaDomainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqSigmaTyCode

/-- An `equivIntro`-headed source and a `subsume`-headed target are
not convertible.  Equivalence introducer versus modal subsumption —
distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {innerSubsumed : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqSubsume

/-- An `equivIntro`-headed source and a `sumCode`-headed target are
not convertible.  Equivalence introducer versus sum-type code —
distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {sumFirstCode sumSecondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSumCode, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqSumCode

/-- An `equivIntro`-headed source and a `transpFill`-headed target are
not convertible.  Equivalence introducer versus transport-fill operation
— ternary target, three trailing slots in inversion tuple. -/
theorem Conv.equivIntro_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {pathTerm intervalTerm sourceRawTerm : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqTranspFill

/-- An `equivIntro`-headed source and a `uaToEquiv`-headed target are
not convertible.  Equivalence introducer versus univalence-to-equivalence
bridge — distinct canonical heads at the raw level. -/
theorem Conv.equivIntro_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {proofTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqUaToEquiv, _⟩ :=
    RawStep.parStar.uaToEquiv_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqUaToEquiv

/-- An `equivIntro`-headed source and a `universeCode`-headed target
are not convertible.  Equivalence introducer versus universe-type code
— nullary target, direct equality inversion closes equivIntro row. -/
theorem Conv.equivIntro_ne_universeCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {forwardFn backwardFn : RawTerm scope}
    {innerLevel : Nat}
  ...
  have joinEqUniv : joinRaw = RawTerm.universeCode innerLevel :=
    RawStep.parStar.universeCode_inv targetToJoin
  nomatch joinEqEquivIntro.symm.trans joinEqUniv
...
/-- An `effectPerform`-headed source and an `eitherCode`-headed
target are not convertible.  Effect perform versus sum type-code
— distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_eitherCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPerform, _, _⟩ :=
    RawStep.parStar.effectPerform_inv sourceToJoin
  obtain ⟨_, _, joinEqEitherCode, _, _⟩ :=
    RawStep.parStar.eitherCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqEitherCode

/-- An `effectPerform`-headed source and an `equivCode`-headed
target are not convertible.  Effect perform versus equivalence
type-code — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_equivCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqEquivCode, _, _⟩ :=
    RawStep.parStar.equivCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqEquivCode

/-- An `effectPerform`-headed source and an `equivCompose`-headed
target are not convertible.  Effect perform versus
equivalence-composition operation — distinct canonical heads
at the raw level. -/
theorem Conv.effectPerform_ne_equivCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {firstEquiv secondEquiv : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqEquivCompose, _, _⟩ :=
    RawStep.parStar.equivCompose_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqEquivCompose

/-- An `effectPerform`-headed source and an `equivIntro`-headed
target are not convertible.  Effect perform versus equivalence
introduction — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_equivIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {forwardMap backwardMap : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqEquivIntro, _, _⟩ :=
    RawStep.parStar.equivIntro_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqEquivIntro

/-- An `effectPerform`-headed source and a `glueIntro`-headed
target are not convertible.  Effect perform versus cubical glue
introduction — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_glueIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {baseCarrier glueCarrier : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqGlueIntro, _, _⟩ :=
    RawStep.parStar.glueIntro_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqGlueIntro

/-- An `effectPerform`-headed source and an `idCode`-headed target
are not convertible.  Effect perform versus identity-type code
(ternary at the raw level) — distinct canonical heads. -/
theorem Conv.effectPerform_ne_idCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {typeCode leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqIdCode, _, _, _⟩ :=
    RawStep.parStar.idCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqIdCode

/-- An `effectPerform`-headed source and an `idStrictRefl`-headed
target are not convertible.  Effect perform versus strict-identity
reflexivity witness — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_idStrictRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {witness : RawTerm scope}
  ...
  obtain ⟨_, joinEqStrictRefl, _⟩ :=
    RawStep.parStar.idStrictRefl_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqStrictRefl

/-- An `effectPerform`-headed source and an `intervalJoin`-headed
target are not convertible.  Effect perform versus interval-join
operation — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalJoin, _, _⟩ :=
    RawStep.parStar.intervalJoin_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqIntervalJoin

/-- An `effectPerform`-headed source and an `intervalMeet`-headed
target are not convertible.  Effect perform versus interval-meet
operation — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqIntervalMeet

/-- An `effectPerform`-headed source and an `intervalOpp`-headed
target are not convertible.  Effect perform versus interval-negation
operation — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqIntervalOpp

/-- An `effectPerform`-headed source and a `listCode`-headed
target are not convertible.  Effect perform versus list-type code
— distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqListCode

/-- An `effectPerform`-headed source and a `modIntro`-headed
target are not convertible.  Effect perform versus modality
introduction — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqModIntro, _⟩ :=
    RawStep.parStar.modIntro_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqModIntro

/-- An `effectPerform`-headed source and an `oeqFunext`-headed
target are not convertible.  Effect perform versus observational-
equality functional-extensionality witness — distinct canonical
heads at the raw level. -/
theorem Conv.effectPerform_ne_oeqFunext
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {pointwiseProof : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqFunext, _⟩ :=
    RawStep.parStar.oeqFunext_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqOeqFunext

/-- An `effectPerform`-headed source and an `oeqJ`-headed target
are not convertible.  Effect perform versus observational-equality
eliminator (J) — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_oeqJ
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {baseCase witness : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqJ, _, _⟩ :=
    RawStep.parStar.oeqJ_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqOeqJ

/-- An `effectPerform`-headed source and an `oeqRefl`-headed
target are not convertible.  Effect perform versus observational-
equality reflexivity — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {witnessTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqOeqRefl

/-- An `effectPerform`-headed source and an `oeqTrans`-headed
target are not convertible.  Effect perform versus observational-
equality transitivity — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {firstProof secondProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqOeqTrans

/-- An `effectPerform`-headed source and an `optionCode`-headed
target are not convertible.  Effect perform versus option-type
code — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqOptionCode

/-- An `effectPerform`-headed source and a `pathCompose`-headed
target are not convertible.  Effect perform versus path-composition
operation — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {leftPath rightPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqPathCompose

/-- An `effectPerform`-headed source and a `piTyCode`-headed
target are not convertible.  Effect perform versus Π-type code —
distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {domainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTyCode, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqPiTyCode

/-- An `effectPerform`-headed source and a `productCode`-headed
target are not convertible.  Effect perform versus product-type
code — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProductCode, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqProductCode

/-- An `effectPerform`-headed source and a `recordIntro`-headed
target are not convertible.  Effect perform versus record
introduction — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecordIntro, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqRecordIntro

/-- An `effectPerform`-headed source and a `refineIntro`-headed
target are not convertible.  Effect perform versus refinement-type
introduction — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqRefineIntro

/-- An `effectPerform`-headed source and a `sessionRecv`-headed
target are not convertible.  Effect perform versus session-receive
operation — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqSessionRecv

/-- An `effectPerform`-headed source and a `sessionSend`-headed
target are not convertible.  Effect perform versus session-send
operation — distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqSessionSend

/-- An `effectPerform`-headed source and a `sigmaTyCode`-headed
target are not convertible.  Effect perform versus Σ-type code —
distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {firstCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqSigmaTyCode

/-- An `effectPerform`-headed source and a `subsume`-headed target
are not convertible.  Effect perform versus modal subsumption —
distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {innerSubsumed : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqPerform.symm.trans joinEqSubsume

/-- An `effectPerform`-headed source and a `sumCode`-headed target
are not convertible.  Effect perform versus sum-type code —
distinct canonical heads at the raw level. -/
theorem Conv.effectPerform_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {operationTag arguments : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
...
/-- A `arrowCode`-headed source and an `idCode`-headed target are
not convertible.  Both type-codes but for distinct families: arrow
type versus HoTT identity type. -/
theorem Conv.arrowCode_ne_idCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {typeCode leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqIdCode, _, _, _⟩ :=
    RawStep.parStar.idCode_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqIdCode

/-- A `arrowCode`-headed source and an `idStrictRefl`-headed target
are not convertible.  Type-code versus strict-identity reflexivity
witness — distinct canonical heads. -/
theorem Conv.arrowCode_ne_idStrictRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {witness : RawTerm scope}
  ...
  obtain ⟨_, joinEqIdStrict, _⟩ :=
    RawStep.parStar.idStrictRefl_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqIdStrict

/-- A `arrowCode`-headed source and an `intervalJoin`-headed target
are not convertible.  Type-code versus interval-join operation —
distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalJoin, _, _⟩ :=
    RawStep.parStar.intervalJoin_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqIntervalJoin

/-- A `arrowCode`-headed source and an `intervalMeet`-headed target
are not convertible.  Type-code versus interval-meet operation —
distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqIntervalMeet

/-- A `arrowCode`-headed source and an `intervalOpp`-headed target
are not convertible.  Type-code versus interval-negation operation
— distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqIntervalOpp

/-- A `arrowCode`-headed source and a `listCode`-headed target are
not convertible.  Both type-codes but for distinct families: arrow
type versus list type former. -/
theorem Conv.arrowCode_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqListCode

/-- A `arrowCode`-headed source and a `modIntro`-headed target are
not convertible.  Type-code versus modal-introduction form —
distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqModIntro, _⟩ :=
    RawStep.parStar.modIntro_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqModIntro

/-- A `arrowCode`-headed source and an `oeqFunext`-headed target are
not convertible.  Type-code versus observational-equality function
extensionality witness — distinct canonical heads. -/
theorem Conv.arrowCode_ne_oeqFunext
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {pointwiseEquality : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqFunext, _⟩ :=
    RawStep.parStar.oeqFunext_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqOeqFunext

/-- A `arrowCode`-headed source and an `oeqJ`-headed target are
not convertible.  Type-code versus observational-equality J
recursor — distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_oeqJ
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {baseCase witness : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqJ, _, _⟩ :=
    RawStep.parStar.oeqJ_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqOeqJ

/-- A `arrowCode`-headed source and an `oeqRefl`-headed target are
not convertible.  Type-code versus observational-equality reflexivity
witness — distinct canonical heads. -/
theorem Conv.arrowCode_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {witness : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqOeqRefl

/-- A `arrowCode`-headed source and an `oeqTrans`-headed target are
not convertible.  Type-code versus observational-equality
transitivity witness — distinct canonical heads. -/
theorem Conv.arrowCode_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {firstEquality secondEquality : RawTerm scope}
    {sourceTerm : Term context sourceType
      (RawTerm.arrowCode domainCode codomainCode : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.oeqTrans firstEquality secondEquality : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqOeqTrans

/-- A `arrowCode`-headed source and an `optionCode`-headed target
are not convertible.  Both type-codes but for distinct families:
arrow type versus option type former. -/
theorem Conv.arrowCode_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqOptionCode

/-- A `arrowCode`-headed source and a `pathCompose`-headed target
are not convertible.  Type-code versus path-composition operation —
distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    ...
    {sourceTerm : Term context sourceType
      (RawTerm.arrowCode domainCode codomainCode : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.pathCompose firstPath secondPath : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqPathCompose

/-- A `arrowCode`-headed source and a `piTyCode`-headed target are
not convertible.  Both type-codes but for distinct families: simple
arrow versus dependent Π — siblings at the type-code level. -/
theorem Conv.arrowCode_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {piDomainCode : RawTerm scope}
    {piCodomainCode : RawTerm (scope + 1)}
    {sourceTerm : Term context sourceType
      (RawTerm.arrowCode domainCode codomainCode : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.piTyCode piDomainCode piCodomainCode : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  obtain ⟨_, _, joinEqArrow, _, _⟩ :=
    RawStep.parStar.arrowCode_inv sourceToJoin
  obtain ⟨_, _, joinEqPiTy, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqPiTy

/-- A `arrowCode`-headed source and a `productCode`-headed target
are not convertible.  Both type-codes but for distinct families:
arrow type versus product type former. -/
theorem Conv.arrowCode_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProduct, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqProduct

/-- A `arrowCode`-headed source and a `recordIntro`-headed target
are not convertible.  Type-code versus record-introduction form —
distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecord, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqRecord

/-- A `arrowCode`-headed source and a `refineIntro`-headed target are
not convertible.  Type-code versus refinement-introduction form —
distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefine, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqRefine

/-- A `arrowCode`-headed source and a `sessionRecv`-headed target are
not convertible.  Type-code versus session-receive form — distinct
canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqSessionRecv

/-- A `arrowCode`-headed source and a `sessionSend`-headed target are
not convertible.  Type-code versus session-send form — distinct
canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqSessionSend

/-- A `arrowCode`-headed source and a `sigmaTyCode`-headed target
are not convertible.  Both type-codes but for distinct families:
arrow type versus dependent Σ type code. -/
theorem Conv.arrowCode_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {sigmaFirstCode : RawTerm scope}
    {sigmaSecondCode : RawTerm (scope + 1)}
    {sourceTerm : Term context sourceType
      (RawTerm.arrowCode domainCode codomainCode : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.sigmaTyCode sigmaFirstCode sigmaSecondCode : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  obtain ⟨_, _, joinEqArrow, _, _⟩ :=
    RawStep.parStar.arrowCode_inv sourceToJoin
  obtain ⟨_, _, joinEqSigmaTy, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqSigmaTy

/-- A `arrowCode`-headed source and a `subsume`-headed target are
not convertible.  Type-code versus modal-subsumption form —
distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqSubsume

/-- A `arrowCode`-headed source and a `sumCode`-headed target are
not convertible.  Both type-codes but for distinct families: arrow
type versus sum type former. -/
theorem Conv.arrowCode_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSum, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqSum

/-- A `arrowCode`-headed source and a `transpFill`-headed target
are not convertible.  Type-code versus cubical transport-fill
operation — distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {pathTerm intervalTerm fillSource : RawTerm scope}
    {sourceTerm : Term context sourceType
      (RawTerm.arrowCode domainCode codomainCode : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.transpFill pathTerm intervalTerm fillSource : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqTranspFill

/-- A `arrowCode`-headed source and a `uaToEquiv`-headed target are
not convertible.  Type-code versus univalence-to-equivalence
operator — distinct canonical heads at the raw level. -/
theorem Conv.arrowCode_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {pathProof : RawTerm scope}
    {sourceTerm : Term context sourceType
      (RawTerm.arrowCode domainCode codomainCode : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.uaToEquiv pathProof : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  obtain ⟨_, joinEqUaToEquiv, _⟩ :=
    RawStep.parStar.uaToEquiv_inv targetToJoin
  nomatch joinEqArrow.symm.trans joinEqUaToEquiv

/-- A `arrowCode`-headed source and a `universeCode`-headed target
are not convertible.  Both type-codes but for distinct kinds: arrow
type former versus universe code (the type-of-types code).  Closes
the arrowCode compound × compound row.

The `universeCode` inv lemma is nullary: it returns a direct
equality `target = universeCode innerLevel`, not an existential
witness, because `universeCode` carries only a Nat universe level
(not a RawTerm payload).  Hence the proof uses `have` rather than
`obtain` for the target side. -/
theorem Conv.arrowCode_ne_universeCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {domainCode codomainCode : RawTerm scope}
    {innerLevel : Nat}
...
/-- A `codataUnfold`-headed source and an `intervalJoin`-headed
target are not convertible.  Codata unfold versus interval-join
operation — distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_intervalJoin
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalJoin, _, _⟩ :=
    RawStep.parStar.intervalJoin_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqIntervalJoin

/-- A `codataUnfold`-headed source and an `intervalMeet`-headed
target are not convertible.  Codata unfold versus interval-meet
operation — distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqIntervalMeet

/-- A `codataUnfold`-headed source and an `intervalOpp`-headed
target are not convertible.  Codata unfold versus interval-negation
operation — distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqIntervalOpp

/-- A `codataUnfold`-headed source and a `listCode`-headed target
are not convertible.  Codata unfold versus list-type code —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqListCode

/-- A `codataUnfold`-headed source and a `modIntro`-headed target
are not convertible.  Codata unfold versus modality introduction —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {modBody : RawTerm scope}
    {sourceTerm : Term context sourceType
      (RawTerm.codataUnfold initialState transition : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.modIntro modBody : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  obtain ⟨_, joinEqModIntro, _⟩ :=
    RawStep.parStar.modIntro_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqModIntro

/-- A `codataUnfold`-headed source and an `oeqFunext`-headed target
are not convertible.  Codata unfold versus observational-equality
function-extensionality witness — distinct canonical heads. -/
theorem Conv.codataUnfold_ne_oeqFunext
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {pointwiseEquality : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqFunext, _⟩ :=
    RawStep.parStar.oeqFunext_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqOeqFunext

/-- A `codataUnfold`-headed source and an `oeqJ`-headed target are
not convertible.  Codata unfold versus observational-equality J
recursor — distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_oeqJ
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {baseCase witness : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqJ, _, _⟩ :=
    RawStep.parStar.oeqJ_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqOeqJ

/-- A `codataUnfold`-headed source and an `oeqRefl`-headed target
are not convertible.  Codata unfold versus observational-equality
reflexivity witness — distinct canonical heads. -/
theorem Conv.codataUnfold_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {witness : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqOeqRefl

/-- A `codataUnfold`-headed source and an `oeqTrans`-headed target
are not convertible.  Codata unfold versus observational-equality
transitivity witness — distinct canonical heads. -/
theorem Conv.codataUnfold_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {firstEquality secondEquality : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqOeqTrans

/-- A `codataUnfold`-headed source and an `optionCode`-headed target
are not convertible.  Codata unfold versus option-type code —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqOptionCode

/-- A `codataUnfold`-headed source and a `pathCompose`-headed target
are not convertible.  Codata unfold versus path-composition
operation — distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {firstPath secondPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqPathCompose

/-- A `codataUnfold`-headed source and a `piTyCode`-headed target
are not convertible.  Codata unfold versus dependent Π type code
(binary at the raw level: domain code + codomain code under a
binder) — distinct canonical heads. -/
theorem Conv.codataUnfold_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {piDomainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTy, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqPiTy

/-- A `codataUnfold`-headed source and a `productCode`-headed target
are not convertible.  Codata unfold versus product type code —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProduct, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqProduct

/-- A `codataUnfold`-headed source and a `recordIntro`-headed target
are not convertible.  Codata unfold versus record introduction —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecord, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqRecord

/-- A `codataUnfold`-headed source and a `refineIntro`-headed target
are not convertible.  Codata unfold versus refinement-introduction
form — distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefine, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqRefine

/-- A `codataUnfold`-headed source and a `sessionRecv`-headed target
are not convertible.  Codata unfold versus session-receive form —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqSessionRecv

/-- A `codataUnfold`-headed source and a `sessionSend`-headed target
are not convertible.  Codata unfold versus session-send form —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqSessionSend

/-- A `codataUnfold`-headed source and a `sigmaTyCode`-headed target
are not convertible.  Codata unfold versus dependent Σ type code
(binary at the raw level: first code + second code under a binder)
— distinct canonical heads. -/
theorem Conv.codataUnfold_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {sigmaFirstCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTy, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqSigmaTy

/-- A `codataUnfold`-headed source and a `subsume`-headed target
are not convertible.  Codata unfold versus modal-subsumption form
— distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {subsumedBody : RawTerm scope}
    {sourceTerm : Term context sourceType
      (RawTerm.codataUnfold initialState transition : RawTerm scope)}
    {targetTerm : Term context targetType
      (RawTerm.subsume subsumedBody : RawTerm scope)} :
    ¬ Conv sourceTerm targetTerm := by
  intro convertibility
  obtain ⟨joinRaw, sourceToJoin, targetToJoin⟩ :=
    Conv.canonicalRaw convertibility
  ...
  obtain ⟨_, joinEqSubsume, _⟩ :=
    RawStep.parStar.subsume_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqSubsume

/-- A `codataUnfold`-headed source and a `sumCode`-headed target
are not convertible.  Codata unfold versus sum type code —
distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_sumCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {leftCode rightCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSum, _, _⟩ :=
    RawStep.parStar.sumCode_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqSum

/-- A `codataUnfold`-headed source and a `transpFill`-headed target
are not convertible.  Codata unfold versus cubical transport-fill
operation (ternary at the raw level) — distinct canonical heads. -/
theorem Conv.codataUnfold_ne_transpFill
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {pathTerm intervalTerm fillSource : RawTerm scope}
  ...
  obtain ⟨_, _, _, joinEqTranspFill, _, _, _⟩ :=
    RawStep.parStar.transpFill_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqTranspFill

/-- A `codataUnfold`-headed source and a `uaToEquiv`-headed target
are not convertible.  Codata unfold versus univalence-to-equivalence
operator — distinct canonical heads at the raw level. -/
theorem Conv.codataUnfold_ne_uaToEquiv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {pathProof : RawTerm scope}
  ...
  obtain ⟨_, joinEqUaToEquiv, _⟩ :=
    RawStep.parStar.uaToEquiv_inv targetToJoin
  nomatch joinEqCodataUnfold.symm.trans joinEqUaToEquiv

/-- A `codataUnfold`-headed source and a `universeCode`-headed
target are not convertible.  Codata unfold versus the type-of-types
code.  Closes the codataUnfold compound × compound row at 32/32.
...
(not a RawTerm payload).  Hence the proof uses `have` rather than
`obtain` for the target side. -/
theorem Conv.codataUnfold_ne_universeCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {initialState transition : RawTerm scope}
    {innerLevel : Nat}
...
/-- A `cumulUpMarker`-headed source and an `intervalMeet`-headed
target are not convertible.  Universe cumulativity marker versus
interval-meet operation — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_intervalMeet
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {leftInterval rightInterval : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqIntervalMeet, _, _⟩ :=
    RawStep.parStar.intervalMeet_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqIntervalMeet

/-- A `cumulUpMarker`-headed source and an `intervalOpp`-headed
target are not convertible.  Universe cumulativity marker versus
interval-negation — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_intervalOpp
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {intervalTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqIntervalOpp, _⟩ :=
    RawStep.parStar.intervalOpp_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqIntervalOpp

/-- A `cumulUpMarker`-headed source and a `listCode`-headed
target are not convertible.  Universe cumulativity marker versus
list-type code — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_listCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqListCode, _⟩ :=
    RawStep.parStar.listCode_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqListCode

/-- A `cumulUpMarker`-headed source and a `modIntro`-headed
target are not convertible.  Universe cumulativity marker versus
modality introduction — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_modIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {innerTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqModIntro, _⟩ :=
    RawStep.parStar.modIntro_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqModIntro

/-- A `cumulUpMarker`-headed source and an `oeqFunext`-headed
target are not convertible.  Universe cumulativity marker versus
observational-equality functional-extensionality witness —
distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_oeqFunext
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {pointwiseProof : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqFunext, _⟩ :=
    RawStep.parStar.oeqFunext_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqOeqFunext

/-- A `cumulUpMarker`-headed source and an `oeqJ`-headed
target are not convertible.  Universe cumulativity marker versus
observational-equality eliminator (J) — distinct canonical heads
at the raw level. -/
theorem Conv.cumulUpMarker_ne_oeqJ
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {baseCase witness : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqJ, _, _⟩ :=
    RawStep.parStar.oeqJ_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqOeqJ

/-- A `cumulUpMarker`-headed source and an `oeqRefl`-headed
target are not convertible.  Universe cumulativity marker versus
observational-equality reflexivity — distinct canonical heads
at the raw level. -/
theorem Conv.cumulUpMarker_ne_oeqRefl
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {witnessTerm : RawTerm scope}
  ...
  obtain ⟨_, joinEqOeqRefl, _⟩ :=
    RawStep.parStar.oeqRefl_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqOeqRefl

/-- A `cumulUpMarker`-headed source and an `oeqTrans`-headed
target are not convertible.  Universe cumulativity marker versus
observational-equality transitivity — distinct canonical heads
at the raw level. -/
theorem Conv.cumulUpMarker_ne_oeqTrans
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {firstProof secondProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqOeqTrans, _, _⟩ :=
    RawStep.parStar.oeqTrans_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqOeqTrans

/-- A `cumulUpMarker`-headed source and an `optionCode`-headed
target are not convertible.  Universe cumulativity marker versus
option-type code — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_optionCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {elementCode : RawTerm scope}
  ...
  obtain ⟨_, joinEqOptionCode, _⟩ :=
    RawStep.parStar.optionCode_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqOptionCode

/-- A `cumulUpMarker`-headed source and a `pathCompose`-headed
target are not convertible.  Universe cumulativity marker versus
path-composition operation — distinct canonical heads at the raw
level. -/
theorem Conv.cumulUpMarker_ne_pathCompose
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {leftPath rightPath : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPathCompose, _, _⟩ :=
    RawStep.parStar.pathCompose_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqPathCompose

/-- A `cumulUpMarker`-headed source and a `piTyCode`-headed
target are not convertible.  Universe cumulativity marker versus
Π-type code — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_piTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {domainCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqPiTyCode, _, _⟩ :=
    RawStep.parStar.piTyCode_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqPiTyCode

/-- A `cumulUpMarker`-headed source and a `productCode`-headed
target are not convertible.  Universe cumulativity marker versus
product-type code — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_productCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {firstCode secondCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqProductCode, _, _⟩ :=
    RawStep.parStar.productCode_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqProductCode

/-- A `cumulUpMarker`-headed source and a `recordIntro`-headed
target are not convertible.  Universe cumulativity marker versus
record introduction — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_recordIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {firstField : RawTerm scope}
  ...
  obtain ⟨_, joinEqRecordIntro, _⟩ :=
    RawStep.parStar.recordIntro_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqRecordIntro

/-- A `cumulUpMarker`-headed source and a `refineIntro`-headed
target are not convertible.  Universe cumulativity marker versus
refinement-type introduction — distinct canonical heads at the
raw level. -/
theorem Conv.cumulUpMarker_ne_refineIntro
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {rawValue predicateProof : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqRefineIntro, _, _⟩ :=
    RawStep.parStar.refineIntro_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqRefineIntro

/-- A `cumulUpMarker`-headed source and a `sessionRecv`-headed
target are not convertible.  Universe cumulativity marker versus
session-receive operation — distinct canonical heads at the raw
level. -/
theorem Conv.cumulUpMarker_ne_sessionRecv
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {channel : RawTerm scope}
  ...
  obtain ⟨_, joinEqSessionRecv, _⟩ :=
    RawStep.parStar.sessionRecv_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqSessionRecv

/-- A `cumulUpMarker`-headed source and a `sessionSend`-headed
target are not convertible.  Universe cumulativity marker versus
session-send operation — distinct canonical heads at the raw
level. -/
theorem Conv.cumulUpMarker_ne_sessionSend
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {channel payload : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSessionSend, _, _⟩ :=
    RawStep.parStar.sessionSend_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqSessionSend

/-- A `cumulUpMarker`-headed source and a `sigmaTyCode`-headed
target are not convertible.  Universe cumulativity marker versus
Σ-type code — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_sigmaTyCode
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {firstCode : RawTerm scope}
  ...
  obtain ⟨_, _, joinEqSigmaTyCode, _, _⟩ :=
    RawStep.parStar.sigmaTyCode_inv targetToJoin
  nomatch joinEqCumul.symm.trans joinEqSigmaTyCode

/-- A `cumulUpMarker`-headed source and a `subsume`-headed
target are not convertible.  Universe cumulativity marker versus
modal subsumption — distinct canonical heads at the raw level. -/
theorem Conv.cumulUpMarker_ne_subsume
    {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
    {sourceType targetType : Ty level scope}
    {innerCode : RawTerm scope}
    {innerSubsumed : RawTerm scope}
