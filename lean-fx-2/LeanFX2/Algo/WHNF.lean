...
def Term.headCtor {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
  ...
  | .oeqRefl _ _ => .oeqRefl
  | .oeqJ _ _ => .oeqJ
  | .oeqFunext _ _ _ _ _ => .oeqFunext
  | .idStrictRefl _ _ _ => .idStrictRefl
  | .idStrictRec _ _ _ => .idStrictRec
  | .modIntro _ => .modIntro
  | .modElim _ => .modElim
  | .subsume _ => .subsume
  | .interval0 => .interval0
  ...
  | .pathLam _ _ _ _ _ => .pathLam
  | .pathApp _ _ _ => .pathApp
  | .glueIntro _ _ _ _ _ => .glueIntro
  ...
  | .effectPerform _ _ _ _ _ _ => .effectPerform
  ...
  | .equivIntroHet _ _ _ _ => .equivIntroHet
...
def Term.isWHNF {mode : Mode} {level scope : Nat} {context : Ctx mode level scope}
  ...
  | .refl _ _ => true
  | .oeqRefl _ _ => true
  | .oeqFunext _ _ _ _ _ => true
  | .idStrictRefl _ _ _ => true
  | .natSucc _ => true
  | .listCons _ _ => true
  | .optionSome _ => true
  | .eitherInl _ => true
  ...
  | .sessionRecv _ => true
  | .effectPerform _ _ _ _ _ _ => true
  ...
  -- OEq J has congruence parity only in the current typed layer.
  | .oeqJ _ _ => true
  ...
  -- layer has no codata observation β-rule yet.
  | .codataDest _ => true
