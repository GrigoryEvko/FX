...
This file holds `Reducible` itself plus every closure / CR2 / CR3 /
fundamental-theorem case proved against it.  Two upstream slices
were carved out to keep elaboration parallelism healthy and to give
each sub-module a bounded axiom budget:

* `Reducibility/Basic.lean` — `RawStep.parProgress`,
  `RawTerm.isStronglyNormalizing`, `Term.isStronglyNormalizing`
  (the SN foundation that is independent of `Reducible`).
* `Reducibility/Neutral.lean` — `RawTerm.IsNeutral` inductive and
  the `..._par_preserves` cascade that proves neutrality is closed
  under raw parallel reduction.

Re-imported via the two `import` statements above so every consumer
of the historical `LeanFX2.Reducibility` symbol set sees the same
...
K12.5 attempted to extend the inductive with the
