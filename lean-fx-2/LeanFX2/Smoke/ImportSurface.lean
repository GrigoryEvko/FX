...
This smoke imports the rich public umbrella plus the separate FX1 and FX1Bridge
umbrellas, then checks the direct imports of every visible public-production
`LeanFX2.*` module.  Production modules may not depend directly on `Smoke`,
`Tools`, `Sketch`, the broad `LeanFX2` root, or host-heavy modules such as
`Lean` and `Std`; Lean's ambient `Init` prelude is handled by
declaration-dependency audits.  The same broad environment also checks FX1
source imports, prevents the legacy `LeanFX2.Lean.Kernel` scaffold path from
leaking into rich production modules, prevents rich production modules from
depending on FX1, and verifies that production modules do not import later
semantic layers.
