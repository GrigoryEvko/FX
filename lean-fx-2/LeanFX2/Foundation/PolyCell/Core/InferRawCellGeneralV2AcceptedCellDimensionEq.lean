...
/-! # InferRawCellGeneralV2AcceptedCellDimensionEq — existential preserves dim

This file ships `inferRawCellGeneralV2?_accepted_cellDimension_eq`:
...
The existential wrapper `inferRawCellGeneralV2?` returns a
`CertifiedRawCellResultV2` whose `cellDimension` is a FIELD (not a
...
1. **Unfold** `inferRawCellGeneralV2?` in the hypothesis to expose
   the underlying `match certifyRawCellExactV2? ... with`.
2. **Case** on the underlying `certifyRawCellExactV2?` result via
...
when `inferRawCellGeneralV2? scope raw` accepts, the result's
...
theorem inferRawCellGeneralV2?_accepted_cellDimension_eq
