...
/-! # Foundation/PolyCell/Core/HasEqualDimV2 — value-level dim reconciliation

This file ships the propext-free dim-equality predicate used by the
certified `PolyCellV2.generatingCell` constructor to reconcile source
and target endpoints of a dim-(n+1) generating cell.  The whole point
of v2's un-indexed raw layer (`RawCellV2 scope` carries no dim type
index) is that dim is a COMPUTED value (`RawCellV2.dim : ... → Nat`),
...
v2 dissolves the trap by un-indexing the raw layer: `RawCellV2` carries
no dim index, dim is `def RawCellV2.dim : RawCellV2 scope → Nat` (a
...
1. **Spec alignment.**  `polycell.md` §4's `PolyCellV2.generatingCell`
...
Defined as `Nat` equality on `RawCellV2.dim`.  The certified
`PolyCellV2.generatingCell` constructor takes this as a parameter so
...
@[reducible] def HasEqualDim {scope : Nat}
    (source target : RawCellV2 scope) : Prop :=
...
theorem HasEqualDim.refl {scope : Nat} (cell : RawCellV2 scope) :
...
theorem HasEqualDim.trans {scope : Nat}
    {firstCell middleCell lastCell : RawCellV2 scope}
...
/-- Identity over `RawCellV2.dim`: two cells have equal `HasEqualDim`
