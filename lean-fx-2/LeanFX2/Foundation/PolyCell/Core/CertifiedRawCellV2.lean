...
/-! # Foundation/PolyCell/Core/CertifiedRawCellV2 — raw-indexed certified package

This file ships `CertifiedRawCellV2`: the v2 analog of v1's
`CertifiedRawCell` (`Core/Certified.lean`).  It is a raw-INDEXED
certified package — like `CertifiedCellV2` (#154) but with the
`rawCell` as a TYPE PARAMETER, not a field.

## The two package types

* `CertifiedCellV2 profile scope` — EXISTENTIAL.  rawCell is a
  field, varying per cell.  Used by `inferRawCellGeneralV2?` (#163)
  — the existential wrapper.

* `CertifiedRawCellV2 profile scope rawCell` — RAW-INDEXED.
  rawCell is a parameter; the type pins the certificate to a
  specific raw input.  Used by `certifyRawCellExactV2?` (#162) —
...
```
structure CertifiedRawCellV2 (profile : PolyProfile) (scope : Nat)
    (rawCell : RawCellV2 scope) where
  sort : CellSort
  boundary : CellBoundaryV2 profile sort rawCell.dim scope
  certifiedCell : PolyCellV2 profile sort rawCell.dim scope boundary rawCell
```

Three fields (vs CertifiedCellV2's five): `sort`, `boundary`,
...
In CertifiedCellV2 the dim was an existential field (varied
per cell).  In CertifiedRawCellV2 we KNOW the rawCell, so dim is
determined: it's `rawCell.dim` (the computed function from RawCellV2).

This commits the package to dimensional honesty — you can't
construct a CertifiedRawCellV2 whose cell's dim differs from its
rawCell's dim.
...
existential dim field.  This means a CertifiedRawCellV2 with
...
structure CertifiedRawCellV2 (profile : PolyProfile) (scope : Nat)
  ...
  certifiedCell :
    PolyCellV2 profile sort rawCell.dim scope boundary rawCell
