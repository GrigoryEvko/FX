...
/-! # Foundation/PolyCell/Core/RawTermV2SubstCompose — THE HEADLINE THIRD ACTION LAW
...
Position in the cross-direction fusion ladder:
  ...
  Action RawTermSubstV2 instance  (next — closes V2-L2.7)
...
across #181a-c5:
* `RawTermSubstV2.compose` — `@[reducible]` def.
* `RawTermSubstV2.lift_compose_pointwise` — Fin pattern match;
  `⟨0,_⟩` rfl, `⟨k+1,_⟩` uses BOTH commutes from #181c4/#181c5.
* `iterateLiftRaw_RawTermSubstV2_compose_pointwise` — Nat induction.
...
`Action RawTermSubstV2` instance: ~50 lines of citing the three
...
`mu_RawTermV2 : T(T X) -> T X` at the data level. -/
@[reducible] def RawTermSubstV2.compose
...
theorem RawTermSubstV2.lift_compose_pointwise
    {sourceScope middleScope targetScope : Nat}
    (firstSubstitution : RawTermSubstV2 sourceScope middleScope)
    (secondSubstitution : RawTermSubstV2 middleScope targetScope) :
    RawTermSubstV2.PointwiseEq
        (RawTermSubstV2.compose firstSubstitution secondSubstitution).lift
        (RawTermSubstV2.compose
  ...
  | ⟨priorPositionValue + 1, hBound⟩ =>
      ...
      dsimp only [RawTermSubstV2.lift, RawTermSubstV2.compose,
                  RawTermV2.weaken]
...
theorem iterateLiftRaw_RawTermSubstV2_compose_pointwise
    {sourceScope middleScope targetScope : Nat}
    (firstSubstitution : RawTermSubstV2 sourceScope middleScope)
    (secondSubstitution : RawTermSubstV2 middleScope targetScope)
    (binderDepth : Nat) :
  ...
  | succ _priorDepth priorIH =>
      ...
      exact RawTermSubstV2.lift_compose_pointwise
              (iterateLiftRaw firstSubstitution _priorDepth)
              (iterateLiftRaw secondSubstitution _priorDepth)
              position
...
theorem RawTermV2.subst_compose
    {sourceScope middleScope targetScope : Nat}
    (firstSubstitution : RawTermSubstV2 sourceScope middleScope)
    (secondSubstitution : RawTermSubstV2 middleScope targetScope)
  ...
  | .mkGen someGenerator somePayload someChildren =>
    ...
    case neg =>
      ...
      show RawTermV2.subst secondSubstitution
      ...
      dsimp only [RawTermV2.subst, foldV2, GenAlgebraV2.canonical]
      ...
      · -- Children fusion via mutual IH.
        exact RawTermChildrenV2.subst_compose
...
`iterateLiftRaw_RawTermSubstV2_compose_pointwise` (.symm) +
subst_pointwise closes. -/
theorem RawTermChildrenV2.subst_compose
    {sourceScope middleScope targetScope : Nat}
    (firstSubstitution : RawTermSubstV2 sourceScope middleScope)
    (secondSubstitution : RawTermSubstV2 middleScope targetScope)
    {binderShifts : List Nat}
    (someChildren : RawTermChildrenV2 binderShifts sourceScope) :
    RawTermChildrenV2.subst secondSubstitution
  ...
  | headShift :: _, .childCons childHead childTail =>
      show RawTermChildrenV2.childCons
              (RawTermV2.subst (iterateLiftRaw secondSubstitution headShift)
                  (RawTermV2.subst (iterateLiftRaw firstSubstitution headShift)
                      childHead))
              (RawTermChildrenV2.subst secondSubstitution
      ...
      have headBridge :
        ...
        RawTermV2.subst_pointwise
          ...
          childHead
      have tailFusion :=
        RawTermChildrenV2.subst_compose
...
theorem RawTermV2.subst_compose_unit_smoke
    {sourceScope middleScope targetScope : Nat}
    (firstSubstitution : RawTermSubstV2 sourceScope middleScope)
    (secondSubstitution : RawTermSubstV2 middleScope targetScope) :
    RawTermV2.subst secondSubstitution
