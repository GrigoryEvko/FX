...
/-! # InferRawCellGeneralV2Sound — existential no-laundering keystone

This file ships `inferRawCellGeneralV2?_sound`: the third and final
...
Combined with:
* #167 `inferRawCellGeneralV2?_accepted_cellDimension_eq` — dim preserves
* #168 `inferRawCellGeneralV2?_accepted_rawCell_heq` — raw preserves
...
1. `result.certifiedCell.raw = result.rawCell` — by definition of
   `PolyCellV2.raw` (the extractor returns the implicit rawCell
   index, which for `result.certifiedCell : PolyCellV2 ... result.rawCell`
   is `result.rawCell`).  Closes by `rfl`.

2. `HEq result.rawCell raw` — by #168
   (`inferRawCellGeneralV2?_accepted_rawCell_heq`).

The composition:
```
HEq.trans
  (heq_of_eq (show result.certifiedCell.raw = result.rawCell from rfl))
  (inferRawCellGeneralV2?_accepted_rawCell_heq accepted)
```

In v2 (un-indexed rawCell, all types `RawCellV2 scope`), the HEq
...
* `inferRawCellGeneralV2?_accepted_rawCell_heq` (#168, zero-axiom)
...
theorem inferRawCellGeneralV2?_sound
