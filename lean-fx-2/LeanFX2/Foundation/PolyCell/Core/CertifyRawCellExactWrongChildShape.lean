...
Two reachability witnesses that invoke `certifyChildrenInlineFueled?`
...
The probes call `certifyChildrenInlineFueled?` directly with
...
3. Recursively certifies `(.termBase unit)` to
   `headCert : CertifiedRawCell ... 0 unit` with `sort = .term`.
...
Invokes `certifyChildrenInlineFueled?` directly with a
...
theorem certifyChildrenInlineFueled?_typeSort_rejects_termChild :
    ...
    certifyChildrenInlineFueled? (profile := fxProfile)
...
Invokes `certifyChildrenInlineFueled?` directly with a childSpec
...
theorem certifyChildrenInlineFueled?_nonZeroDim_rejects_termChild :
    let unitChild : RawTerm 0 := .mkGen .gen_unit () .childNil
    let dimMismatchSpec : ChildSpec :=
