...
/-! # Foundation/PolyCell/Core/RawTermV2RenameCompose — renaming-side lift-compose fusion
...
`RawTermV2` is needed.  The headline `RawRenaming.lift_compose_pointwise`
is a pure Fin pattern match (0 / k+1) that closes by `rfl` on both
branches.  The iterated version is a Nat induction.

The TERM-level fusion (`RawTermV2.rename_compose`) ships in a
...
arithmetic; no `RawTermV2` induction. -/
...
theorem RawRenaming.lift_compose_pointwise
    ...
    RawRenaming.PointwiseEq
        (LeanFX2.RawRenaming.compose firstRenaming secondRenaming).lift
        (LeanFX2.RawRenaming.compose firstRenaming.lift secondRenaming.lift) := by
  intro position
  match position with
  | ⟨0, _⟩ => rfl
...
binder depths.  Needed by future foldV2-based theorems
(`RawTermV2.rename_compose`, `subst_compose`) at each `childCons`
spine descent. -/
...
theorem RawRenaming.lift_compose_pointwise_zero_smoke
    ...
    (LeanFX2.RawRenaming.compose firstRenaming secondRenaming).lift
      ...
      LeanFX2.RawRenaming.compose firstRenaming.lift secondRenaming.lift
        (⟨0, Nat.zero_lt_succ sourceScope⟩ : Fin (sourceScope + 1)) :=
...
theorem iterateLiftRaw_RawRenaming_compose_pointwise_zero_smoke
    ...
    iterateLiftRaw
        (LeanFX2.RawRenaming.compose firstRenaming secondRenaming) 0
        position =
      LeanFX2.RawRenaming.compose
