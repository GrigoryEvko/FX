...
## Dim-0 boundary uniqueness (deferred)
...
`CellBoundaryV2.trivial`.  A dedicated `headAtDim0` helper that
performs the dim-cast in one step is deferred to a follow-up — the
cast requires dependent elimination across both the dim AND the
boundary (the boundary's type is dim-indexed), which `subst`
cannot dispatch directly when the LHS is a field projection
(`headSpec.cellDimension`) rather than a free variable.  Callers
can still chain `headWithBoundary` with a `Subsingleton.elim` on
the boundary at dim 0, just not as a one-liner.
