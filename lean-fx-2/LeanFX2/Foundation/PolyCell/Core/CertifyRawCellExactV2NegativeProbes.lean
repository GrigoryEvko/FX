...
/-! # CertifyRawCellExactV2NegativeProbes — rejection-side coverage suite
...
* `.wrongSort` — `checkRawCellAsV2?` with expectedSort != inferred sort.

The remaining seven rejection branches in `CellCheckRejection.all` are
forward-compat dead code under fxProfile:

* `.unknownGenerator` — `supportedGeneratorV2?` always returns `some`
  under fxProfile (all 194 admitted).  Restricted profiles (e.g.
  "no-HoTT FX") refine `SupportedGeneratorV2` with fewer arms,
...
* `.fuelExhausted` — top-level wrapper supplies `raw.size + 1` fuel,
  ...
  `certifyRawCellExactV2Fueled?` with insufficient fuel.
* `.badBoundaryEndpoint`, `.unsupportedCertification` — used by v2's
  `buildGeneratingCellExactV2?` and reserved for future ingress
...
pure computation: given a closed malformed RawCellV2 input, it reduces
via the fuel wrapper + structural dispatch to a definite `Except.error
<rejection>` value.

The reduction chain for `.unsupportedCompH`:

1. `inferRawCellGeneralV2? scope rawCell` → matches on `certifyRawCellExactV2?`
2. `certifyRawCellExactV2? scope rawCell`
   → `certifyRawCellExactV2Fueled? (rawCell.size + 1) scope rawCell`
...
7. `certifiedResultRejectionV2?` projects: `some .unsupportedCompH`
...
1. `checkRawCellAsV2? .type 0 unitTermRaw`
2. Calls `inferRawCellGeneralV2? 0 unitTermRaw`
...
* The `certifiedResultRejectionV2?` helper is a one-line `def`
  matching on Except
...
Dual of `certifiedResultSortV2?` (#170).  Where the positive coverage
...
def certifiedResultRejectionV2? {profile : PolyProfile} {scope : Nat} :
...
def horizontalCompositeUnitRaw : RawCellV2 0 :=
  .horizontalComposite
    (.termBase (.mkGen .gen_unit () .childNil))
    (.termBase (.mkGen .gen_unit () .childNil))
...
def verticalCompositeUnitRaw : RawCellV2 0 :=
  .verticalComposite
    (.termBase (.mkGen .gen_unit () .childNil))
    (.termBase (.mkGen .gen_unit () .childNil))

end NegativeProbesV2
...
theorem negative_verticalCompositeUnit_rejects_badVerticalBoundary
    ...
    certifiedResultRejectionV2?
      (inferRawCellGeneralV2? (profile := profile) 0
        NegativeProbesV2.verticalCompositeUnitRaw) =
...
Closes by `rfl`: `inferRawCellGeneralV2?` accepts the unit term at
inferred sort `.term` (per #170's `coverage_unitTermRaw_sort`), but
`checkRawCellAsV2?`'s sort comparison rejects when the expected sort
is `.type`.

This is the only `.wrongSort` rejection currently reachable: bare
`inferRawCellGeneralV2?` has no external sort expectation and never
rejects with `.wrongSort` — the rejection class is specific to
`checkRawCellAsV2?`'s expected-shape contract.

Pattern note: rather than introduce a separate negative fixture,
this theorem REUSES `CoverageV2.unitTermRaw` from #170.  Same raw
...
theorem negative_unitTerm_as_type_rejects_wrongSort
    {profile : PolyProfile} :
    (match checkRawCellAsV2? (profile := profile) .type 0 CoverageV2.unitTermRaw with
      | Except.ok _ => none
      | Except.error rejection => some rejection) =
      some .wrongSort := rfl
