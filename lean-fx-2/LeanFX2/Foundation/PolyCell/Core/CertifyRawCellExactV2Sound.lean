...
/-! # Foundation/PolyCell/Core/CertifyRawCellExactV2Sound — raw-indexed soundness (type-level)

This file ships `certifyRawCellExactV2?_sound`: the **type-level
...
* `CertifyRawCellExactV2Shape.lean` (V2-fix-1, this commit) — shape
  pin lemmas (`certifyRawCellExactV2?_identityCell_boundary`, etc.)
  that DO inspect the acceptance hypothesis and discharge case
  splits on the recursive call's result.
* `CertifyRawCellExactV2CompHRejects.lean` (#166) — totality-off-compH
...
* `certifyRawCellExactV2?_identityCell_boundary` and other shape
  lemmas in `CertifyRawCellExactV2Shape.lean` (V2-fix-1) — behavioral
