...
/-! # Foundation/PolyCell/Core/RawTermV2Subst0 — single-variable subst (β-substrate)
...
* `RawTermSubstV2.singleton rawArg : RawTermSubstV2 (scope + 1) scope`
...
* `RawTermV2.subst0 body rawArg : RawTermV2 scope` — convenience
...
Both `RawTermSubstV2.singleton` and `RawTermV2.subst0` are marked
...
* **`RawTermSubstV2.singleton_rename_commute_pointwise`** and other
  commute lemmas — these live with the v1 analogs in
  `Foundation/RawSubst/SubstIdentityAndBeta.lean`.  The v2 cascade
  laws file (`RawCellV2CascadeLaws.lean`) already covers the
...
* `RawTermSubstV2.identity : RawTermSubstV2 scope scope` — maps each
...
@[reducible] def RawTermSubstV2.singleton {scope : Nat}
    (rawArg : RawTermV2 scope) :
    RawTermSubstV2 (scope + 1) scope :=
  fun position =>
    match position with
    | ⟨0, _⟩      => rawArg
    | ⟨k + 1, h⟩  =>
...
@[reducible] def RawTermV2.subst0 {scope : Nat}
    (body : RawTermV2 (scope + 1)) (rawArg : RawTermV2 scope) :
    RawTermV2 scope :=
  RawTermV2.subst (RawTermSubstV2.singleton rawArg) body

/-- Behavior pin: singleton's position-0 entry returns the substituent. -/
theorem RawTermSubstV2.singleton_var_zero {scope : Nat}
    (rawArg : RawTermV2 scope) :
    RawTermSubstV2.singleton rawArg ⟨0, Nat.zero_lt_succ scope⟩ = rawArg := rfl

/-- Behavior pin: singleton's position-(k+1) entry returns variable k
(the shift-down). -/
theorem RawTermSubstV2.singleton_var_succ {scope : Nat}
    (rawArg : RawTermV2 scope) (k : Nat) (hBound : k + 1 < scope + 1) :
    RawTermSubstV2.singleton rawArg ⟨k + 1, hBound⟩ =
...
theorem RawTermV2.subst0_var_zero {scope : Nat} (rawArg : RawTermV2 scope) :
    RawTermV2.subst0
...
theorem RawTermV2.subst0_var_succ_one_smoke (rawArg : RawTermV2 1) :
    RawTermV2.subst0
...
smoke lemmas. -/
theorem RawTermV2.subst0_unit_smoke {scope : Nat} (rawArg : RawTermV2 scope) :
    RawTermV2.subst0 (.mkGen .gen_unit () .childNil) rawArg =
      (.mkGen .gen_unit () .childNil : RawTermV2 scope) := rfl
