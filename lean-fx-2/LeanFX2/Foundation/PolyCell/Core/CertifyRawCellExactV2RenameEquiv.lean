...
/-! # Foundation/PolyCell/Core/CertifyRawCellExactV2RenameEquiv
...
∀ {sourceScope targetScope} (rho : RawRenaming sourceScope targetScope)
  (term : RawTermV2 sourceScope),
  certifiedResultSortV2?
      (inferRawCellGeneralV2? sourceScope (.termBase term))
    = certifiedResultSortV2?
        (inferRawCellGeneralV2? targetScope
          (.termBase (RawTermV2.rename rho term)))
```

This requires structural induction over `RawTermV2` + the foldV2
...
1 under Fin.succ-based weakening.  If the foldV2's lift-by-shift
...
def unitRenamedToScope1 : RawCellV2 1 :=
  RawCellV2.rename LeanFX2.RawRenaming.weaken unitTermRaw

/-- `varZeroRaw` (scope 1, `Fin 1` payload at 0) renamed to scope 2
via weaken (`Fin 2` payload at 1 -- the index shifts up). -/
def varZeroRenamedToScope2 : RawCellV2 2 :=
  RawCellV2.rename LeanFX2.RawRenaming.weaken varZeroRaw

/-- `pairUnitsRaw` (scope 0, arity-2 spine of units) renamed to
scope 1. -/
def pairUnitsRenamedToScope1 : RawCellV2 1 :=
  RawCellV2.rename LeanFX2.RawRenaming.weaken pairUnitsRaw

/-- `identityUnitCellRaw` (cell-layer fixture) renamed to scope 1. -/
def identityUnitCellRenamedToScope1 : RawCellV2 1 :=
  RawCellV2.rename LeanFX2.RawRenaming.weaken identityUnitCellRaw
...
theorem coverage_unitRaw_renamed_sort {profile : PolyProfile} :
    certifiedResultSortV2?
      (inferRawCellGeneralV2? (profile := profile) 1
        CoverageV2.unitRenamedToScope1) =
      some .term := rfl
...
theorem coverage_varZeroRaw_renamed_sort {profile : PolyProfile} :
    certifiedResultSortV2?
      (inferRawCellGeneralV2? (profile := profile) 2
        CoverageV2.varZeroRenamedToScope2) =
      some .term := rfl
...
theorem coverage_pairUnitsRaw_renamed_sort {profile : PolyProfile} :
    certifiedResultSortV2?
      (inferRawCellGeneralV2? (profile := profile) 1
        CoverageV2.pairUnitsRenamedToScope1) =
      some .term := rfl
...
theorem coverage_identityUnitCellRaw_renamed_sort {profile : PolyProfile} :
    certifiedResultSortV2?
      (inferRawCellGeneralV2? (profile := profile) 1
        CoverageV2.identityUnitCellRenamedToScope1) =
      some .term := rfl
...
theorem rename_equiv_unitTerm_sort_agree {profile : PolyProfile} :
    certifiedResultSortV2?
      ...
      = certifiedResultSortV2?
          (inferRawCellGeneralV2? (profile := profile) 1
            CoverageV2.unitRenamedToScope1) := rfl
