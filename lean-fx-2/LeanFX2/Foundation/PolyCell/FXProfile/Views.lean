...
theorem isTermCell_false_of_isTypeCell (cell : FXCellAt 0)
    (hasTypeCell : cell.isTypeCell = true) :
    cell.isTermCell = false := by
  cases cell with
  | atom cellId payload =>
...
theorem FXTerm.toCell_isTypeCell_false (term : FXTerm) :
  ...
  exact isTypeCell_false_of_isTermCell
    term.toCell term.toCell_isTermCell

theorem FXType.toCell_isTermCell_false (typeCell : FXType) :
    typeCell.toCell.isTermCell = false := by
  exact isTermCell_false_of_isTypeCell
    typeCell.toCell typeCell.toCell_isTypeCell
...
/-- Cell id extraction for raw dim-zero cells. -/
def FXCellAtZero.cellId : FXCellAt 0 → CellId
  | .atom cellId _ => cellId

/-- Payload extraction for raw dim-zero cells. -/
def FXCellAtZero.payload : FXCellAt 0 → Nat
  | .atom _ payload => payload
...
theorem FXTerm.toCell?_ofCellId?_of_isTermConstructor
    (cellId : CellId) (payload : Nat)
    (hasTermConstructor :
      (classifyDimZeroCellId cellId).isTermConstructor = true) :
    Option.map FXTerm.toCell (FXTerm.ofCellId? cellId payload) =
...
theorem FXType.toCell?_ofCellId?_of_isTypeConstructor
    (cellId : CellId) (payload : Nat)
    (hasTypeConstructor :
      (classifyDimZeroCellId cellId).isTypeConstructor = true) :
    Option.map FXType.toCell (FXType.ofCellId? cellId payload) =
      some (PolyTerm.atom (profile := fxProfile) cellId payload :
        FXCellAt 0) := by
  unfold FXType.ofCellId?
...
theorem FXTerm.constructorName?_ofCellId?_of_isTermConstructor
    (cellId : CellId) (payload : Nat)
    (hasTermConstructor :
      (classifyDimZeroCellId cellId).isTermConstructor = true) :
...
theorem FXType.constructorName?_ofCellId?_of_isTypeConstructor
    (cellId : CellId) (payload : Nat)
    (hasTypeConstructor :
      (classifyDimZeroCellId cellId).isTypeConstructor = true) :
...
theorem FXTerm.ofCell?_of_not_isTermCell (cell : FXCellAt 0)
  ...
  | atom cellId payload =>
      ...
      by_cases hasTermRange : cellId < termGeneratorCount
      · have hasTermCell :
            (PolyTerm.atom (profile := fxProfile) cellId payload).isTermCell =
              true := by
          change decide (cellId < PolyTerm.termCellIdLimit) = true
          exact decide_eq_true hasTermRange
        exact False.elim (hasNotTermCell hasTermCell)
      · rw [dif_neg hasTermRange]
        by_cases hasCurrentRange : cellId < totalGeneratorCount
        · rw [dif_pos hasCurrentRange]
        · rw [dif_neg hasCurrentRange]
...
theorem FXType.toCell?_ofCell?_of_isTypeCell (cell : FXCellAt 0)
    (hasTypeCell : cell.isTypeCell = true) :
    Option.map FXType.toCell (FXType.ofCell? cell) =
      some cell := by
  cases cell with
  | atom cellId payload =>
      change
  ...
  | atom cellId payload =>
      ...
      · rw [dif_neg hasTermRange]
        ...
        · have hasLowerBound : PolyTerm.firstTypeCellId ≤ cellId := by
            ...
            exact Nat.le_of_not_gt hasTermRange
          have hasTypeCell :
              (PolyTerm.atom (profile := fxProfile) cellId payload).isTypeCell =
                true := by
            change
              (decide (PolyTerm.firstTypeCellId ≤ cellId) &&
                decide (cellId < PolyTerm.typeCellIdLimit)) = true
            have hasLowerBoundDecide :
                decide (PolyTerm.firstTypeCellId ≤ cellId) = true :=
              decide_eq_true hasLowerBound
            have hasUpperBoundDecide :
                decide (cellId < PolyTerm.typeCellIdLimit) = true := by
              change decide (cellId < totalGeneratorCount) = true
              exact decide_eq_true hasCurrentRange
            rw [hasLowerBoundDecide, hasUpperBoundDecide]
            rfl
...
theorem FXTerm.cellId_ofCell?_of_isTermCell (cell : FXCellAt 0)
    (hasTermCell : cell.isTermCell = true) :
    Option.map FXTerm.cellId (FXTerm.ofCell? cell) =
...
theorem FXTerm.payload_ofCell?_of_isTermCell (cell : FXCellAt 0)
    (hasTermCell : cell.isTermCell = true) :
    Option.map FXTerm.payload (FXTerm.ofCell? cell) =
...
theorem FXType.cellId_ofCell?_of_isTypeCell (cell : FXCellAt 0)
    (hasTypeCell : cell.isTypeCell = true) :
    Option.map FXType.cellId (FXType.ofCell? cell) =
      some (FXCellAtZero.cellId cell) := by
  cases cell with
  | atom cellId payload =>
      change
...
theorem FXType.payload_ofCell?_of_isTypeCell (cell : FXCellAt 0)
    (hasTypeCell : cell.isTypeCell = true) :
    Option.map FXType.payload (FXType.ofCell? cell) =
      some (FXCellAtZero.payload cell) := by
  cases cell with
  | atom cellId payload =>
      change
...
theorem FXTerm.constructorName?_ofCell?_of_isTermCell
    (cell : FXCellAt 0) (hasTermCell : cell.isTermCell = true) :
    (FXTerm.ofCell? cell).bind FXTerm.constructorName? =
...
theorem FXType.constructorName?_ofCell?_of_isTypeCell
    (cell : FXCellAt 0) (hasTypeCell : cell.isTypeCell = true) :
    (FXType.ofCell? cell).bind FXType.constructorName? =
...
theorem FXTerm.toCell?_ofCell?_toCell (term : FXTerm) :
    Option.map FXTerm.toCell (FXTerm.ofCell? term.toCell) =
      some term.toCell := by
  exact FXTerm.toCell?_ofCell?_of_isTermCell
    term.toCell term.toCell_isTermCell
...
theorem FXType.toCell?_ofCell?_toCell (typeCell : FXType) :
  ...
  exact FXType.toCell?_ofCell?_of_isTypeCell
    typeCell.toCell typeCell.toCell_isTypeCell
...
theorem FXTerm.cellId_ofCell?_toCell (term : FXTerm) :
  ...
  | mk cell hasTermCell =>
      cases cell with
      | atom cellId payload =>
          exact FXTerm.cellId_ofCell?_of_isTermCell
...
theorem FXTerm.payload_ofCell?_toCell (term : FXTerm) :
  ...
  | mk cell hasTermCell =>
      cases cell with
      | atom cellId payload =>
          exact FXTerm.payload_ofCell?_of_isTermCell
            (.atom cellId payload) hasTermCell
...
theorem FXTerm.constructorIndex_val_ofCell?_toCell (term : FXTerm) :
  ...
  | mk cell hasTermCell =>
      cases cell with
      | atom cellId payload =>
          exact FXTerm.constructorIndex_val_ofCell?_of_isTermCell
            (.atom cellId payload) hasTermCell
...
theorem FXType.cellId_ofCell?_toCell (typeCell : FXType) :
  ...
  | mk cell hasTypeCell =>
      cases cell with
      | atom cellId payload =>
          exact FXType.cellId_ofCell?_of_isTypeCell
...
theorem FXType.payload_ofCell?_toCell (typeCell : FXType) :
  ...
  | mk cell hasTypeCell =>
      cases cell with
      | atom cellId payload =>
          exact FXType.payload_ofCell?_of_isTypeCell
            (.atom cellId payload) hasTypeCell
...
theorem FXType.constructorIndex_val_ofCell?_toCell (typeCell : FXType) :
  ...
  | mk cell hasTypeCell =>
      cases cell with
      | atom cellId payload =>
          exact FXType.constructorIndex_val_ofCell?_of_isTypeCell
            (.atom cellId payload) hasTypeCell

theorem FXTerm.constructorName?_ofCell?_toCell (term : FXTerm) :
    (FXTerm.ofCell? term.toCell).bind FXTerm.constructorName? =
      term.constructorName? := by
  cases term with
  | mk cell hasTermCell =>
      cases cell with
      | atom cellId payload =>
          exact FXTerm.constructorName?_ofCell?_of_isTermCell
            (.atom cellId payload) hasTermCell

theorem FXType.constructorName?_ofCell?_toCell (typeCell : FXType) :
    (FXType.ofCell? typeCell.toCell).bind FXType.constructorName? =
      typeCell.constructorName? := by
  cases typeCell with
  | mk cell hasTypeCell =>
      cases cell with
      | atom cellId payload =>
...
theorem FXTerm.ofCell?_ofType_toCell (typeCell : FXType) :
  ...
  exact FXTerm.ofCell?_of_not_isTermCell typeCell.toCell
    (by
      intro hasTermCell
      have hasTermCellFalse :=
        isTermCell_false_of_isTypeCell
          typeCell.toCell typeCell.toCell_isTypeCell
      rw [hasTermCell] at hasTermCellFalse
      cases hasTermCellFalse)
...
theorem FXType.ofCell?_ofTerm_toCell (term : FXTerm) :
  ...
  exact FXType.ofCell?_of_not_isTypeCell term.toCell
    (by
      intro hasTypeCell
      have hasTypeCellFalse :=
        isTypeCell_false_of_isTermCell
          term.toCell term.toCell_isTermCell
      rw [hasTypeCell] at hasTypeCellFalse
      cases hasTypeCellFalse)
