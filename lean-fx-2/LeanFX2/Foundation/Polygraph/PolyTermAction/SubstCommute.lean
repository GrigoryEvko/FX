...
theorem RawPolyTerm.subst_pointwise {sourceScope targetScope : Nat}
  ...
  | var position =>
      dsimp only [RawPolyTerm.subst]; rw [substEq position]
  | unit => rfl
  | lam body bodyIH =>
      dsimp only [RawPolyTerm.subst]
      rw [bodyIH (RawPolyTermSubst.lift_pointwise substEq)]
  | app fn arg fnIH argIH =>
      dsimp only [RawPolyTerm.subst]; rw [fnIH substEq, argIH substEq]
  | pair fv sv fvIH svIH =>
      dsimp only [RawPolyTerm.subst]; rw [fvIH substEq, svIH substEq]
  | fst pairTerm pairIH =>
      dsimp only [RawPolyTerm.subst]; rw [pairIH substEq]
  | snd pairTerm pairIH =>
      dsimp only [RawPolyTerm.subst]; rw [pairIH substEq]
  | boolTrue => rfl
  | boolFalse => rfl
  ...
  | natZero => rfl
  | natSucc p pIH =>
      dsimp only [RawPolyTerm.subst]; rw [pIH substEq]
  ...
  | optionNone => rfl
  | optionSome v vIH =>
      dsimp only [RawPolyTerm.subst]; rw [vIH substEq]
  ...
  | refl witness witnessIH =>
      dsimp only [RawPolyTerm.subst]; rw [witnessIH substEq]
  | idJ base witness baseIH witnessIH =>
      dsimp only [RawPolyTerm.subst]
      rw [baseIH substEq, witnessIH substEq]
  | modIntro inner innerIH =>
      dsimp only [RawPolyTerm.subst]; rw [innerIH substEq]
  ...
  | interval1 => rfl
  | intervalOpp i iIH =>
      dsimp only [RawPolyTerm.subst]; rw [iIH substEq]
  | intervalMeet l r lIH rIH =>
      dsimp only [RawPolyTerm.subst]; rw [lIH substEq, rIH substEq]
  ...
  | glueElim gluedValue gluedIH =>
      dsimp only [RawPolyTerm.subst]; rw [gluedIH substEq]
  ...
  | hcomp sides cap sidesIH capIH =>
      dsimp only [RawPolyTerm.subst]; rw [sidesIH substEq, capIH substEq]
  | oeqRefl witness witnessIH =>
      dsimp only [RawPolyTerm.subst]; rw [witnessIH substEq]
  | oeqJ baseCase witness baseIH witnessIH =>
      dsimp only [RawPolyTerm.subst]; rw [baseIH substEq, witnessIH substEq]
  | oeqFunext pointwiseEquality pointwiseIH =>
      dsimp only [RawPolyTerm.subst]; rw [pointwiseIH substEq]
  ...
  | refineIntro rawValue predicateProof rawIH proofIH =>
      dsimp only [RawPolyTerm.subst]; rw [rawIH substEq, proofIH substEq]
  | refineElim refinedValue refinedIH =>
      dsimp only [RawPolyTerm.subst]; rw [refinedIH substEq]
  | recordIntro firstField firstIH =>
      dsimp only [RawPolyTerm.subst]; rw [firstIH substEq]
  | recordProj recordValue recordIH =>
      dsimp only [RawPolyTerm.subst]; rw [recordIH substEq]
  ...
  | listCode elementCode elementIH =>
      dsimp only [RawPolyTerm.subst]; rw [elementIH substEq]
  ...
  | uaToEquiv proofRaw proofIH =>
      dsimp only [RawPolyTerm.subst]; rw [proofIH substEq]
  | equivApply equivRaw argRaw equivIH argIH =>
      dsimp only [RawPolyTerm.subst]; rw [equivIH substEq, argIH substEq]
...
theorem RawTerm.subst_toRawPoly_commute :
  ...
  | lam body bodyIH =>
      intro substitution
      dsimp only [RawTerm.subst, RawTerm.toRawPoly, RawPolyTerm.subst]
