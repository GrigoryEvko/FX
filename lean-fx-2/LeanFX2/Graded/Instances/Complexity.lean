...
/-! # Graded/Instances/Complexity — bounded-or-unbounded cost semiring

`ComplexityGrade` tracks checked cost bounds.  A bare `Nat` is not
faithful for FX because the language explicitly distinguishes bounded
cost from `unbounded`.  The carrier is therefore `Nat ∪ {unbounded}`.

Operations:

* `0` = `bound 0`
* `1` = `bound 1`
* `+` = cost-bound addition, with `unbounded` absorbing
...
This is still a conservative v0.5 model of asymptotic complexity: it
does not yet quotient by Big-O equivalence.  It is materially stronger
than the old Nat wrapper because it represents the spec-mandated
`unbounded` case and keeps the semiring laws honest.
...
/-- Complexity grade: finite natural bound or unbounded cost. -/
inductive ComplexityGrade : Type
  /-- A checked finite cost bound. -/
  | bound (limit : Nat)
  /-- Explicitly unbounded cost. -/
  | unbounded
...
def add : ComplexityGrade → ComplexityGrade → ComplexityGrade
...
def mul : ComplexityGrade → ComplexityGrade → ComplexityGrade
  | .bound leftBound, .bound rightBound => .bound (leftBound * rightBound)
...
def le : ComplexityGrade → ComplexityGrade → Prop
  | _, .unbounded => True
...
/-- Complexity bounds form `WithTop Nat` under natural addition and
...
instance : GradeSemiring ComplexityGrade where
  ...
  add_assoc := ComplexityGrade.add_assoc
  add_comm := ComplexityGrade.add_comm
  add_zero_left := ComplexityGrade.add_zero_left
  add_zero_right := ComplexityGrade.add_zero_right
  mul_assoc := ComplexityGrade.mul_assoc
  mul_one_left := ComplexityGrade.mul_one_left
  mul_one_right := ComplexityGrade.mul_one_right
  mul_distrib_left := ComplexityGrade.mul_distrib_left
  mul_distrib_right := ComplexityGrade.mul_distrib_right
  mul_zero_left := ComplexityGrade.mul_zero_left
  mul_zero_right := ComplexityGrade.mul_zero_right
  le_refl := ComplexityGrade.le_refl
  le_trans := ComplexityGrade.le_trans
  add_mono := ComplexityGrade.add_mono
  mul_mono := ComplexityGrade.mul_mono

/-! ## Smoke samples -/

/-- Combining finite cost witnesses adds their bounds. -/
example :
    ComplexityGrade.add (.bound 2) (.bound 3) = .bound 5 := rfl

/-- Combining with unbounded cost is unbounded. -/
example :
    ComplexityGrade.add (.bound 2) .unbounded = .unbounded := rfl

/-- Zero cost annihilates an unbounded sequential factor. -/
example :
    ComplexityGrade.mul (.bound 0) .unbounded = .bound 0 := rfl

/-- Nonzero finite cost times unbounded cost is unbounded. -/
example :
    ComplexityGrade.mul (.bound 2) .unbounded = .unbounded := rfl
