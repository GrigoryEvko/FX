...
accepted fragment: sorts, bound variables, and constants.  This is not yet
the full Lean kernel checker theorem.
-/
...
Coverage is deliberately narrow: sorts, bound variables, and constants.
Unsupported Lean expression forms are rejected by `check`, so they add no
trusted computation rule in this slice.
-/
