...
`Ty.interval`, and `Ty.effect`.  Each ships the SN-direct case;
renaming-stable transport is handled once by the generic
`Reducible.fundamental_subsume_stable` and
`Reducible.fundamental_modIntro_stable` theorems.
