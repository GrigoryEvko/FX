...
`modIntro` / `modElim` SN preservation plus generic first/second-
projection SN preservation (raw + typed).  The intermediate
head-β `fst_pair` / `snd_pair` helpers were retired alongside the
Tait-era `IsNeutral` cascade — the surviving Kripke step-indexed
reducibility chain consumes only the generic projection lemmas plus
the constructor-introduction lemma `RawTerm.pair_isStronglyNormalizing`.
-/

namespace LeanFX2


/-- **modIntro SN preservation**.  Unary cong-only ctor at the modal-
introduction position.  Used by the typed Kripke fundamental chain to
discharge modal introductions. -/
...
/-- **Modal elimination SN preservation**.
...
/-- **Pair SN preservation** — binary cong-only SN helper.  Pair has
two parallel subterms; the SN proof needs nested induction (outer on
firstIsSN with `generalizing` to expose second's SN as IH input, inner
on secondIsSN) plus a per-side disequality split.  When
`pair currentFirst currentSecond` steps to `pair firstTarget
secondTarget` with the pair distinct, at least one side must have
advanced; case-split on which to discharge via the outer or inner
IH. -/
...
/-- Typed first-projection SN preservation. -/
...
/-- Typed second-projection SN preservation. -/
