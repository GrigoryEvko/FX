...
Strong-normalization preservation theorems shared by the Kripke
step-indexed candidate and Term-level SN endpoints.  The file lives under
`Reducibility/SN` because several proofs consume raw reduction inversion
and compatibility lemmas; `Term/SN/Helpers.lean` remains a compatibility
shim for the historical import path.
