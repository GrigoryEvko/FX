...
* [~] D2.5 cubical β rules (#1309): typed β for transp/hcomp (#1527-1528) and transpRefl / transpPi / transpSigma / closed-types / betaPathReflApp / glueAtFace (#1555-1560) **PENDING — cd cascade prerequisite** (see #1561 D2.5-CASCADE).  Adding any RawStep.par.<transpβ> ctor forces an exhaustive cd_lemma arm that requires extending RawTerm.cd to fire the constant-path β at the recognizer level, which in turn requires a propext-clean `RawTerm.unweaken?_imp_weaken` inversion lemma (~250 lines structural induction).  First investigation 2026-05-07 hit the indexed-inductive-partial-match propext trap; two WIP attempts preserved as git stash{0,1}.  D2.5.3 (#1529) ✅ verified — `Step.betaGlueElimIntro` IS the typed glueBeta (existing docstring "Cubical Glue β-reduction").

Acceptance: rename/subst preserves Step ✅ (raw and typed levels); typed `Step.par.<X>Cong.{rename,subst}_compatible` 14 of 14 done (100%).  Outstanding: D2.5.1/D2.5.2/D2.5.4-9 typed cubical β-rules (deferred behind D2.5-CASCADE #1561 — the cd cascade extension blocked on a propext-clean inversion lemma; do NOT estimate as "atomic ~30-50 LoC each").  Not blocking M06 SR or PHASE7-CONV-TRANS since those are cong-rule consumers, not β-rule consumers.
