...
/-! Probe: E3 forward renaming — grown typing is preserved along ANY renaming satisfying the
flag-coherent Conv condition.  The formation companion renames a formation derivation into a
GROWN derivation (the var arm needs the grown conv rule: the condition's Conv half re-classifies
the target variable, the IMAGE half supplies the reclassifier).  Binder crossings extend the
condition via `consConv` with the renamed component as the new target domain (Conv.refl pin;
image = target syntactically). -/
...
#print axioms FX1Poly.Typed.HasTypeDesc.renameAlongFlagCoherentToGrown
#print axioms FX1Poly.Typed.DescTelescope.renameAlongFlagCoherentToGrownTelescope
#print axioms FX1Poly.Typed.HasTypeDescPi.renameAlongFlagCoherent
#print axioms FX1Poly.Typed.DescTelescopePi.renameAlongFlagCoherentTelescope
