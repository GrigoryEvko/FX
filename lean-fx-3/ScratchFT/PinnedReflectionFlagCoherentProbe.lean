...
/-! Probe: E2-0 — the flag-coherent pinned-reflection motive, its FREE transfer from the
shipped plain master (the enriched condition projects, so every plain conclusion weakens), and
the enriched residual definitions (the precise route-(A) targets, with the flag-coherent
condition in the premises the λ-reduct discharge will consume). -/
...
#print axioms FX1Poly.Typed.PinnedReflectionConclusion.toFlagCoherent
#print axioms FX1Poly.Typed.HasTypeDescPi.pinnedReflectionFlagCoherentOfPlainResidual
