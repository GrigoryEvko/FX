...
/-! Probe: STR-8b brick i-a — the GUARDED conditional master.  Copy of the conditional grown
master with a SIZE BOUND + NORMALITY threaded through the mutual: the residual parameter weakens
to a bound-guarded, normal-subject form, which is what lets the plateau knot close by strong
induction on the bound (the residual at bound N only ever consumes the master at bounds < N). -/
...
#print axioms FX1Poly.Typed.HasTypeDescPi.pinnedReflectionGuarded
#print axioms FX1Poly.Typed.DescTelescopePi.pinnedReflectionTelescopeGuarded
