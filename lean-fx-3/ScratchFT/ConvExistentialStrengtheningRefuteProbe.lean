...
/-! Probe: even the CONV-existential strengthening is FALSE without a pinned classifier (STR-5b).

Sharpens STR-1 (which refuted the SYNTACTIC-image existential): the weakened identity λ is grown-typed
at `Π (var 0). (var 1)` in `[Type@0]` — a NORMAL classifier that is not `Conv` to ANY weakening (its
domain mentions the fresh variable irreducibly).  So the strengthening reflection's image-pinned
classifier premise is MANDATORY, not an artifact. -/
...
#print axioms FX1Poly.Typed.variableDomainPi_isStepNormalForm
#print axioms FX1Poly.Typed.weakenedIdentityTypedAtVariableDomainPi
#print axioms FX1Poly.Typed.variableDomainPi_notConvWeakenImage
#print axioms FX1Poly.Typed.convExistentialStrengthening_isFalse
