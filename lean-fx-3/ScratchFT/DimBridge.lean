...
/-! Scratch probe (DIM2-5 iii-b): bridge kernel `shift`/`substAt` to the σ-algebra, the β-composition
(★), `Reduces.applySubstitution`/`Reduces.substAt`, and SN-reflection.  Funext-free; `ite` via
`rw [if_pos]`/`rw [if_neg]`; Nat order via explicit lemmas, never `omega`. -/
...
#print axioms shift_eq_renameTerm
#print axioms shift_zero_eq_renameTerm
#print axioms substAt_eq_applySubstitution
#print axioms substAt_zero_applySubstitution_lift
#print axioms GradedLambda.Reduces.applySubstitution
#print axioms GradedLambda.Reduces.substAt
#print axioms GradedLambda.IsStronglyNormalizing.ofSubstAt
