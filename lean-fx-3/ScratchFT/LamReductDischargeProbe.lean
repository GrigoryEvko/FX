...
/-! Probe: STR-8b brick iii-a — reduce the λ-REDUCT head residual to ONE named statement: the
λ-CLASSIFIER PIN residual (a normal, in-image, grown-typed λ has a pinned classifier).  Given that
pin, the λ-reduct discharge is the exact mirror of the neutral one (SR-star types the reduct, the
pin IS the Π pin, the core finishes).  Composing through the dispatcher + the conditional master,
the WHOLE campaign's open core becomes this one residual. -/
...
#print axioms FX1Poly.Typed.pinnedReflectionPiElimLamReductResidualOfLamClassifierResidual
#print axioms FX1Poly.Typed.pinnedReflectionPiElimResidualOfLamClassifierResidual
#print axioms FX1Poly.Typed.HasTypeDescPi.pinnedReflectionOfLamClassifierResidual
