...
/-! Probe: the PINNING ANALYSIS — a Π-classifier `Conv` to a weakening exposes components EXACTLY in
the weaken image (STR-8 brick 1).

From `Conv (Π domainCode. codomainCode) (weaken classifierBase)`: `reducesToPiTyCode` reduces the
weakening to a Π-reduct with `Conv`-related components; `StepStar.reflectRename` pulls the chain back
through the weakening (the image is reduction-closed); the mkGen drilling forces the reduct's
components to be EXACT renames — `weaken domainBase` and `lift-weaken codomainBase`.  This is the
ingredient that makes the route-H reflection's piIntro arm close (the floating domain becomes
in-image up to `Conv` with an EXACT image representative). -/
...
#print axioms FX1Poly.Typed.Conv.pinnedPiComponentsInWeakenImage
