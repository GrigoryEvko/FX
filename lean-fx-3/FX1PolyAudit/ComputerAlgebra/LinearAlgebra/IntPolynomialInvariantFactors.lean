...
Per-declaration zero-axiom gate for the dimension-2 invariant factors and the rational-canonical-form
block count (the fourth brick of the char-matrix → invariant-factors layer, WP-ENDO #2255): `s_2 = d_2 /
d_1` via monic division (exact for the derogatory scalar, whose invariant factors `[x−2, x−2]`
reconstruct `(x−2)²`) and the block count `1 + deg d_1`, a decidable similarity invariant separating
`2·I` (`2` blocks) from the Jordan block `[[2,1],[0,2]]` (`1` block) — matrices with equal characteristic
polynomial `(x−2)²`.

Monic division + `polyDegree` + `decide` groundings + a `Nat` inequality.  Must be free of `propext`,
...
#assert_no_axioms FX1Poly.ComputerAlgebra.fxIntPoly_hasInvariantFactorsTwo
