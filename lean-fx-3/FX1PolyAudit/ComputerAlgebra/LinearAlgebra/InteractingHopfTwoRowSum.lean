...
/-! # FX1PolyAudit/ComputerAlgebra/LinearAlgebra/InteractingHopfTwoRowSum —
    zero-axiom gate (WP-PROP-3 brick 10: the split/merge fans, the gadget tensor,
    and the two-row spatial sum)
...
* the split / merge per-strand fan arities and denotations (T1: `ihtSplitLayerDomArity`,
  ...
  `ihtSplitLayerDenote`, `ihtMergeLayerDenote`, `ihtHasFanDenotations`);
...
* the right-whisker spec and the gadget tensor (T2: `ihtWhiskerRightSpec`,
  ...
  `ihtGadgetTensorCodArity`, `ihtHasGadgetTensor`);
...
* THE COMMITTED STATEMENT INHABITED VERBATIM (T3: `ihtTwoRowSpatialSum`) with its
  DECIDED marker (`ihtHasTwoRowSpatialSum`) and kernel-decided fires
...
* the two-row NF carrier toward `ihzNormalFormStatement` (T4:
  `ihtTwoRowNormalFormCarrier`) and the owner-false general-compiler wall
  (`ihtHasNormalFormCompiler`).
...
#assert_no_axioms FX1Poly.ComputerAlgebra.ihtHasFanDenotations
...
#assert_no_axioms FX1Poly.ComputerAlgebra.ihtHasGadgetTensor
...
#assert_no_axioms FX1Poly.ComputerAlgebra.ihtHasNormalFormCompiler
