...
Per-declaration zero-axiom gate for the forward step-refinement development
...
the functional-abstraction builder (`ofAbstraction`), and the identity instance
(`identity`) with its `Nat` witness.
