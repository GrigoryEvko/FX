...
/-! # FX1PolyAudit/ComputerAlgebra/Decision/CombinationCompleteness — zero-axiom gate
    (Nelson–Oppen completeness for the ground-congruence + Farkas combination)
...
(`nccAugmentedRationallyInfeasible`), the discharged Farkas leg
(`nccAugmentedInfeasibleIffFarkasCertificate` through `lreFarkasCompletenessHolds` /
`lreScaledInfeasibilityOfAcceptedCertificate`), the completeness reduction
`nccAcceptedCertificateOfAugmentedInfeasible`, the joint-model companion
`nccJointModelSatisfiesAugmented`, the single honest wall
...
`nccPropagatedCountBoundedByTriangle`), the markers, and the concrete congruence fires
(a=b lifted through f to f(a)=f(b)=x0=x1, contradicting the arithmetic gap; the SAT
...
#assert_no_axioms FX1Poly.ComputerAlgebra.fxNccCombine_hasCompletenessReduction
