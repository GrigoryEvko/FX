...
(`locateCupThenCap_of_cup_of_not_allCups`, `locateCupThenCap_of_not_valley`, `locateCupThenCapSpine`), the total
dispatch collapsing the oracle's open surface to the single STRAIGHTEN residual
(`valleyCellDescentStepOracle`), and the sharpened reduction of `MatchingReductsShareSpineTrace` to
`CellStraightenStepInput` ∧ `CellValleyTraceEquiv`
(`matchingReductsShareSpineTrace_of_straighten_of_valleyTraceEquiv`).  Every declaration must be free of
...
#assert_no_axioms FX1Poly.Polygraph.matchingReductsShareSpineTrace_of_straighten_of_valleyTraceEquiv
