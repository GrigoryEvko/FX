...
import FX1Poly.Typed.Engine.RuleTables.UnifiedRuleSignature
...
import FX1Poly.Typed.Engine.Formation.IdFormerTermIndexedRetrofit
...
import FX1Poly.Typed.Metatheory.Universe.UniverseModeGenerators
...
/-! ## NATIVE-17 (#1294) — the Id retrofit (idCode formable + refl classifier formable)
...
term-indexed-formable given `A : Type@e` (refl classifier formable).  `closedIdUniverseFormable` is the
...
(via the union's reflexive data-intro arm) and the FORMATION of the type it inhabits (via the term-indexed table,
through `HasTypeUnion.formationRuleOfObligations`, no host bridge) both route through native rules.  No bespoke
`idFormation`.  Axiom-free. -/
