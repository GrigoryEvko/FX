...
-- closed modality, which pulls Quot.sound).  The STC ledger's logicalRelationConstruction level is
-- now witnessed by the BRIDGED construction (STC/FxLogicalRelation.lean — its semantic side is
-- definitionally the Tait pipeline, fxStcFundamental_semantic_isTaitWitness); INDEPENDENCE remains
