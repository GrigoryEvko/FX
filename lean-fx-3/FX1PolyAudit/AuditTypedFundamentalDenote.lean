...
-- FTGEN-5.1/5.2: the table-driven carrier-aware arm.  The carrier-aware-shape inversion (recovers both carrier
-- candidates + the assembled-candidate pointwise iff) is the deterministic-carrier finisher, and
-- the all-levels carrier-recursive leaf is what `ofDataFlatCarrierAware` denotes.
