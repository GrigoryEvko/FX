...
/-- ★ **Honesty marker — the PARTITION (corelation) soundness of the full spider congruence is CLOSED (FROB-3, r3).**
`SpiderConv` (`Frobenius/SpiderConv.lean`) is the convertibility generated by { equivalence, the thirteen special
rows at their boundary, the fully-contextual Godement interchange, the boundary-partition-preserving contextual
whisker }, and `spiderConv_partitionSound` proves EVERY convertible pair induces the same boundary partition
(`extraSpiderDiagramOf`, the extraspecial / corelation invariant) — the per-ctor induction closes zero-axiom.  The
contextual interchange is discharged uniformly by the shared reachable interchange `brauer_reachable_interchange` (the
state-parametric union-find independence the r1 residual named — CLOSED by the Brauer route and reused here, its
Brauer extract equality reconstructed to the connectivity view); the whisker uniformly by
`extractSpiderDiagram_forget_eq_of_connectivityView`.  NON-VACUOUS: `spiderConv_H_not_identity` refutes a genuine
parallel pair.  The FULL special (`Cospan(FinSet)`) invariant `spiderDiagramOf` additionally tracks the
closed-component count, whose preservation under the boundary view is the separate
