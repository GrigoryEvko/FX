...
/-! # WalkingMonad — the EZ word-builder: turn a fold output (`List Nat`) into a canonical free 2-cell

`WalkingMonad/MonadDeltaDecision` completed the SOUNDNESS leg `mapEqOfConv`.  The COMPLETENESS leg `convOfMapEq`
(`monadMonotoneMapOf a = monadMonotoneMapOf b → MonadSaturatedTwoCellConv a b`) needs the Eilenberg–Zilber
staircase: read the monotone map off the fold and REBUILD a canonical free 2-cell that folds back to it.  The
recon named the missing piece exactly — "there is NO function turning a `List Nat` into a `RawTwoCellExpr`".
This file ships that function and proves it round-trips through the fold.

## The per-target-gadget construction (avoids separating epi/mono)

A monotone map `v : [n] → [m]` is the horizontal composite, over the `m` target positions, of a GADGET for each
target `j`: the `c_j = |v⁻¹(j)|` source strands mapping to `j` are MERGED to one.  The gadget for a count `c`:

  * `c = 0` — the **unit** `eta : id ⇒ t` (a FACE / degeneracy-free insertion),
  * `c = 1` — the **identity** `id_t : t ⇒ t`,
  * `c + 2` — `mu ∘ (t ◁ gadget(c+1))`, a right-leaning tree of `mu`s merging `c+2` strands to one.

Each gadget `t^c ⇒ t` FOLDS to the constant map `replicate c 0` (all `c` strands hit the single output `0`).
This merges the EZ epi (the `mu`s) and mono (the `eta` when `c = 0`) into one uniform builder — no bundled
`rankList`/`imageList` decomposition into individual generators is needed.

## What this file ships (each piece zero-axiom)

  * **`monadGadget`** — the count → cell builder (`eta` / `id_t` / `mu`-tree).
  * ★ **`monadGadget_map`** — the fold of `monadGadget c` is `replicate c 0` (structural induction on `c`, via the
    shipped `vcomp` / `whiskerLeft` homomorphisms + the merge computation).

Raw Lean 4 + Init; structural recursion on `Nat` (the count), `propext`/`Quot.sound`/`Classical`/`sorry`/
