...
## What this file ships (each piece zero-axiom)
  ...
  * **`embedLocalMap`** (via the cons-only `ascendingPrepend` / `shiftPrepend`, no `++`, propext-safe) — the
    ordinal-sum embedding `id_L ⊕ localMap ⊕ id_R` of a local map into a `leftLen`-prefixed, `rightLen`-suffixed
    context; its length + three region-wise value characterizations.
