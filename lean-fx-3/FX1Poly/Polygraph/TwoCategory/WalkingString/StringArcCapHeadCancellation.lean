...
/-! # WalkingString/StringArcCapHeadCancellation — the cap-head transport is injective, ported
(FC-3 r21, THE 110-PERCENT GRIND — Branch B, the CANCEL theorem)

Phantom-signature clone of the walking-adjunction `ArcCapHeadCancellation`, re-plumbed onto the
FOUR-generator adjoint-triple seed and NARROWED to the pure-cap regime (Route C).  The assembled
cap-head transport (`stringArcCapHeadFolded_extractArc`) rewrites both composite extracts over the SAME
peeled cap into transported fresh extracts; this file recovers the fresh-extract equality from the
composite-extract equality — the direction the head-cancellation assembly needs when the two pure-cap
spines share the peeled head.  Every transported field is inverted: the totals by successor
injectivity, the spliced internal-count lists by insert-at cancellation (the window position fits under
both list lengths), and the partner leg by insert-at cancellation followed by injectivity of the
two-zone fresh shift.  The private cancellation kit is graph-neutral and re-declared verbatim; only the
two transport rewrites carry the `AllCapArity` witnesses.
...
/-- ★ **Cap-head transport cancellation** (adjoint-triple, pure-cap): two chained pure-cap tails with
the SAME composite extract over the SAME peeled cap (window `windowPosition` at bottom boundary
`bottomCount`) already have the same fresh extract at the tail boundary.  Rewrites both sides through
the assembled transport (`stringArcCapHeadFolded_extractArc`), then inverts every transported field:
the totals by successor injectivity, the spliced internal-count lists by insert-at cancellation, and
the partner leg by insert-at cancellation followed by injectivity of the two-zone fresh shift. -/
theorem stringArcCapHeadFolded_extractArc_cancel
    ...
    (firstAllCap : AllCapArity firstAtoms)
    (secondAllCap : AllCapArity secondAtoms)
    ...
    extractArc tailBoundary
      ...
      = extractArc tailBoundary
          (processArcSpine
            ...
            secondAtoms) := by
  rw [stringArcCapHeadFolded_extractArc bottomCount windowPosition tailBoundary windowFits
      tailBoundaryFits firstAtoms firstAllCap firstChained,
    stringArcCapHeadFolded_extractArc bottomCount windowPosition tailBoundary windowFits
      tailBoundaryFits secondAtoms secondAllCap secondChained] at compositeExtractsAgree
...
/-- **Honesty marker — cap-head transport cancellation, adjoint-triple pure-cap (FC-3 r21, the CANCEL
theorem).**  `stringArcCapHeadFolded_extractArc_cancel`: two chained pure-cap tails with the same
composite extract over the same peeled cap have the same fresh extract at the tail boundary — the
assembled transport (`stringArcCapHeadFolded_extractArc`) is injective, inverted field by field
(successor injectivity on the cap total, insert-at cancellation on the spliced lists, the injective
two-zone shift on the partner leg).  The private cancellation kit is graph-neutral; only the two
transport rewrites thread the `AllCapArity` witnesses.  What this marker does NOT claim: locating the
matching head inside the second spine, the pin inhabitation, or the sort-driver assembly discharging
`StringCapHeadExtractionWordPin` — the remaining rungs (the bubble campaign, r22+).  `= true`. -/
def fxString_hasArcCapHeadCancellation : Bool := true
