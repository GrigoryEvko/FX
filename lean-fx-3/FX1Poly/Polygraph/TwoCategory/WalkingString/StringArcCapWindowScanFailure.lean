...
/-! # WalkingString/StringArcCapWindowScanFailure — the consumed window pair fails every folded scan
test, ported (FC-3 r20, THE CLONE CAMPAIGN — floor)

Phantom-signature two-token clone of the walking-adjunction `ArcCapWindowScanFailure`, re-plumbed onto
the FOUR-generator adjoint-triple seed.  The composite extract's candidate range carries two
candidates the fresh extract never sees — the window positions `windowPosition` and `windowPosition +
1` — and the consumed pair can never pass the partner scan: the left wire IS the strand-closure anchor
(the string seed-closure miss `stringArcCapHeadFolded_windowAnchorMissesReindexed`), and the right
wire is linked to the left through the fold (the string persistence
`stringArcCapHeadFolded_consumedPairLinked`), so the query transfers.  With both tests false, the
interleave's middle segment drops out of the composite scan.  The private plumbing is graph-neutral and
re-declared verbatim; the signature is a pure phantom, so ONLY the `SpineAtom`-quantified statements
clone.
...
theorem stringArcCapHeadFolded_windowLeftScanTestFails
  ...
  have rootsMiss : (unionFindRootOf
      (processArcSpine
        ...
        == unionFindRootOf
            ...
            (arcHeadReindex (natListRemoveTwoAt (List.range bottomCount) windowPosition) 3
              anchorProbe)) = false :=
    stringArcCapHeadFolded_windowAnchorMissesReindexed bottomCount windowPosition tailBoundary
...
theorem stringArcCapHeadFolded_windowRightScanTestFails
    {overallSource overallTarget : adjointTripleGraph.Mode}
    (bottomCount windowPosition tailBoundary : Nat)
    (windowFits : windowPosition + 2 ≤ bottomCount)
    (tailBoundaryFits : tailBoundary + 2 = bottomCount)
...
composite scan. -/
theorem stringArcCapHeadFolded_windowPairScanTestsFail
    {overallSource overallTarget : adjointTripleGraph.Mode}
    (bottomCount windowPosition tailBoundary : Nat)
    (windowFits : windowPosition + 2 ≤ bottomCount)
    (tailBoundaryFits : tailBoundary + 2 = bottomCount)
  ...
  | head =>
      exact stringArcCapHeadFolded_windowLeftScanTestFails bottomCount windowPosition tailBoundary
        windowFits tailBoundaryFits atoms chained topWires excludeIndex anchorProbe
  | tail _ restMem =>
      cases restMem with
      | head =>
          exact stringArcCapHeadFolded_windowRightScanTestFails bottomCount windowPosition
...
/-- **Honesty marker — the consumed window pair fails every folded scan test, ported (FC-3 r20 clone
campaign).**  At the cap-head folded end state, both window candidates' exclude-and-root tests are
false against any reindexed-fresh-probe root: the left wire is the strand-closure anchor (seed-closure
miss), the right wire transfers its query to the anchor through the persistent consumed-pair link.
Packaged as the `dropMiddle` middle-fails hypothesis.  What this marker does NOT claim: the
per-candidate test correspondence between the composite and fresh scans, and the assembled
scan/partner/diagram equality.  `= true`. -/
def fxString_hasArcCapWindowScanFailure : Bool := true
