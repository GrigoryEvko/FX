...
/-! # WalkingString/StringValleyCupTopTopFold — the two-floor peel induction of the top-top offset agreement,
over the walking ADJOINT-TRIPLE (`F ⊣ G ⊣ H`) signature (FC-3 r34, Piece-II tail)

The string clone of the walking-adjunction `ValleyCupTopTopFold`.  The per-step engine `topRegionOffsetAgrees_step`
and the offset-agreement relation `topRegionOffsetAgrees` (`ValleyCupTopTopStepTransport`) are pure `Nat`/`List Nat`
— signature-BLIND — and REUSED verbatim by import.  This file FOLDS that engine across a whole pure-cup
boundary-chained block at TWO floors simultaneously — `floorHi` (the whole valley, `bottomCount`) and `floorLo`
(the cup alone, `midWidth`) — carried as a single cons-induction.

## Why cons induction (not reverse/peel-last)

`diagramPartner_stepCupArc` rewrites `partner(stepCupArc state w)` in terms of `partner(state)`.  Cons induction on
the cup block processes the FIRST cup onto each seed and recurses on the tail with the STEPPED seed pair.  The
invariant carried IN is the seed-pair agreement; each cons step promotes it by `topRegionOffsetAgrees_step` (after
rewriting the two partners by `diagramPartner_stepCupArc`), then recurses.  At the empty block the carried seed
agreement IS the conclusion.  The arc engine (`stepCupArc`, `processArcSpine`, `extractArc`,
`diagramPartner_stepCupArc`, `arcStateFresh_stepCupArc`, `isUnionFindForest_stepCupArc`,
`arcBoundaryCensus_stepCupArc`, `stepArcAtom_openWires_tracksBoundary`, `stepArcAtom_eq_stepCupArc`) is
signature-blind or signature-generic and REUSED by import; only the statement's `SpineAtom` signature is swapped.

## What this file lands (zero-axiom)

  * ★ `stringTopRegionOffsetAgrees_fold` — the two-floor peel induction: from a seed-pair agreement over the seed
    open-wire region, the whole cup block's two runs agree over the final open-wire count.

What remains for `stringCupTopTopPartner` (`StringValleyCupTopTopSeed`): the base-case seed reconciliation and the
WireState ↔ arc bridge.  No master flag is flipped.
...
/-! ## Local plumbing (distinct `SCTTF` suffix, propext-free copies) -/

private theorem rangeLoopLenSCTTF : (count : Nat) → (accumulated : List Nat) →
    (List.range.loop count accumulated).length = count + accumulated.length
  | 0, accumulated => (Nat.zero_add accumulated.length).symm
  | count + 1, accumulated => by
      have inner := rangeLoopLenSCTTF count (count :: accumulated)
      show (List.range.loop count (count :: accumulated)).length = count + 1 + accumulated.length
      rw [inner]
      show count + (accumulated.length + 1) = count + 1 + accumulated.length
      rw [← Nat.add_assoc count accumulated.length 1, Nat.add_right_comm count accumulated.length 1]

private theorem rangeLenSCTTF (count : Nat) : (List.range count).length = count := by
  show (List.range.loop count []).length = count
  rw [rangeLoopLenSCTTF count []]
  exact Nat.add_zero count

/-- A top-of-stack cup adds exactly two open wires (defeq to `natListInsertAt_length`). -/
private theorem stepCupArcOpenWiresLenSCTTF (state : ArcWireState) (position : Nat) :
...
private theorem extractArcPartnerLenSCTTF (floor : Nat) (state : ArcWireState) :
  ...
  rw [mapLength, rangeLenSCTTF]

/-! ## The two-floor peel induction -/

/-- ★ **The two-floor peel induction of the top-top offset agreement.**  From a seed-pair agreement over the seed
open-wire region, processing the SAME pure-cup boundary-chained block through BOTH runs (floors `floorHi` /
`floorLo`) preserves the top-region offset-space agreement, over the final open-wire count.  Cons induction on the
block: each cup promotes the seed agreement by `topRegionOffsetAgrees_step` (both partners rewritten by
...
theorem stringTopRegionOffsetAgrees_fold {overallSource overallTarget : adjointTripleGraph.Mode}
    (floorHi floorLo : Nat) :
  ...
  | firstCup :: rest, cupPure, stateHi, stateLo,
    ...
    | cons hasCupDomArity hasCupCodArity restPure =>
      ...
      have lenHi : (extractArc floorHi stateHi).diagram.partner.length = floorHi + stateHi.openWires.length :=
        extractArcPartnerLenSCTTF floorHi stateHi
      have lenLo : (extractArc floorLo stateLo).diagram.partner.length = floorLo + stateHi.openWires.length := by
        rw [extractArcPartnerLenSCTTF floorLo stateLo, openWiresEq]
      ...
      have steppedAgree : topRegionOffsetAgrees floorHi floorLo
        ...
        rw [diagramPartner_stepCupArc floorHi stateHi firstCup.leftContext.length
            ...
            stepCupArcOpenWiresLenSCTTF stateHi firstCup.leftContext.length]
      ...
      have openWiresEq' : (stepCupArc stateHi firstCup.leftContext.length).openWires.length
        ...
        rw [stepCupArcOpenWiresLenSCTTF stateHi firstCup.leftContext.length,
          stepCupArcOpenWiresLenSCTTF stateLo firstCup.leftContext.length, openWiresEq]
      -- recurse on the tail with the stepped seed pair
      have recResult := stringTopRegionOffsetAgrees_fold floorHi floorLo rest restPure
...
/-- **Marker — the two-floor peel induction of the top-top offset agreement is LANDED on the STRING side
(FC-3 r34).**

Landed here, zero-axiom:

  * `stringTopRegionOffsetAgrees_fold` — the two-floor peel induction: from a seed-pair top-region offset
    agreement, the same pure-cup boundary-chained block's two runs (floors `floorHi` / `floorLo`) agree over the
    final open-wire count.  Cons induction folds the imported blind `topRegionOffsetAgrees_step` across the block,
    threading the per-seed `ArcStateFresh` / `isUnionFindForest` / `seedBelowFresh` / `ArcBoundaryCensus`
    invariants and the shared window fit; the base returns the carried agreement.

What remains for `stringCupTopTopPartner` (`StringValleyCupTopTopSeed`): the base-case seed reconciliation and the
WireState ↔ arc bridge.  No master flag is flipped.  `= true`. -/
def fxString_hasCupTopTopFold : Bool := true
