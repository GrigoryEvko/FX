...
/-! # WalkingString/StringArcCapWindowPartners — the consumed window pair partner each other, ported
(FC-3 r20, THE CLONE CAMPAIGN — Branch A)

Phantom-signature two-token clone of the walking-adjunction `ArcCapWindowPartners`, re-plumbed onto the
FOUR-generator adjoint-triple seed.  The composite extract's partner list carries two entries the fresh
extract never sees — the window positions `windowPosition` and `windowPosition + 1` — and they partner
EACH OTHER: the left window index's partner scan returns the right and vice versa.  First-hit discipline
drives both: every below-window candidate reads a reindexed fresh node and the strand-closure anchor
misses every reindexed probe (the string seed-closure miss
`stringArcCapHeadFolded_windowAnchorMissesReindexed`, transferring across the persistent consumed-pair
link `stringArcCapHeadFolded_consumedPairLinked` for the right exclude), the window's own index is
excluded by the bang-inequality, and the partner wire passes.  The private plumbing is graph-neutral and
...
whole fold. -/
theorem stringArcCapHeadFolded_windowLeftPartner
  ...
  have belowFails : ∀ candidate, candidate ∈ List.range windowPosition →
    ...
    have readCandidate := stringArcCapHeadFolded_boundaryRead_shifted bottomCount windowPosition
...
theorem stringArcCapHeadFolded_windowRightPartner
    {overallSource overallTarget : adjointTripleGraph.Mode}
    (bottomCount windowPosition tailBoundary : Nat)
    (windowFits : windowPosition + 2 ≤ bottomCount)
    (tailBoundaryFits : tailBoundary + 2 = bottomCount)
...
/-- **Honesty marker — the consumed window pair partner each other, ported (FC-3 r20 clone campaign).**
At the cap-head folded end state, `partnerIndexOf` at the left window index returns the right window
index and vice versa: the below-window front fails through (every candidate reads a reindexed fresh
node and the strand-closure anchor misses every reindexed probe, transferring across the persistent
consumed-pair link for the right exclude), the excluded index skips by the bang-inequality, and the
partner wire first-hits through the seed link's persistence.  What this marker does NOT claim: the
mapped partner-list equality and the assembled `DiagramType` / `FullArcStructure` equality.  `= true`. -/
def fxString_hasArcCapWindowPartners : Bool := true
