...
/-! # ArcCapInternalCountsPointwise — the per-port cap-count characterization (leaf 2a-ii)

For a boundary-chained PURE-CAP spine the per-port internal cap count is a FUNCTION of the boundary
`diagram`: port `index` carries exactly one cap-turnback iff it is a bottom port (`index < bottomCount`)
whose partner is ALSO a bottom port (`partner index < bottomCount`) — a bottom-bottom short chord — and
zero otherwise.  This is the exact DUAL of the cup case (leaf 2a-i, top-top short chord), but where the
cup infrastructure peels the LAST cup (cups create fresh top legs, undisturbed by the tail), the cap
infrastructure is PEEL-FIRST: a cap CONSUMES two existing bottom ports, so the head cap is arc-anchored
and the shipped cap-head list splices (`arcCapHeadFolded_internalCapCountsCorr`,
`arcCapHeadFolded_partnerListCorr`) express the whole composite lists as the fresh lists (at the SHRUNK
boundary `bottomCount - 2`) with the consumed window pair spliced in.

  * ★ `pureCap_internalCapCounts_pointwise` — the pointwise characterization, by peel-FIRST induction on
    the spine (mirroring `pureCapSpine_sort`): the empty spine reads both sides `0`, and peeling the head
    cap splices `[1, 1]` into the count list exactly where the short chord `[windowPos + 1, windowPos]` is
    spliced into the partner list, so the two spliced window ports read `1` (bottom-bottom) and every
    other port shift-tracks through `freshShiftAbove` and inherits from the tail characterization at the
    shrunk boundary — where `freshShiftAbove windowPos 2` preserves the `· < bottomCount` verdict RELATIVE
    to the shrunk boundary `bottomCount - 2` (the shift adds exactly the boundary difference).
  * ★ `pureCapSpines_internalCapCountsAgree_ofDiagram` — the agreement corollary: two pure-cap spines
    whose `diagram`s agree (hence whose `partner`s agree) have equal `internalCapCounts`, since each entry
    is a function of the shared partner.  This discharges the `capInternalCapCountsAgree` residual of
...
/-- The per-port cap indicator: `1` when `index` is a bottom port whose partner is a bottom port (a
bottom-bottom short chord), `0` otherwise.  Read as a pure `Bool` function so the agreement corollary is
`congrArg`.  This is the DUAL of `cupPortIndicator` (which tests `bottomCount ≤ ·` — top ports). -/
def capPortIndicator (bottomCount partnerValue index : Nat) : Nat :=
  if Nat.blt index bottomCount && Nat.blt partnerValue bottomCount then 1 else 0
...
`windowPosition` (`arcCapHeadFolded_internalCapCountsCorr`) exactly where the short chord
`[windowPosition + 1, windowPosition]` is spliced into the partner list
(`arcCapHeadFolded_partnerListCorr`); the two window ports read a bottom-bottom chord (indicator `1`), and
...
private theorem stepPointwiseCap
  ...
  have countSplice : (extractArc bottomCount
      ...
      = natListInsertAt (arcStructureOfSpineList tailBoundary atoms).internalCapCounts windowPosition
          [1, 1] :=
    arcCapHeadFolded_internalCapCountsCorr bottomCount windowPosition tailBoundary windowFits
  ...
  have partnerSplice : (extractArc bottomCount
    ...
    arcCapHeadFolded_partnerListCorr bottomCount windowPosition tailBoundary windowFits
      tailBoundaryFits atoms chained
  have kLtFreshPlus2 : k < (tailBoundary
      + (processArcSpine (ArcWireState.mk (List.range tailBoundary) [] tailBoundary 0 [] [])
        atoms).openWires.length) + 2 := by
    have tot := arcCapHeadFolded_totalPorts bottomCount windowPosition tailBoundary windowFits
...
private theorem pointwiseCapFueled {overallSource overallTarget : adjunctionGraph.Mode} :
  ...
  | fuel + 1, bottomCount, atoms, lenBound, chained, pureCap => by
      ...
      | headCap :: tailAtoms, lengthBound, chained, pureCap =>
          intro k kRange
          obtain ⟨c1Dom, c1Cod⟩ := headCapArity pureCap
          have tailPure : AllCapArity tailAtoms := allCapArity_ofCons pureCap
          ...
          have tailIH := pointwiseCapFueled fuel headCap.codBoundaryLength tailAtoms tailLenBound
          ...
          exact stepPointwiseCap bottomCount headCap.leftContext.length headCap.codBoundaryLength
...
theorem pureCap_internalCapCounts_pointwise {overallSource overallTarget : adjunctionGraph.Mode}
    (bottomCount : Nat)
    (atoms : List (SpineAtom adjunctionModeSignature overallSource overallTarget))
    ...
    natListGetAt (arcStructureOfSpineList bottomCount atoms).internalCapCounts k
      = capPortIndicator bottomCount
          (natListGetAt (arcStructureOfSpineList bottomCount atoms).diagram.partner k) k :=
  pointwiseCapFueled atoms.length bottomCount atoms (Nat.le_refl atoms.length) chained pureCap k kRange
...
theorem pureCapSpines_internalCapCountsAgree_ofDiagram {overallSource overallTarget : adjunctionGraph.Mode}
    (bottomCount : Nat)
  ...
  rw [pureCap_internalCapCounts_pointwise bottomCount firstList firstPureCap firstChained index indexFirst,
    pureCap_internalCapCounts_pointwise bottomCount secondList secondPureCap secondChained index indexSecond,
    partnerAgree]

/-! ## Honesty marker -/

/-- **Honesty marker — the per-port cap-count characterization (leaf 2a-ii) is SHIPPED.**
`pureCap_internalCapCounts_pointwise`: on a boundary-chained pure-cap spine the internal cap count at each
port is `capPortIndicator` of the boundary partner (a bottom-bottom short chord reads `1`, all else `0`),
proved by peel-FIRST induction (empty spine both-sides-zero; head-cap splice `[1, 1]` at the same window
the short chord `[windowPos + 1, windowPos]` is spliced into the partner, other ports shift-tracking
through `freshShiftAbove` whose `bltShiftAbove` invariance carries the verdict from the shrunk boundary
`bottomCount - 2`).  `pureCapSpines_internalCapCountsAgree_ofDiagram`: equal `diagram`s force equal
`internalCapCounts`.  This discharges the `capInternalCapCountsAgree` 2a residual of
`sameMatchingValleys_spineTraceEquiv` — the exact dual of the cup case (leaf 2a-i).  No gate flag is
flipped.  `= true`. -/
def fxMode_hasArcCapInternalCountsPointwise : Bool := true
