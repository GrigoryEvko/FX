...
/-! # ArcCapHeadTransport — arc-structure equality locates the cap-head's realizing atom

The head-location campaign's CLOSING theorem: when a CAP-HEADED reference spine (the head
cap consuming seed ports `windowPosition, windowPosition + 1`, chained tail) and a second
spine run from the same seed have EQUAL full arc structures, the second spine contains a
cap consuming exactly those two seed ports adjacently — the `ArcPairCapWindow` certificate.

The transport: the reference spine's extract values at the window are SHIPPED
(`arcCapHeadFolded_windowLeftPartner` pins the partner read-off to `windowPosition + 1`;
`arcCapHeadFolded_internalCapCountsCorr` pins the per-port cap-event counts to an
insert-at-window `[1, 1]` block).  Structure equality pushes both values through the
`partner` / `internalCapCounts` fields onto the second spine's extract (mapped-range reads
via `natListGetAt_map_inRange`), producing exactly the raw pins that
`arcPairCapWindow_ofFinalPins` consumes.
...
theorem arcPairCapWindow_ofCapHeadExtractEq
    {firstSource firstTarget secondSource secondTarget : adjunctionGraph.Mode}
    ...
    (firstTail : List (SpineAtom adjunctionModeSignature firstSource firstTarget))
    (chained : SpineBoundaryChained tailBoundary firstTail)
    (secondAtoms : List (SpineAtom adjunctionModeSignature secondSource secondTarget))
    (extractEq : extractArc bottomCount
      ...
      = extractArc bottomCount
          (processArcSpine (ArcWireState.mk (List.range bottomCount) [] bottomCount 0 [] [])
            secondAtoms)) :
    ArcPairCapWindow bottomCount windowPosition (windowPosition + 1) secondAtoms := by
  ...
  have partnerPinSecond : partnerIndexOf
    ...
    rw [← partnerReadsEq]
    exact arcCapHeadFolded_windowLeftPartner bottomCount windowPosition tailBoundary
          ...
          ((List.range (bottomCount
            ...
            (internalEventCountAt
              ...
              (processArcSpine
                ...
                secondAtoms).capEventNodes))
          windowPosition := by
    rw [← arcCapHeadFolded_internalCapCountsCorr bottomCount windowPosition tailBoundary
      windowFits tailBoundaryFits firstTail chained]
  ...
  have countPinSecond : internalEventCountAt
    ...
    exact insertRead
  exact arcPairCapWindow_ofFinalPins bottomCount secondAtoms leftBelow rightBelow
    (natNeOfLt (Nat.lt_succ_self windowPosition)) partnerPinSecond countPinSecond

/-! ## Honesty marker -/

/-- **Honesty marker — the cap-head pin transport is SHIPPED.**  Full-arc-structure equality
between a cap-headed reference spine and any second spine from the same seed transports the
reference's window values (partner = the adjacent port, strand cap count = one) onto the
second spine's extract and locates a cap in the second spine consuming exactly the head's
two seed ports adjacently.  NOT yet shipped: the cup-head analogue (the orbit-form cup
discharge) and the bubble-to-front witness over the located window.  `= true`. -/
def fxMode_hasArcCapHeadPinTransport : Bool := true
