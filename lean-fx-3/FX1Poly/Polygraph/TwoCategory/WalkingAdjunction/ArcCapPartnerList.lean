...
/-! # ArcCapPartnerList — the composite partner list is the spliced shift-image

The cap-head partner correspondence, assembled at the LIST level (peel campaign H,
rung E-3, part 6).  The composite extract's partner list — the whole
`(range totalPorts).map partnerIndexOf` the boundary `DiagramType` carries — equals the
fresh extract's partner list transported by the two-zone index shift with the consumed
window pair `[windowPosition + 1, windowPosition]` spliced in at the window: below-window
entries are shift-images of fresh partners (the scan correspondence at fixed indices), the
two window entries partner each other (the window-partner brick), and past-window entries
are shift-images at shifted indices.  This is the partner leg of the cap-head
`DiagramType` transport in closed form.
...
theorem arcCapHeadFolded_partnerListCorr
              ...
              (partnerIndexOf
                ...
                (tailBoundary
                  + (processArcSpine
                    ...
                    atoms).openWires.length)
                candidate) :=
          arcCapHeadFolded_partnerScanCorr bottomCount windowPosition tailBoundary
  ...
  have windowSegEq : ([windowPosition, windowPosition + 1]).map
    ...
    rw [arcCapHeadFolded_windowLeftPartner bottomCount windowPosition tailBoundary
        windowFits tailBoundaryFits atoms chained,
      arcCapHeadFolded_windowRightPartner bottomCount windowPosition tailBoundary
        windowFits tailBoundaryFits atoms chained]
...
/-- **Honesty marker — the composite partner list is the spliced shift-image (peel
campaign H, rung E-3, part 6).**  At the cap-head folded end state, the composite
extract's whole partner list equals `natListInsertAt (freshPartnerList.map shift)
windowPosition [windowPosition + 1, windowPosition]`: below/past-window entries ride the
assembled partner-scan correspondence, the window pair rides the window-partner brick, and
the splice rides the insert-at-front-length decomposition.  What this marker does NOT
claim: the assembled `DiagramType` equality (bottomCount/topCount/partner/loops fields at
`extractDiagram`) and the full `FullArcStructure` cap-head transport.  `= true`. -/
def fxMode_hasArcCapPartnerList : Bool := true
