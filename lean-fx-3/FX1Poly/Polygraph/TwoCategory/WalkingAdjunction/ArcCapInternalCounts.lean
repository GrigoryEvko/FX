...
/-! # ArcCapInternalCounts — the composite internal-count lists are the spliced fresh lists

The internal cup/cap count LIST transports at the cap head (peel campaign H, rung E-3,
part 10).  The composite extract's per-port internal-count lists — the
`internalCupCounts` / `internalCapCounts` fields of the `FullArcStructure` — equal the
fresh extract's lists with the consumed strand's two values spliced in at the window
position: count VALUES are boundary-invariant (unlike partner indices they need no
reindexing), below/past-window entries ride the on-the-nose event-count transports at
shifted boundary reads, and the two window ports read the consumed strand (one cap event,
no cup event, same root on both ports).
...
theorem arcCapHeadFolded_internalCapCountsCorr
  ...
  exact internalCountListCorr_ofPointwise
    ...
    (fun candidate candidateMem => by
      ...
      have readShifted := arcCapHeadFolded_boundaryRead_shifted bottomCount
      ...
      show countEventsInRoot
        ...
        = countEventsInRoot
            ...
            (processArcSpine
              ...
              atoms).capEventNodes
      rw [readShifted]
      exact arcCapHeadFolded_capEventCount_ofChained bottomCount windowPosition
    ...
    (by
      ...
      rw [boundaryReadLeft]
      exact arcCapHeadFolded_windowStrandCapCount bottomCount windowPosition tailBoundary
    ...
    (by
      ...
      rw [boundaryReadRight,
        arcCapHeadFolded_windowRightRootEq bottomCount windowPosition windowFits atoms]
      exact arcCapHeadFolded_windowStrandCapCount bottomCount windowPosition tailBoundary
        windowFits tailBoundaryFits atoms chained)
...
theorem arcCapHeadFolded_internalCupCountsCorr
    {overallSource overallTarget : adjunctionGraph.Mode}
    (bottomCount windowPosition tailBoundary : Nat)
    (windowFits : windowPosition + 2 ≤ bottomCount)
    (tailBoundaryFits : tailBoundary + 2 = bottomCount)
  ...
  exact internalCountListCorr_ofPointwise
      ...
      show countEventsInRoot
        ...
        = countEventsInRoot
            ...
            (processArcSpine
              ...
              atoms).cupEventNodes
      rw [readShifted]
      exact arcCapHeadFolded_cupEventCountAtImage bottomCount windowPosition
        tailBoundary windowFits tailBoundaryFits atoms chained
        (natListGetAt
          (List.range tailBoundary
            ++ (processArcSpine
    ...
    (by
      ...
      rw [boundaryReadLeft]
      exact arcCapHeadFolded_windowStrandCupCount bottomCount windowPosition tailBoundary
        windowFits tailBoundaryFits atoms chained)
    (by
      show countEventsInRoot
          (processArcSpine
...
/-- **Honesty marker — the composite internal-count lists are the spliced fresh lists
(peel campaign H, rung E-3, part 10).**  `arcCapHeadFolded_internalCapCountsCorr` /
`arcCapHeadFolded_internalCupCountsCorr`: at the cap-head folded end state the whole
per-port internal cap-count list equals the fresh list with `[1, 1]` spliced at the window
(the head's own cap event on both consumed-strand ports), and the cup-count list equals
the fresh list with `[0, 0]` spliced.  Count values are boundary-invariant, so unlike the
partner list no value reindexing appears — the generic spliced-list assembly runs once
over both event kinds.  What this marker does NOT claim: the assembled `FullArcStructure`
equality (diagram + totals + these lists packaged) — the next rung.  `= true`. -/
def fxMode_hasArcCapInternalCounts : Bool := true
