...
/-! # ArcCapWindowCounts — the consumed strand's event counts at the cap head

The window ports' internal-count values (peel campaign H, rung E-3, part 9).  The peeled
cap's consumed strand — left wire, right wire, head event node — carries exactly ONE cap
event (the head's own) and NO cup event at the composite end state: the composite event
lists are the reindexed fresh lists over the head suffix, every reindexed image misses the
strand (the seed-closure payoff), and the head event rides the persistent event-to-wire
link.  The right port reads the same strand (the consumed pair stays linked), so its root
coincides with the left port's.  These are the two spliced values of the internal-count
list transports.
...
theorem arcCapHeadFolded_windowStrandCapCount
  ...
  have missesRoot : ∀ eventNode : Nat,
    ...
    fun eventNode =>
      natBeqSymmFalse
        (arcCapHeadFolded_windowAnchorMissesReindexed bottomCount windowPosition
...
strand. -/
theorem arcCapHeadFolded_windowStrandCupCount
    {overallSource overallTarget : adjunctionGraph.Mode}
    (bottomCount windowPosition tailBoundary : Nat)
    (windowFits : windowPosition + 2 ≤ bottomCount)
    (tailBoundaryFits : tailBoundary + 2 = bottomCount)
...
port's counts. -/
theorem arcCapHeadFolded_windowRightRootEq
    {overallSource overallTarget : adjunctionGraph.Mode}
...
/-- **Honesty marker — the consumed strand's event counts (peel campaign H, rung E-3,
part 9).**  `arcCapHeadFolded_windowStrandCapCount` / `arcCapHeadFolded_windowStrandCupCount`:
the peeled cap's consumed strand carries exactly one cap event (the head's own, riding the
persistent event-to-wire link) and no cup event (every reindexed fresh image misses the
strand — the seed-closure payoff over the append-split event lists).
`arcCapHeadFolded_windowRightRootEq`: the right window port's root equals the left's, so
both spliced ports inherit these counts.  What this marker does NOT claim: the assembled
internal-count LIST transports or the `FullArcStructure` equality — the next rungs.
`= true`. -/
def fxMode_hasArcCapWindowCounts : Bool := true
