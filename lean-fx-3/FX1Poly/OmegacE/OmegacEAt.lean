...
/-- Three-successor cancellation, propext-free.

`Nat.add_right_cancel` from the standard library is proved through
machinery that depends on `propext`; this direct triple `Nat.succ.inj`
keeps the indexed `DecidableEq` below axiom-free. -/
private def OmegacECell.addThreeCancel {leftBase rightBase : Nat}
    (sumsEqual : leftBase + 3 = rightBase + 3) : leftBase = rightBase :=
  Nat.succ.inj (Nat.succ.inj (Nat.succ.inj sumsEqual))

/-- Reindexing a `higherCoherence` cell across an index equality on the base
dimension leaves the cell unchanged (the `Fin 2` slot does not depend on the
base dimension). -/
private def OmegacECell.castHigher {baseDim baseDimReindexed : Nat}
    (cellIndex : Fin 2) (indexEqual : baseDimReindexed + 3 = baseDim + 3) :
    (indexEqual ▸ OmegacECell.higherCoherence baseDimReindexed cellIndex
        : OmegacECell (baseDim + 3))
      = OmegacECell.higherCoherence baseDim cellIndex :=
  (OmegacECell.addThreeCancel indexEqual).rec
    (motive := fun (targetBase : Nat) (_baseEqual : baseDimReindexed = targetBase) =>
      ∀ (sumsEqual : baseDimReindexed + 3 = targetBase + 3),
        (sumsEqual ▸ OmegacECell.higherCoherence baseDimReindexed cellIndex
            : OmegacECell (targetBase + 3))
          = OmegacECell.higherCoherence targetBase cellIndex)
    (fun _sumsEqual => rfl)
    indexEqual

/-- Decide equality of two `higherCoherence` cells at the same base dimension
by comparing their `Fin 2` slot, propext-free. -/
private def OmegacECell.decideHigher {baseDim : Nat} (leftIndex rightIndex : Fin 2) :
    Decidable (OmegacECell.higherCoherence baseDim leftIndex
      = OmegacECell.higherCoherence baseDim rightIndex) :=
  if indicesEqual : leftIndex = rightIndex then
    isTrue (by subst indicesEqual; rfl)
  else
    isFalse (fun cellsEqual =>
      indicesEqual (OmegacECell.higherCoherence.inj cellsEqual))

/-- Hand-rolled propext-free decidable equality for the indexed cell family.

The standard `deriving DecidableEq` on an indexed inductive leaks `propext`
through the match compiler's impossible-by-index equation lemmas (the
match-arity trap).  This definition dispatches with an outer single-scrutinee
match on `left` (which refines the dimension index cleanly) and an inner
`casesOn` over `right` carrying an index-equality witness, discharging the
cross-dimension impossible constructors via `Nat.noConfusion`. -/
def OmegacECell.decEq : {dimension : Nat} →
    (left right : OmegacECell dimension) → Decidable (left = right)
  | _, .sourceVertex, right =>
    OmegacECell.casesOn
      (motive := fun (dim : Nat) (candidate : OmegacECell dim) =>
        (dimEqual : dim = 0) →
        Decidable (OmegacECell.sourceVertex = dimEqual ▸ candidate))
      right
      (fun _ => isTrue rfl)
      (fun _ => isFalse (fun cellsEqual => by cases cellsEqual))
      (fun (dimEqual : (1:Nat) = 0) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (1:Nat) = 0) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (2:Nat) = 0) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (2:Nat) = 0) => Nat.noConfusion dimEqual)
      (fun _ _ (dimEqual : (_ + 3:Nat) = 0) => Nat.noConfusion dimEqual)
      rfl
  | _, .targetVertex, right =>
    OmegacECell.casesOn
      (motive := fun (dim : Nat) (candidate : OmegacECell dim) =>
        (dimEqual : dim = 0) →
        Decidable (OmegacECell.targetVertex = dimEqual ▸ candidate))
      right
      (fun _ => isFalse (fun cellsEqual => by cases cellsEqual))
      (fun _ => isTrue rfl)
      (fun (dimEqual : (1:Nat) = 0) => Nat.noConfusion dimEqual)
  ...
  | _, .quasiIso, right =>
    OmegacECell.casesOn
      (motive := fun (dim : Nat) (candidate : OmegacECell dim) =>
        (dimEqual : dim = 1) →
        Decidable (OmegacECell.quasiIso = dimEqual ▸ candidate))
      right
      (fun (dimEqual : (0:Nat) = 1) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (0:Nat) = 1) => Nat.noConfusion dimEqual)
      (fun _ => isTrue rfl)
      (fun _ => isFalse (fun cellsEqual => by cases cellsEqual))
      (fun (dimEqual : (2:Nat) = 1) => Nat.noConfusion (Nat.succ.inj dimEqual))
      (fun (dimEqual : (2:Nat) = 1) => Nat.noConfusion (Nat.succ.inj dimEqual))
      (fun _ _ (dimEqual : (_ + 3:Nat) = 1) =>
        Nat.noConfusion (Nat.succ.inj dimEqual))
      rfl
  | _, .quasiInverse, right =>
    OmegacECell.casesOn
      (motive := fun (dim : Nat) (candidate : OmegacECell dim) =>
        (dimEqual : dim = 1) →
        Decidable (OmegacECell.quasiInverse = dimEqual ▸ candidate))
      right
      (fun (dimEqual : (0:Nat) = 1) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (0:Nat) = 1) => Nat.noConfusion dimEqual)
      (fun _ => isFalse (fun cellsEqual => by cases cellsEqual))
  ...
  | _, .alphaUnit, right =>
    OmegacECell.casesOn
      (motive := fun (dim : Nat) (candidate : OmegacECell dim) =>
        (dimEqual : dim = 2) →
        Decidable (OmegacECell.alphaUnit = dimEqual ▸ candidate))
      right
      (fun (dimEqual : (0:Nat) = 2) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (0:Nat) = 2) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (1:Nat) = 2) => Nat.noConfusion (Nat.succ.inj dimEqual))
      (fun (dimEqual : (1:Nat) = 2) => Nat.noConfusion (Nat.succ.inj dimEqual))
      (fun _ => isTrue rfl)
      (fun _ => isFalse (fun cellsEqual => by cases cellsEqual))
      (fun _ _ (dimEqual : (_ + 3:Nat) = 2) =>
        Nat.noConfusion (Nat.succ.inj (Nat.succ.inj dimEqual)))
      rfl
  | _, .betaCounit, right =>
    OmegacECell.casesOn
      (motive := fun (dim : Nat) (candidate : OmegacECell dim) =>
        (dimEqual : dim = 2) →
        Decidable (OmegacECell.betaCounit = dimEqual ▸ candidate))
      right
      (fun (dimEqual : (0:Nat) = 2) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (0:Nat) = 2) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (1:Nat) = 2) => Nat.noConfusion (Nat.succ.inj dimEqual))
      ...
      (fun _ _ (dimEqual : (_ + 3:Nat) = 2) =>
        Nat.noConfusion (Nat.succ.inj (Nat.succ.inj dimEqual)))
      rfl
  | _, .higherCoherence baseDim leftIndex, right =>
    OmegacECell.casesOn
      (motive := fun (dim : Nat) (candidate : OmegacECell dim) =>
        (dimEqual : dim = baseDim + 3) →
        Decidable (OmegacECell.higherCoherence baseDim leftIndex
          = dimEqual ▸ candidate))
      right
      (fun (dimEqual : (0:Nat) = baseDim + 3) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (0:Nat) = baseDim + 3) => Nat.noConfusion dimEqual)
      (fun (dimEqual : (1:Nat) = baseDim + 3) =>
        Nat.noConfusion (Nat.succ.inj dimEqual))
      (fun (dimEqual : (1:Nat) = baseDim + 3) =>
        Nat.noConfusion (Nat.succ.inj dimEqual))
      (fun (dimEqual : (2:Nat) = baseDim + 3) =>
        Nat.noConfusion (Nat.succ.inj (Nat.succ.inj dimEqual)))
      (fun (dimEqual : (2:Nat) = baseDim + 3) =>
        Nat.noConfusion (Nat.succ.inj (Nat.succ.inj dimEqual)))
      (fun _ rightIndex (dimEqual : (_ + 3:Nat) = baseDim + 3) =>
        (OmegacECell.castHigher rightIndex dimEqual)
          ▸ OmegacECell.decideHigher leftIndex rightIndex)
      rfl

instance instDecidableEqOmegacECell {dimension : Nat} :
    DecidableEq (OmegacECell dimension) := OmegacECell.decEq
