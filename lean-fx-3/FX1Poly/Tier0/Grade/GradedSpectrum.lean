...
## §7 rulings upheld (see `grade-mode-spectrum.md` §7)
  ...
  * §7.2 fibrancy is ONE mode property: φ is the single `geometric` field, the finer reading of
    `FibrancyMode` — nothing replaced, no `Tier0/Fibrancy/` folder.
  * §7.3 no fifth axis: this file lives in `Tier0/Mode/`, introduces no new judgment index / ω-category /
...
The mode carrier (`Polygraph/Computad`) is untouched; this file imports only `Tier0/Mode` siblings.
