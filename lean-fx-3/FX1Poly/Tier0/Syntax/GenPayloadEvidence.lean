...
This file ships the per-generator payload-evidence layer:
`GenPayloadEvidence generator scope payload : Type`.  The discipline
is honest about what the payload-TYPE-level structure already enforces
versus what genuine evidence would need to add.

## What the payload type already enforces (no evidence needed)

`Generator.payload : Generator → Nat → Type` selects one of three
payload shapes per generator:

* `gen_var → Fin scope` — scope safety is **structural by
  construction**.  `Fin scope` has no out-of-bounds inhabitants, so
  the `index < scope` side condition is enforced by the payload type
  itself.  The polycell.md §4 spec confirms this explicitly:
  *"var's index < scope is structural via the Fin scope payload"*.
...
## Design commitment: fxProfile uses UNBOUNDED universes
...
`GenPayloadEvidence` definition (`Unit`).

* All other generators have `Unit` payload — only one inhabitant,
...
**Consistency with `SupportedGenerator`**: both admission layers
...
any `level : Nat`, those payloads admit under fxProfile. -/
