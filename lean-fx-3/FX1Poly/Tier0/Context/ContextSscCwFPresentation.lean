...
## What is deferred (the honest wall)
  ...
  * This is the SELF-CONTAINED Tier0 presentation residue, NOT a Core `StepOverTable` iota row
...
laws / strictification / singleton bricks — and `FxBaseSubstColimits` — the initial object).  A Tier0
...
algebra (`Tier0/Term/TermAxis.lean`); the comprehension laws (`context-1` `ComprehensionLaws.lean`), the
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 presentation residue assembling the shipped
