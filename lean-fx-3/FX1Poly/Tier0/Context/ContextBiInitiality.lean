...
## What is deferred (the honest wall)
  ...
  * The WEAK (∞,ω) BI-INITIALITY — the interpreting morphism's action on substitutions (`×term`), the
    ...
    `fib-5c`/`fib-5d`.
  * The bi-initiality here is the SELF-CONTAINED Tier0 residue, NOT a Core `StepOverTable` iota row
    (`fxContextBiInitiality_isOverCoreIotaTable = false`).

Imports only `Initiality` (its `context-5` sibling).  A Tier0 design-lock must not import up into `Core/` /
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 residue of bi-initiality, re-presenting `context-5`'s
