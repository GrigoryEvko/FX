...
## What is deferred (the honest wall)
  ...
  * The classifier here is the SELF-CONTAINED Tier0 analog, NOT a Core `StepOverTable` iota row
...
`discreteUniverseObject` / `[C, U]` substrate).  A Tier0 design-lock must not import up into `Core/` / `Typed/`.
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 analog of a Core table-native classifier row; a
Tier0 design-lock cannot import the Core engine, so the classifier is re-shipped here over the `context-30` /
`context-37` substrate.  Gluing the Tier0 and Core forms is cross-axis `×type`.  `= false`. -/
