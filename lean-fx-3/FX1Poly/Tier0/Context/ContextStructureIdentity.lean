...
## What is deferred (the honest wall)
  ...
  * The SIP transport here is the SELF-CONTAINED Tier0 analog, NOT a Core `StepOverTable` iota row
...
Tier0 design-lock must not import up into `Core/` / `Typed/`.  Zero external dependencies — raw Lean 4 +
Init only.
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 analog of a Core table-native SIP row; a Tier0
design-lock cannot import the Core engine, so the transport and observational substitution are re-shipped
here over the `context-30`/`context-31` universe object.  Gluing the Tier0 and Core forms is cross-axis
`×type`.  `= false`. -/
