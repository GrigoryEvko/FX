...
(`Tier0/Term/Rewrite/MarkedComplicial.lean`, the marked/complicial term ω-category) and of `mode-7`'s thin-cell
...
`composeHom` / strict-law / `DirectedHomIso` substrate).  A Tier0 design-lock must not import up into `Core/` /
...
(`Tier0/Term/Rewrite/MarkedComplicial.lean`); the `context-33` synthetic ∞-category
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 analog of a Core table-native marked-complicial row;
a Tier0 design-lock cannot import the Core engine, so the marking is re-shipped here as a strict
construction.  Gluing the Tier0 and Core forms is cross-axis `×type`.  `= false`. -/
