...
modeled here as the finite enumeration `ContextFormer` (a Tier0 design-lock cannot import the typed layer,
...
## What is deferred (the honest wall)
  ...
  * The presentation here is the SELF-CONTAINED Tier0 analog, NOT a Core `StepOverTable` iota row
...
`discreteUniverseObject` substrate).  A Tier0 design-lock must not import up into `Core/` / `Typed/`.  Zero
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 analog of a Core table-native polygraph row; a
Tier0 design-lock cannot import the Core engine (nor the typed-layer `TypingContext`), so the generators are
re-modeled here over the scope substrate.  Gluing the Tier0 and Core forms is cross-axis `×type`.  `= false`. -/
