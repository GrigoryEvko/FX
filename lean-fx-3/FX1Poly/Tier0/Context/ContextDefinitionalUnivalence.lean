...
conversion is the generic crank.  A Tier0 design-lock cannot import the Core table engine, so `context-31`
ships the SAME move SELF-CONTAINED over a small context-universe code algebra:
  ...
  * **`ContextUnivCode.normalize`** — the structural normalizer realizing the reduction (the Tier0 analog of
...
## Honesty
  ...
  * The reduction is the SELF-CONTAINED Tier0 analog of the Core table-native `type-7` row; it is NOT literally
    ...
    Core/Tier0 gluing is cross-axis `×type`.
...
/-- The structural normalizer realizing the `Id_U ↝ Equiv` reduction (the Tier0 analog of `type-7`'s
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 analog of the Core table-native `type-7` row
`univalenceShapedDemoRule` (over `StepOverTable`, with generic orthogonal-table confluence).  A Tier0
design-lock cannot import the Core engine, so the reduction is re-shipped here as a structural normalizer; the
two agree in shape (drop the universe scrutinee, size-decreasing, decidable conversion).  Gluing the Tier0 and
