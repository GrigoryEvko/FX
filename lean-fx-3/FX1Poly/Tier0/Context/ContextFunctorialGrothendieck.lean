...
## What is deferred (the honest wall)
  ...
  * The classification + functoriality here are the SELF-CONTAINED Tier0 analog, NOT a Core `StepOverTable` iota
...
directed-universe substrate).  A Tier0 design-lock must not import up into `Core/` / `Typed/`.  Zero external
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Tier0 analog of a Core table-native functorial-Grothendieck
row; a Tier0 design-lock cannot import the Core engine, so the categories, functors, and isomorphism are
re-shipped here as strict (1-categorical) constructions.  Gluing the Tier0 and Core forms is cross-axis `×type`.
`= false`. -/
