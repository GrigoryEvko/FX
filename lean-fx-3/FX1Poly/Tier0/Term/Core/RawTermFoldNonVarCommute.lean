...
the head generator is `.gen_var`:
  ...
  * NON-VARIABLE case — recurse into the children spine and rebuild the cell, casting the payload
    across scopes via `Generator.payload_scope_invariant_of_not_var` (the 205-generator enumeration
...
`Generator.payload_scope_invariant_of_not_var` (the one-site 205-generator enumeration).
