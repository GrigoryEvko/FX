...
/-! # FX1Poly/Core/ConsistencyViaSconing
    — consistency via the sconing leg: no closed well-typed proof of the empty type (SN-050/053)
...
canonicity (`closedReducesToValue`), and now consistency (`emptyHasNoClosedMember`) — each derived from
the SAME fundamental obligation, the BKS "sconing is enough" thesis (the consistency corner of SN-110).
...
* `consistencyScone` — the sconing witness for consistency.  `computable` is the empty candidate; the
  ...
  `#672`-free `emptyHasNoClosedMember`; `fundamental` (closed well-typed empty term ⟹ candidate member —
...
* `consistencyViaSconing` — `SconingWitness.canonicity` applied: given the fundamental, every closed
  well-typed term of the empty type yields `False`.  This is SN-050/053 consistency via Path C —
  structurally identical to the canonicity derivation, since consistency IS canonicity at the empty type.

## Honest scope

`#672`-FREE EXTRACTION.  The `fundamental` obligation (closed well-typed empty term ⟹ empty-candidate
member) is the typed reducibility fundamental theorem, gated on `#672` / SN-043, and is an explicit
hypothesis here — not discharged.  This mirrors the canonicity files exactly.

## Zero-axiom verification

A record literal over `SconingWitness` whose `extraction` field is the shipped `#672`-free
...
/-- **The consistency sconing witness (Path C), `#672`-free EXTRACTION.**  The empty type's reducibility
...
EXTRACTION obligation is discharged `#672`-free by `emptyHasNoClosedMember` (no closed term inhabits the
...
parameter — the typed reducibility fundamental theorem (`#672` / SN-043). -/
...
/-- **Consistency via the sconing leg (SN-050/053): no closed well-typed proof of the empty type.**
...
consistency, all from one fundamental.  The extraction half is shipped `#672`-free. -/
