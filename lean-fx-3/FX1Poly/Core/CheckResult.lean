...
inductive CellCheckRejection where
  ...
  /-- The fuelled checker exhausted its structural recursion budget. -/
  | fuelExhausted
...
def all : List CellCheckRejection :=
  [.unknownGenerator, .wrongSort, .badPayload, .wrongArity,
    ...
    .unsupportedCompH, .unsupportedCertification, .fuelExhausted]
...
/-- The rejection enumeration has exactly ten entries. -/
theorem all_length : all.length = 10 := rfl
