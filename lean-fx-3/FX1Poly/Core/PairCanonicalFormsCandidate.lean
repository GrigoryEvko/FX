...
/-! # Foundation/PolyCell/Core/PairCanonicalFormsCandidate
    — the binary data reducibility candidate: Σ-pairs, unconditional + zero-axiom

Bool (`BoolCanonicalFormsCandidate.lean`) is a NULLARY constructor; Nat (`NatCanonicalFormsCandidate.lean`)
is a UNARY recursive constructor.  The Σ pair `(a, b)` is the BINARY case — two children of arbitrary
...
(Σ-canonicity).  This is the data core of the Σ-introduction reducibility member; the fst/snd
projection-reducibility consumes this candidate.
...
`isPairValue_impliesStepNormalForm`.  The Tait candidate for the Σ-type, the data core of Σ reducibility. -/
...
reducibility: a pair of reducible-normal components inhabits its type's reducibility candidate. -/
...
fundamental theorem) this is closed-Σ canonicity.  The extraction shown here
is fundamental-free. -/
