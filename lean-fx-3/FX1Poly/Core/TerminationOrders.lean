...
/-! # FX1Poly/Core/TerminationOrders
    — the lexicographic list order + measure-based termination certificates (SN-117 lex foundation, #620)

Companion to SN-116's Dershowitz-Manna multiset order.  This file adds the other classical argument-comparison
...
The lexicographic order is the comparison LPO uses for its arguments (SN-117, "for ι-eliminator rules") and that
RPO uses for lexicographic-status symbols (SN-115); the multiset order (SN-116) is for multiset-status symbols.
The recursive PATH ordering on the FX term structure (composing a precedence on the 194 generators with these two
comparison orders) is the larger downstream construction the FX certified-fragment termination (`SN-131`,
typed-SN-gated) will assemble — this file supplies the lex comparison order it builds on.
...
* `wellFounded_of_multisetMeasure` / `wellFounded_of_lexMeasure` — **termination certificates**: a step relation
  ...
  `InvImage.wf` of SN-116 / the lex order.

## Zero-axiom verification

Same `List`-existential discipline as SN-116: inversion by `obtain` + `cases commonPrefix` (a clean `List` split,
...
inverse image of the SN-116 multiset order under the measure. -/
