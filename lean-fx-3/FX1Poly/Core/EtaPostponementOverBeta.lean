...
/-! # FX1Poly/Core/EtaPostponementOverBeta
    — η-postponement over β: per-η-constructor quasi-commutation

The open βη-SN assembly (`accUnionBetaEta`) consumes one hypothesis,
...
This file collects those per-constructor lemmas; `etaQuasiCommutesOverBeta` assembles them by casing on the
η-step.

## The postponement shape (etaLam)
...
duplicating constructors (etaPair) genuinely use.
...
/-- **etaModIntro quasi-commutes over β.**  The etaModIntro case of
...
/-- **etaPair quasi-commutes over β.**  The etaPair case of `QuasiCommutesRightOverLeft Step
...
/-! ## etaPathLam (the cubical binder case)
...
/-- **etaPathLam quasi-commutes over β.**  The etaPathLam case of
...
/-! ## etaGlueIntro (the second DUPLICATING case)
...
/-- **etaGlueIntro quasi-commutes over β.**  The etaGlueIntro case of
...
/-! ## Assembly: the full crux EtaQuasiCommutesOverBeta

Casing on the η-step dispatches to the five per-constructor lemmas above.  The `cases` is over the indexed
`Step.eta` with FREE-variable indices (`a`, `b` are free), so index unification is pure
substitution — no impossible cases, no `noConfusion`, propext-clean (verified by the per-decl gate). -/

/-- **η quasi-commutes over β — the discharged crux.**  The full `EtaQuasiCommutesOverBeta`: every
...
— making open βη-SN UNCONDITIONAL for well-typed terms. -/
