...
/-! # FX1Poly/Core/ConvWordJoinableBridge
    — Conv ⟹ word joinability under the generated system
...
bridge: `Conv a b ⟹ FxWordJoinable fxStepSystem a.toCode b.toCode`.

The forward map is clean: a common term reduct `c` (from `Conv`) encodes to a common WORD reduct `c.toCode`, and
each `StepStar` leg maps to a `FxWordRewritesMany` leg via `StepStar.toWordRewrites`.
...
word→term completeness blocked in `StepWordRewriteEquivariance.lean`: the free word monoid is more
...
system (typed-SN-gated), so it is NOT claimed.  This is exactly the joinability shape the
OmegacE `decidableConvertibleModulo_ofConvergent` consumes — the FX
word decider instantiates on it once termination + confluence land.
...
the word-rewrite soundness.  No `axiom`, `sorry`, `propext`, `Quot.sound`, `Classical`, `native_decide`, `omega`.
...
term reduct encodes to a common word reduct, and each `StepStar` leg maps via `StepStar.toWordRewrites`. -/
