import FX1Poly.Core.Substrate.Profile.PolyCell
...
The rawCell is a TYPE INDEX of `PolyCell`, not a separately-
...
1. **Downstream rewrites.**  Code that pattern-matches on a
   `PolyCell` value and wants to relate its raw erasure to the
...
`PolyCell.raw : PolyCell ... rawCell → RawCell scope` takes
...
def raw {profile : PolyProfile} {sort : CellSort} {dim scope : Nat}
    ...
    (_cell : PolyCell profile sort dim scope boundary rawCell) :
    RawCell scope :=
  rawCell

/-- The `gen` constructor's raw erasure is the `RawCell.termBase`
wrapper around `RawTerm.mkGen` with the generator, payload, and
children spine.
...
theorem gen_raw_eq {profile : PolyProfile} {scope : Nat}
    ...
    (PolyCell.gen admission payloadEvidence childSpine).raw =
      .termBase (.mkGen generator payload children) :=
  rfl

/-- The `generatingCell` constructor's raw erasure is the
`RawCell.generatingCell` triple of `(rule.ruleId, source, target)`.
...
`RawCell.verticalComposite` of the two component raw cells.
...
theorem verticalComposite_raw_eq {profile : PolyProfile}
    ...
    (secondCell : PolyCell profile sort (dim + 1) scope
                    ...
                    secondRaw) :
    (PolyCell.verticalComposite firstCell secondCell).raw =
      .verticalComposite firstRaw secondRaw :=
  rfl

/-- The `identityCell` constructor's raw erasure is
`RawCell.identityCell` of the base raw cell.
...
theorem identityCell_raw_eq {profile : PolyProfile} {sort : CellSort}
    ...
    (PolyCell.identityCell baseCell).raw =
