...
/-! # FX1Poly/Core/GeneratorFinitePolygraph
    — the FX kernel as a FINITE POLYGRAPH over the 195-Generator table (SN-123, #626)

The FX kernel's term former table `Generator` (195 nullary constructors, `GeneratorCore.lean`) is exactly the
generating 0-cell set of the kernel's polygraph presentation.  This file packages it as a FINITE polygraph: the
generators are indexed bijectively-on-range into `Fin 195`, and each carries its dimension (child count) and
boundary (binder shifts), coherently.

Reuses the SHIPPED §11.6.4 table validation (`GeneratorTagRoundTrip.lean`): `Generator.toNat` (the tag 0–194),
...
* `Generator.toNat_lt` — every tag is `< 195`, so `toNat` embeds `Generator` into `Fin 195` (injective + bounded
  ⟹ `Generator` is finite of cardinality `≤ 195`).
* `Generator.fromTag_total_on_range` — the inverse table is TOTAL on `0–194` (every tag names a generator), the
  surjectivity-on-range completing the `Generator ≅ Fin 195` correspondence.
...
witness that the FX kernel IS a finite polygraph over the 195-Generator table.  This is the Leg-3 anchor for the
later `Generator → polygraph-generator` map (SN-124) and the `RawCell → OmegacEWord` encoding (SN-125).

## Zero-axiom verification

`toNat_lt` is `cases generator <;> decide` (each tag a literal `< 195`); `fromTag_total_on_range` is `decide` over
the bounded `∀ tag < 195` (raised `maxRecDepth` for the 195-wide table evaluation — plain `decide`, NOT
...
/-- The `toNat` tag of every generator is below the table size 195 — so `Generator.toNat` embeds `Generator` into
`Fin 195`.  With `toNat_injective` (shipped) this proves `Generator` is finite of cardinality at most 195. -/
theorem Generator.toNat_lt (generator : Generator) : generator.toNat < 195 := by
  cases generator <;> decide

set_option maxRecDepth 4000 in
/-- The inverse table `Generator.fromTag` is TOTAL on the tag range `0–194`: every tag names a generator.  The
surjectivity-on-range that, with the round-trip `fromTag_toNat`, completes the `Generator ≅ Fin 195`
correspondence.  Proved by `decide` over the bounded `∀ tag < 195` (evaluating the 195-entry table). -/
theorem Generator.fromTag_total_on_range :
    ∀ (tag : Nat), tag < 195 → (Generator.fromTag tag).isSome = true := by
  decide
...
/-- **The FX kernel IS a finite polygraph over the 195-Generator table.**  The 195 generators are indexed
injectively (`toNat_injective`) and boundedly (`toNat_lt`) into `Fin 195`, with the total inverse table
