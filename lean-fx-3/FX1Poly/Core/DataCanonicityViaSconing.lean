...
/-! # FX1Poly/Core/DataCanonicityViaSconing
    — the GENERIC data-canonicity sconing witness: one theorem for every data axis

`BoolCanonicityViaSconing.lean`'s concrete data sconing witness uses the bool-specific extraction
`boolClosedReducesToTrueOrFalse`.  The key observation this file makes precise:
...
`isValue`" fundamental-free for ANY value predicate `isValue` (a closed member is non-neutral, so its
"neutral OR reduces-to-value" disjunct collapses to the right branch).  The bool witness is a
...
* `dataCanonicityScone` — the GENERIC data sconing witness, parametric in the value predicate `isValue`.
  ...
  fundamental-free `CanonicalFormsPredicate.closedReducesToValue`; `fundamental` (closed well-typed data ⟹
...
* `natCanonicityViaSconing` / `listCanonicityViaSconing` / `optionCanonicityViaSconing` /
  ...
  the §3.12 canonicity headline per data type via the sconing leg.

This is the "one functor ⟹ canonicity for all data" realization (the BKS sconing-is-enough thesis)
made concrete for the data axis: a SINGLE generic witness, each data type a thin
specialization — not one copy-pasted file per type.  Every instance shares the SAME fundamental-free
extraction and the SAME (per-type) fundamental obligation that Path A also needs.

## Zero-axiom verification

A record literal over `SconingWitness` whose `extraction` field is the generic fundamental-free
...
obligation is discharged fundamental-free by the uniform `CanonicalFormsPredicate.closedReducesToValue` (a
...
Path-A fundamental theorem.  Subsumes the bool-specific `boolCanonicityScone`. -/
...
for every data type; each concrete type instantiates `isValue`.  Extraction is fundamental-free, so the
...
/-- **Closed-Nat canonicity via the sconing leg.**  `dataCanonicityViaSconing` at
...
/-- **Closed-List canonicity via the sconing leg.**  `dataCanonicityViaSconing` at
...
/-- **Closed-Option canonicity via the sconing leg.**  `dataCanonicityViaSconing` at
...
/-- **Closed-Either canonicity via the sconing leg.**  `dataCanonicityViaSconing` at
...
/-- **Closed-Pair (Σ-intro) canonicity via the sconing leg.**  `dataCanonicityViaSconing`
...
/-- **Closed-Unit canonicity via the sconing leg.**  `dataCanonicityViaSconing` at
`isValue := isUnitValue`: every closed well-typed Unit term reduces to the unit constructor, modulo the
fundamental obligation.  The one-value (§3.12) canonicity instance. -/
...
/-- **Closed-identity canonicity via the sconing leg (introduction half).**
`dataCanonicityViaSconing` at `isValue := isReflValue`: every closed well-typed identity-type term reduces to
`refl`, modulo the fundamental obligation.  The identity-introduction canonicity instance.  The generic
sconing witness covers ALL data axes (bool / nat / list / option / either / pair / unit / identity). -/
