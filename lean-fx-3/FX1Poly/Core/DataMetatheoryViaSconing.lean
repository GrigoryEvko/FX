...
/-! # FX1Poly/Core/DataMetatheoryViaSconing
    — the BKS bundling capstone: ONE fundamental obligation ⟹ BOTH normalization AND canonicity
...
obligation yields BOTH metatheorems, sharing the SAME (fundamental-gated) hard work.
...
extraction (`CanonicalFormsPredicate.closedReducesToValue`).  Both projections are fundamental-free.  So
from the one fundamental obligation (closed well-typed ⟹ candidate member — the fundamental theorem of
the logical relation) BOTH metatheorems follow with no further hard work.
...
* `dataMetatheoryViaSconing` — the BKS capstone: given the single fundamental obligation, BOTH
  ...
  (the sconing-is-enough thesis at the BKS-package ledger level),
  restricted to the data axis where both extractions are fundamental-free.

This does NOT flip the Tier-0 `ln.hasBKSMetatheoryPackage` ledger flag: that flag tracks the CATEGORICAL
BKS package over `fxBaseRMC` (the glued-model preservation of Π/Σ/universe), which is a
...
/-- **The BKS bundling capstone for the data axis: one fundamental ⟹ both metatheorems.**
Given the single fundamental obligation (closed well-typed ⟹ data-candidate member — the fundamental
theorem of the logical relation, the SOLE fundamental-gated hard work), BOTH normalization and canonicity
