...
/-! # FX1Poly/Core/BoolCanonicityViaSconing
    — the FIRST concrete data-type sconing witness: bool canonicity (`→ true`/`false`) via Path C (SN-092/100)
...
"reduces to `true` or `false`" — and discharges its EXTRACTION obligation `#672`-free:

* `boolCanonicityScone` — the concrete bool sconing witness.  `computable` is the bool reducibility candidate
  (`CanonicalFormsPredicate boolIsValue`); `extraction` is exactly the shipped `#672`-free
  ...
  explicit parameter, the SOLE remaining obligation (= the Path-A fundamental theorem, gated on `#672` / SN-043).
...
The point is the BKS thesis made concrete: for bool canonicity the EXTRACTION half is already done `#672`-free
...
methods, one object", and one shared `#672` obligation.

## Zero-axiom verification

A record literal over `SconingWitness` whose `extraction` field is `boolClosedReducesToTrueOrFalse` (the shipped
`#672`-free canonicity extraction) plus `SconingWitness.canonicity` (composition).  The `fundamental` obligation
...
/-- **The concrete bool-canonicity sconing witness (Path C), `#672`-free EXTRACTION.**  Relative to any
...
EXTRACTION obligation is discharged `#672`-free by `boolClosedReducesToTrueOrFalse` (a closed candidate member
reduces to a boolean value).  The FUNDAMENTAL obligation (closed well-typed bool ⟹ candidate member) is the
explicit parameter — the SOLE remaining hard obligation, the Path-A fundamental theorem (`#672` / SN-043). -/
...
canonicity as the Tait route, modulo the SAME fundamental theorem (`#672`).  The extraction half is shipped
`#672`-free, so the sconing leg adds no new obligation beyond the one Path A already carries. -/
