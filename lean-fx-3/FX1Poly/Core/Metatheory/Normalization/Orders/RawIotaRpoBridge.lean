...
## What this ships
  ...
  * `genRank` / `realGenPrecedence` / `realGenPrecedence_wellFounded` — the real generator precedence
    ...
    decidable-equality `if`s (no 205-constructor wildcard match, which would leak propext).
...
Defined by decidable-equality `if`s (no 205-constructor wildcard match, which would leak propext). -/
