...
/-! # FX1Poly/Core/ModalCanonicityViaSconing
    — modal box canonicity via the sconing leg, completing canonicity-via-sconing to the modal axis
...
This file adds its sconing-canonicity instance, completing canonicity-via-sconing coverage
...
extraction is the SAME uniform fundamental-free `CanonicalFormsPredicate.closedReducesToValue` the data witnesses use,
so this canonicity is conditional ONLY on the per-type fundamental obligation (closed well-typed box ⟹
box-candidate member) — NOT on typed SN.  Genuinely unblocked, like the data instances.
...
* `modIntroCanonicityViaSconing` — `dataCanonicityViaSconing` at `isValue := isModIntroValue`: every closed
  ...
  canonicity headline for the modal box via the sconing leg.
...
the generic fundamental-free data-canonicity sconing witness — completing the canonicity-via-sconing coverage to all
