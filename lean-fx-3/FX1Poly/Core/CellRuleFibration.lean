...
import FX1Poly.Axis.Term.Cell.CellSort
...
The Axis `RuleFibration` instantiated at the kernel's own axes and heads: the AXES are the seven
...
The generic lookup / inhabitation / orthogonality machinery comes for free from Axis; the typed
...
open FX1Poly.Axis (RuleFibration)

/-- ★ **The cell-rule fibration** — the Axis `RuleFibration` at the seven `CellSort` axes and the
