...
Continuity for `mulReal`, resting on the real-level product-difference law
`mulRealRespectsIsWithinRealBound`.  `mulReal` is uniformly continuous only on
bounded subdomains (matching the `RealContinuity` scope note):
  ...
  * Bounded-domain, two-variable: when the left first factor and the right
    ...
    recombine exactly.  No fixed modulus works globally (the slope grows), so
    the domain bound `p` enters the modulus.
  ...
  * Fixed-factor, global: `value |-> factor * value` is globally uniformly
    ...
    `(p+1)/1`, the single surviving leg `(p+1)/1 * eps` collapses to `1/(k+1)`
    at the factor-scaled input precision, and the vanishing leg's `L * 0`
    drops.  Packaged as `IsUniformlyContinuous`.
...
(no factor of two), so the bound's numerator cancels the scaled denominator
exactly to the reciprocal. -/
def factorScaledIndex (boundPredecessor index : Nat) : Nat :=
  (boundPredecessor + 1) * index + boundPredecessor

/-- Fixed-factor scale collapse: `(bp+1)/1 * 1/(factorScaledIndex bp i + 1)`
...
/-- Multiplication is uniformly continuous on a bounded domain (two variables).
With the left first factor and the right second factor uniformly
...
/-- Fixed general-sign real factor scales the real-level bound by the magnitude
ceiling — `factor * leftValue` and `factor * rightValue` sit within
`|factor| * bound` whenever the inputs sit within `bound`, for any real factor of
either sign.  `canonicalBound factor` is the two-sided magnitude ceiling
`(|x_0|+2)/1`, and `approximationIsWithinCanonicalBound` witnesses both
`v ≤ ceiling` and `-v ≤ ceiling` at the source.  The slack closure delegates to
`mulRealRespectsIsWithinRealBound` with the factor in both left slots (its
`leftEps = 0`), so the vanishing `|rightValue| * 0` leg drops exactly. -/
...
`value |-> factor * value` is globally uniformly continuous for every real
factor, the bound witness discharged from the factor's own canonical magnitude
ceiling.  With `factorBoundPredecessor = |x_0| + 1`, `factorBoundPredecessor + 1`
is definitionally `canonicalBoundNumerator factor = |x_0| + 2`, and
`approximationIsWithinCanonicalBound` unifies against the required magnitude
shape. -/
...
/-- The scaled integrand is uniformly continuous (general modulus composition).
If `function` is uniformly continuous at `modulus`, then
...
outputPrecision)` — the Lipschitz modulus of the composite, not `modulus`.  The
inner uniform continuity shrinks the input enough for the factor-collapse to land
the product on `1/(outputPrecision+1)`. -/
