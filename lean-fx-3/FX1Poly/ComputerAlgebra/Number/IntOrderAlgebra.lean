...
Init's `Int.le_total`, `Int.add_le_add_left`, `Int.mul_nonneg`, and `Int.mul_pos` leak
`propext`; this module hand-rolls the order/algebra interaction over the additive-witness
interface:
  ...
  * The three sign-case `≤` builders (`intOfNatLeOfNat`, `intNegSuccLeOfNat`,
    ...
    additive-witness intro form; the negSucc/negSucc case lands on
    `intSubNatNatRightSurplus`, and negSucc-below-ofNat is one `NonNeg` constructor.
  * `intLessEqualTotal` — the four-way constructor bash over the clean `Nat.le_total`.
  * Add-monotonicity (`intAddLeAddLeft`, `intAddLeAddRight`) — the witness is unchanged by
    translation, up to one associativity step.
  * `intMulNonNeg` / `intMulPos` — destruct both operands to `ofNat` carriers; the product
    witness is Nat multiplication, with `intMulPos` peeling the explicit `1 +` successor
    shape through two `Nat.add_comm` shuffles (`x * (n + 1)` and `x + (m + 1)` are
    definitional).

Witness arithmetic (`congrArg`/`Eq.trans`) over the additive-witness kit and clean Nat
facts; free of `axiom`, `sorry`, `propext`, `Quot.sound`, `Classical`, `native_decide`,
...
/-- `≤` between `ofNat`s from the Nat order: `Nat.le.dest`'s witness is the Int witness
(`ofNat` addition is definitional). -/
...
/-- Totality of `≤` — four-way constructor bash over the clean `Nat.le_total`. -/
...
/-- A nonnegative Int is an `ofNat` — additive-witness destruction with the `0 +`
collapsed. -/
...
/-- Adding on the left preserves `≤` — the witness is unchanged, re-parenthesized by one
associativity step. -/
...
/-- Products of nonnegatives are nonnegative — both factors are `ofNat` carriers, so the
product witness is the Nat product. -/
...
/-- Products of positives are positive. Both factors destruct to explicit successors
`ofNat (1 + witness)`; the product's own `1 + _` shape is peeled by two `Nat.add_comm`
shuffles (`x * (n + 1)` and `x + (m + 1)` are definitional). -/
...
/-! ## The strict-order kit

Every `<` fact rides the definitional `a < b ≡ a + 1 ≤ b` pin: endpoint rewrites shift the
`+ 1` by one `congrArg`, strict addition-monotonicity re-parenthesizes the weak one, and
the mixed transitivities thread the successor through `intLessEqualTrans`. -/
...
/-- Irreflexivity of `<` — destruct the witness of `value + 1 ≤ value`: left-cancelling
`value` forces `0 = gap + 1`, a constructor disagreement. -/
