...
IEEE "infinitely precise then round" never needs the reals,
...
The file provides the structure, the cross-alignment relation (reflexive + symmetric +
...
via the pumping lemmas at the floor-of-floors common scale), and the semantic theorem
...
cancellation helpers (`intAddNegSwapCancel` / `intSubSubSelfCancel`) live here; the
clamped-gap floor lemma `intGapFloorSymm` lives in `Number/IntGapArithmetic` and the
`toNat` computation pins in `Number/IntToNatCycle`, alongside the rest of the `Number/`
Int kit.

## Zero-axiom

Structure projections + `congrArg`/`Eq.trans` chains over the `Number/` Int kit and
...
/-- Rescaling is value-preserving — the semantic theorem of the carrier.
...
`natAbs`-sized fuel is a separate theorem, not this definition's claim. -/
...
/-- Every value has a normalized representative: run the
...
states its bounds in this order. -/
...
/-- Toward-zero rounding is monotone in the cross-aligned order.  Pump both operands
down to the two-step clamped-gap floor of
...
/-! ## Directed rounding — the floor and ceiling modes
...
cross-aligned mantissas.  Its payoff: the directed modes land within one ulp.
