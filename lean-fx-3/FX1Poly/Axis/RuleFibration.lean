/-! # FX1Poly/Axis/RuleFibration — the maximally generic rule fibration substrate
...
This file is `Init`-only and Axis-low (no Core dependency), so Core and the cell substrate may
