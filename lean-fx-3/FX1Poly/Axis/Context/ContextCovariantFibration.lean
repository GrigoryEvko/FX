...
## What is deferred (the honest wall)
  ...
  * The classification + cocartesian universal property here are the SELF-CONTAINED Axis analog, NOT a Core
...
`DirectedCategoryHom` substrate).  A Axis design-lock must not import up into `Core/` / `Typed/`.  Zero
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Axis analog of a Core table-native directed-Grothendieck
row; a Axis design-lock cannot import the Core engine, so the classification and the cocartesian universal
property are re-shipped here as strict (1-categorical) constructions.  Gluing the Axis and Core forms is
