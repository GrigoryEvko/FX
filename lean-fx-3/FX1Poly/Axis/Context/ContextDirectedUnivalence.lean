...
## Honesty
  ...
  * The reduction is the SELF-CONTAINED Axis analog of the Core table-native directed-univalence row; it is NOT
    over `StepOverTable` (`fxContextDirectedUnivalence_isOverCoreIotaTable = false`).  The two agree in shape
    (drop the universe scrutinee, size-decreasing, decidable conversion); the Core/Axis gluing is cross-axis
    `×type`.
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Axis analog of the Core table-native directed-univalence
row (over `StepOverTable`, with generic orthogonal-table confluence).  A Axis design-lock cannot import the
Core engine, so the reduction is re-shipped here as a structural normalizer; the two agree in shape (drop the
universe scrutinee, size-decreasing, decidable conversion).  Gluing the Axis and Core forms is cross-axis
`×type`.  `= false`. -/
