...
mode-relative metatheory (Gratzer: `Conv`-decidability = mode-decidability) — needs `Axis/ModeOmega`
...
/-! ## The modal comonad `◐_μ⟨μ|−⟩` generated by the adjunction

Dually to the monad, every adjunction `◐_μ ⊣ ⟨μ|−⟩` generates a COMONAD — the modal comonad
`D = ◐_μ⟨μ|−⟩` (in diagrammatic order `D = rightAdjoint.compose leftAdjoint`), the `□`-NECESSITY
structure of MTT.  Its counit is the adjunction's `counit` `ε : D ⇒ id` (already shipped) and its
comultiplication `δ` is the lock `◐_μ` applied to the unit.  The three comonad laws are the EXACT
duals of the monad laws — they fall out of the same two triangle identities and the unit's naturality
square — so the lock-DRA adjunction carries both a monad (the `◇`-possibility) and a comonad (the
`□`-necessity) on the context category.  Stated componentwise (funext-free). -/

/-- The **comultiplication** `δ : D ⇒ D∘D` of the modal comonad `D = ◐_μ⟨μ|−⟩` — at each object the
lock `◐_μ` applied to the unit at `⟨μ|a⟩`.  (`D = rightAdjoint.compose leftAdjoint`; `D∘D` collapses
by lock composition.)  Naturality is the unit's naturality, wrapped through the lock's functoriality.
The dual of `modalMultiplication`. -/
def IsEndoAdjunction.modalComultiplication {category : RawCategory.{u, v}}
    {leftAdjoint rightAdjoint : RawEndofunctor category}
    (adjunction : IsEndoAdjunction leftAdjoint rightAdjoint) :
    RawEndofunctorTransformation (rightAdjoint.compose leftAdjoint)
      ((rightAdjoint.compose leftAdjoint).compose (rightAdjoint.compose leftAdjoint)) where
  component := fun object =>
    leftAdjoint.mapMorphism (adjunction.unit.component (rightAdjoint.mapObject object))
  naturality := fun {objectA objectB} morphism => by
    show category.compose
          (leftAdjoint.mapMorphism (rightAdjoint.mapMorphism morphism))
          (leftAdjoint.mapMorphism (adjunction.unit.component (rightAdjoint.mapObject objectB)))
        = category.compose
          (leftAdjoint.mapMorphism (adjunction.unit.component (rightAdjoint.mapObject objectA)))
          (leftAdjoint.mapMorphism (rightAdjoint.mapMorphism
            (leftAdjoint.mapMorphism (rightAdjoint.mapMorphism morphism))))
    rw [← leftAdjoint.preservesComposition, ← leftAdjoint.preservesComposition]
    exact congrArg leftAdjoint.mapMorphism
      (adjunction.unit.naturality (rightAdjoint.mapMorphism morphism))

/-- **Modal comonad left-counit law** (componentwise): `εD ∘ δ = id_D`.  IS the LEFT triangle identity
read at `⟨μ|a⟩` — the dual of `modalMonad_leftUnit`. -/
theorem IsEndoAdjunction.modalComonad_counitLeft {category : RawCategory.{u, v}}
    {leftAdjoint rightAdjoint : RawEndofunctor category}
    (adjunction : IsEndoAdjunction leftAdjoint rightAdjoint) (object : category.Object) :
    category.compose (adjunction.modalComultiplication.component object)
        (adjunction.counit.component ((rightAdjoint.compose leftAdjoint).mapObject object))
      = category.identity ((rightAdjoint.compose leftAdjoint).mapObject object) :=
  adjunction.unit_counit_left_triangle (rightAdjoint.mapObject object)

/-- **Modal comonad right-counit law** (componentwise): `Dε ∘ δ = id_D`.  IS `◐_μ` applied to the
RIGHT triangle identity — the dual of `modalMonad_rightUnit`. -/
theorem IsEndoAdjunction.modalComonad_counitRight {category : RawCategory.{u, v}}
    {leftAdjoint rightAdjoint : RawEndofunctor category}
    (adjunction : IsEndoAdjunction leftAdjoint rightAdjoint) (object : category.Object) :
    category.compose (adjunction.modalComultiplication.component object)
        ((rightAdjoint.compose leftAdjoint).mapMorphism (adjunction.counit.component object))
      = category.identity ((rightAdjoint.compose leftAdjoint).mapObject object) := by
  show category.compose
        (leftAdjoint.mapMorphism (adjunction.unit.component (rightAdjoint.mapObject object)))
        (leftAdjoint.mapMorphism (rightAdjoint.mapMorphism (adjunction.counit.component object)))
      = category.identity (leftAdjoint.mapObject (rightAdjoint.mapObject object))
  rw [← leftAdjoint.preservesComposition, adjunction.unit_counit_right_triangle]
  exact leftAdjoint.preservesIdentity (rightAdjoint.mapObject object)

/-- **Modal comonad coassociativity law** (componentwise): `Dδ ∘ δ = δD ∘ δ`.  Both sides are `◐_μ`
applied to the unit's naturality AT the unit — the dual of `modalMonad_assoc`, a direct corollary of
`unit`'s naturality square. -/
theorem IsEndoAdjunction.modalComonad_coassoc {category : RawCategory.{u, v}}
    {leftAdjoint rightAdjoint : RawEndofunctor category}
    (adjunction : IsEndoAdjunction leftAdjoint rightAdjoint) (object : category.Object) :
    category.compose (adjunction.modalComultiplication.component object)
        ((rightAdjoint.compose leftAdjoint).mapMorphism
          (adjunction.modalComultiplication.component object))
      = category.compose (adjunction.modalComultiplication.component object)
        (adjunction.modalComultiplication.component
          ((rightAdjoint.compose leftAdjoint).mapObject object)) := by
  show category.compose
        (leftAdjoint.mapMorphism (adjunction.unit.component (rightAdjoint.mapObject object)))
        (leftAdjoint.mapMorphism (rightAdjoint.mapMorphism
          (leftAdjoint.mapMorphism (adjunction.unit.component (rightAdjoint.mapObject object)))))
      = category.compose
        (leftAdjoint.mapMorphism (adjunction.unit.component (rightAdjoint.mapObject object)))
        (leftAdjoint.mapMorphism (adjunction.unit.component
          (rightAdjoint.mapObject (leftAdjoint.mapObject (rightAdjoint.mapObject object)))))
  rw [← leftAdjoint.preservesComposition, ← leftAdjoint.preservesComposition]
  exact congrArg leftAdjoint.mapMorphism
    (adjunction.unit.naturality (adjunction.unit.component (rightAdjoint.mapObject object))).symm
