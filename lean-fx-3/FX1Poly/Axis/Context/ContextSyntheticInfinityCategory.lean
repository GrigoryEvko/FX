...
## What is deferred (the honest wall)
  ...
  * The Segal + Rezk structure here is the SELF-CONTAINED Axis analog, NOT a Core `StepOverTable` iota row
...
`composeHom` / `discreteUniverseObject` substrate).  A Axis design-lock must not import up into `Core/` /
...
/-- **Honesty marker.**  This is the SELF-CONTAINED Axis analog of a Core table-native synthetic-∞-category
row; a Axis design-lock cannot import the Core engine, so the Segal + Rezk structure is re-shipped here as a
strict (1-categorical) construction.  Gluing the Axis and Core forms is cross-axis `×type`.  `= false`. -/
