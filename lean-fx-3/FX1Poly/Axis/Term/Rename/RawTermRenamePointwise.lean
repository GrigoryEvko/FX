...
`FX1Poly.Axis.Syntax.RawRenaming` namespace) to keep the namespaces
...
def RawRenaming.PointwiseEq {sourceScope targetScope : Nat}
    (firstRenaming secondRenaming :
        FX1Poly.Axis.Syntax.RawRenaming sourceScope targetScope) : Prop :=
...
theorem RawRenaming.lift_pointwise {sourceScope targetScope : Nat}
    {firstRenaming secondRenaming :
        FX1Poly.Axis.Syntax.RawRenaming sourceScope targetScope}
    (renamingEq :
...
theorem RawTerm.rename_pointwise_var_smoke
    {sourceScope targetScope : Nat}
    {firstRenaming secondRenaming :
        FX1Poly.Axis.Syntax.RawRenaming (sourceScope + 1) (targetScope + 1)}
    (renamingEq :
        RawRenaming.PointwiseEq firstRenaming secondRenaming) :
    RawTerm.rename firstRenaming
        (.mkGen .gen_var
