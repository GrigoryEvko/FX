...
theorem RawRenaming.lift_compose_pointwise
    ...
    (firstRenaming : FX1Poly.Axis.Syntax.RawRenaming sourceScope middleScope)
    (secondRenaming : FX1Poly.Axis.Syntax.RawRenaming middleScope targetScope) :
    RawRenaming.PointwiseEq
        (FX1Poly.Axis.Syntax.RawRenaming.compose firstRenaming secondRenaming).lift
        (FX1Poly.Axis.Syntax.RawRenaming.compose firstRenaming.lift secondRenaming.lift) := by
  intro position
  match position with
  | ⟨0, _⟩ => rfl
...
theorem iterateLiftRaw_RawRenaming_compose_pointwise
    ...
    RawRenaming.PointwiseEq
        (iterateLiftRaw
            (FX1Poly.Axis.Syntax.RawRenaming.compose firstRenaming secondRenaming)
            binderDepth)
        (FX1Poly.Axis.Syntax.RawRenaming.compose
...
theorem RawRenaming.lift_compose_pointwise_zero_smoke
    ...
    (FX1Poly.Axis.Syntax.RawRenaming.compose firstRenaming secondRenaming).lift
      ...
      FX1Poly.Axis.Syntax.RawRenaming.compose firstRenaming.lift secondRenaming.lift
        (⟨0, Nat.zero_lt_succ sourceScope⟩ : Fin (sourceScope + 1)) :=
...
theorem iterateLiftRaw_RawRenaming_compose_pointwise_zero_smoke
    ...
    iterateLiftRaw
        (FX1Poly.Axis.Syntax.RawRenaming.compose firstRenaming secondRenaming) 0
        position =
      FX1Poly.Axis.Syntax.RawRenaming.compose
