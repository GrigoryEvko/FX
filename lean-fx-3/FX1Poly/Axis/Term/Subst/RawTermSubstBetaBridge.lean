...
/-! # Axis/Term — the term-native β-substitution bridge (`term-beta` re-home)

`context-9` (`Axis/Context/SubstitutionFree.lean`) surfaces a `×term` bridge corollary —
...
`FX1PolyAudit/AuditAxisTermAxis.lean`.
-/
