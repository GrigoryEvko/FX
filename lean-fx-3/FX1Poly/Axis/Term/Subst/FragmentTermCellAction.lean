...
/-! # Axis/Term/Subst/FragmentTermCellAction — the fragment term-to-cell ACTION
...
carrying the kernel `RawTerm.subst` (Axis) to `pasteAlong` (Polygraph) with the SHARED `(t, sigma)`
...
it needs the kernel `RawTerm.subst` (Axis) AND the Steiner `linearizeFull` (Polygraph), and Polygraph may
not import Axis.  The declarations keep the `FX1Poly.Polygraph.Omega` namespace for name stability
...
The genuine kernel `RawTerm.subst` (Axis) fires on the fragment: substituting a depth-`k` tower by the
