...
/-! # Axis/Term/Subst/SubstPasting — substitution composition IS pasting arithmetic (OMEGA-7 r1, B1)

The file lives on the Axis side of the layer DAG (Init -> ComputerAlgebra -> Polygraph -> Axis -> Core)
because the identification NEEDS both the kernel substitution engine (Axis) and the Steiner pasting
arithmetic (Polygraph), and Polygraph files may not import Axis.  The declarations keep the
...
s2)` (`Axis/Term/Subst/RawTermSubstCompose.lean`, the file's own thesis "μ_X ∘ μ_X = μ_X ∘ T_μ_X") — as the
