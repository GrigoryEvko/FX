...
theorem RawRenaming.weaken_lift_commute
    ...
    ∀ position : Fin sourceScope,
      rawRenaming.lift (FX1Poly.Axis.Syntax.RawRenaming.weaken position) =
        FX1Poly.Axis.Syntax.RawRenaming.weaken (rawRenaming position) :=
  fun _ => rfl
...
@[reducible] def RawTermSubst.postRename
    ...
    (rawRenaming : FX1Poly.Axis.Syntax.RawRenaming middleScope targetScope) :
...
theorem RawTermSubst.lift_then_rename_lift_pull
  ...
  | ⟨priorPositionValue + 1, hBound⟩ =>
      ...
      rw [RawTerm.rename_compose FX1Poly.Axis.Syntax.RawRenaming.weaken
            rawRenaming.lift
            (someSubstitution ⟨priorPositionValue, Nat.lt_of_succ_lt_succ hBound⟩),
          RawTerm.rename_compose rawRenaming
            FX1Poly.Axis.Syntax.RawRenaming.weaken
            (someSubstitution ⟨priorPositionValue, Nat.lt_of_succ_lt_succ hBound⟩)]
      -- Goal:
