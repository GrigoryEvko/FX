...
The full `FX1Poly.Axis.Syntax.Action RawTermSubst` typeclass instance,
...
-/
instance instActionRawTermSubst : FX1Poly.Axis.Syntax.Action RawTermSubst where
...
theorem RawTermSubst.identity_eq_action {scope : Nat} :
    (RawTermSubst.identity : RawTermSubst scope scope) =
      (FX1Poly.Axis.Syntax.Action.identity : RawTermSubst scope scope) := rfl
...
theorem RawTermSubst.lift_eq_actionForTy
    ...
    someSubstitution.lift = FX1Poly.Axis.Syntax.Action.liftForTy someSubstitution := rfl
...
theorem RawTermSubst.lift_eq_actionForRaw
    {sourceScope targetScope : Nat}
    (someSubstitution : RawTermSubst sourceScope targetScope) :
    someSubstitution.lift = FX1Poly.Axis.Syntax.Action.liftForRaw someSubstitution := rfl
...
theorem RawTermSubst.compose_eq_action
    ...
    RawTermSubst.compose firstSubstitution secondSubstitution =
      FX1Poly.Axis.Syntax.Action.compose firstSubstitution secondSubstitution := rfl
...
theorem RawTermSubst.action_identity_headIndex_smoke {scope : Nat}
    (position : Fin scope) :
    FX1Poly.Axis.Syntax.Action.headIndex
        (FX1Poly.Axis.Syntax.Action.identity : RawTermSubst scope scope) position =
...
theorem RawTermSubst.action_compose_identity_left_smoke
    ...
    FX1Poly.Axis.Syntax.Action.composeAtHeadIndex
        (FX1Poly.Axis.Syntax.Action.identity : RawTermSubst (scope + 1) (scope + 1))
        someSubstitution
        (⟨0, Nat.zero_lt_succ scope⟩ : Fin (scope + 1)) =
      FX1Poly.Axis.Syntax.Action.headIndex someSubstitution
        (⟨0, Nat.zero_lt_succ scope⟩ : Fin (scope + 1)) := by
  exact FX1Poly.Axis.Syntax.Action.compose_identity_left_pointwise someSubstitution _
