...
that namespace, so renaming the namespace to `FX1Poly.Axis.GradeAlgebra` is deferred to the later
Core↔Axis dependency-inversion pass, which decouples those consumers holistically.
