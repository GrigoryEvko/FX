...
Lives in `Axis/Mode/` (not `Polygraph/`) although its declarations stay in the
`FX1Poly.Polygraph` namespace: the rung-1 and ceiling pins reference the Axis mode-floor
markers (`FX1Poly.Axis.fxMode_hasDecidable*`), and the layer DAG forbids Polygraph → Axis
...
/-- Rung 1 pin: the free rung matches the Axis free-fragment marker. -/
theorem fxDecidableCeiling_freeRung_matchesMarker :
    fxDecidableCeiling.hasGenericFreeDecision
      = FX1Poly.Axis.fxMode_hasDecidableFreeTwoCellEquality := rfl
...
theorem fxDecidableCeiling_generalMarkerSitsAboveLedger :
    FX1Poly.Axis.fxMode_hasDecidableTwoCellEquality = false := rfl
...
FLAG A — `FX1Poly.Axis.fxMode_hasDecidableTwoCellEquality = false` (`Mode.lean`).
...
FLAG B — `FX1Poly.Axis.fxMode_hasModeRelativeConvDecision = false`
