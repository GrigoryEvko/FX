...
DECIDABLE 2-CELL EQUALITY, independent of any type theory.  It is the Axis deliverable the `fib-*` gluing (and
`Context/ModalLock`, which already names `Axis/ModeOmega`) consumes.
