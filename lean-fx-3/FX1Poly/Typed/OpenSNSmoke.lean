...
/-! # FX1Poly/Typed/OpenSNSmoke
    — OPEN-CONTEXT strong-normalization regression corpus

`ClosedSNSmoke.lean` ships UNCONDITIONAL strong normalization for concrete CLOSED terms (empty
context).  This file is its OPEN analogue: concrete terms living under a genuine NON-EMPTY well-formed context
`Γ = (.empty).cons (Type@e)`, each discharged to `IsStronglyNormalizing` by **open strong normalization**
(`HasTypeDescPi.stronglyNormalizingOfWfContextDesc`) — an end-to-end exercise of the open result on terms that
actually inhabit a non-trivial context.
...
the grown engine:
  ...
  * `openBetaRedex_stronglyNormalizing` — the β-redex `(λ (x : Type@(s+1)). x) (Type@s)` typed by `piElim`
    over the lambda: the NON-VACUOUS entry — a term that actually REDUCES, whose termination open strong
    normalization certifies.  Mirrors `ClosedSNSmoke.closedIdentityApplication`, lifted from the empty context
    into `Γ`.

Each is the one-line composition `HasTypeDescPi.stronglyNormalizingOfWfContextDesc
(wfContextDesc_universeBinding ..) (<grown typing in Γ>)`.  Together they demonstrate open strong
normalization is not vacuous on open terms: the open SN machinery fires through formation, a context lookup, a
λ-binder, and a real β-reduction.  The well-formedness witness is the `HasTypeDesc`-defined `WfContextDesc`
(the native `IsTypeDesc`-based predicate).
...
Each theorem is a single `stronglyNormalizingOfWfContextDesc` composition over a concrete grown-engine
derivation.  No `axiom`, `sorry`, `propext`, `Quot.sound`, `Classical`, `native_decide`, `omega`.
...
the universe code `Type@s` (typed by the universe-formation rule lifted into the grown engine) is strongly
normalizing — discharged by open strong normalization (`stronglyNormalizingOfWfContextDesc`).  The open
...
open strong normalization.  The entry that genuinely consumes the context binding — a regression that the
...
at `Γ.cons (Type@(s+1))`, scope 2) is strongly normalizing via open strong normalization.  Exercises the
...
`Type@s : Type@(s+1)`) is strongly normalizing via open strong normalization.  Unlike the other three
entries (all normal forms), this term actually REDUCES — it is a real β-redex — so open strong normalization
here certifies the TERMINATION of a non-trivial open reduction, proving the corpus is not vacuous.  Mirrors
