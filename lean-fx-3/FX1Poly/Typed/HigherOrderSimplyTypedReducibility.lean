...
**OFF-PATH CROSSCHECK: ROUTE-A all-level fragment (extends `FirstOrderSimplyTypedReducibility`'s
member-extension to higher-order domains).  Off the kernel critical path — the bounded route reaches strong
normalization without the all-level member-extension principle.  Retained as a crosscheck.**

`FirstOrderSimplyTypedReducibility` establishes the universe member-extension principle for the FIRST-ORDER
