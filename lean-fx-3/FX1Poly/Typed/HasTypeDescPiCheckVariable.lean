...
reduces to deciding `Conv (context.lookup index) targetType`:
  ...
  * the principal type's own typehood (`lookup index : Type`) — the COMPARE step needs it as DATA, so it is
    extracted via the NATIVE `IsTypeDesc.decideWithWitness` (which returns a `PSum` carrying the typing
...
The lookup-typehood is now decided by the native `IsTypeDesc.decideWithWitness` (HT-A4 B1) over a
...
A `match` on `IsTypeDesc.decideWithWitness` feeding the shipped COMPARE step + variable inversion; the `.inr`
...
`IsTypeDesc.decideWithWitness`, its impossible non-type branch refuted by `WfContextDesc.lookupIsTypeDesc`. -/
def HasTypeDescPi.decidableCheckVariableAtType {profile : PolyProfile} {scope : Nat}
  ...
  match IsTypeDesc.decideWithWitness (WfContextDesc.ofWfContext wellFormed) (context.lookup index) with
