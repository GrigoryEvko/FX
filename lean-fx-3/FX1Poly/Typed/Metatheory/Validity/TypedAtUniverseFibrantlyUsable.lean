...
/-- **★ The classifier-generalized typed-implies-fibrantly-usable bridge.**  Generalizes
`typedAtUniverseImpliesFibrantlyUsable_ofLocksInterval` off the universe code to ANY classifier `classifier`
that is NOT `Conv`-related to the locked interval (`¬ Conv intervalTypeCell classifier`).  Same proof: a
`lockCons`-0 variable looks up to `intervalTypeCell` (`lockedLookupIsInterval`), so a subject typed at a
classifier the interval cannot reach is never the locked dimension — it is fibrantly usable.  The single shared
engine the congruence / gate subject-reduction rows discharge their stepped-child usability against (each
supplies the concrete `¬ Conv intervalTypeCell C` for its former via the head-stability refutations). -/
theorem typedAtNonIntervalImpliesFibrantlyUsable_ofLocksInterval {profile : PolyProfile} {scope : Nat}
    {context : TypingContext profile scope} {subject classifier : RawTerm scope}
    (locksInterval : context.AllLocksAreInterval)
    (notInterval : ¬ Conv intervalTypeCell classifier)
