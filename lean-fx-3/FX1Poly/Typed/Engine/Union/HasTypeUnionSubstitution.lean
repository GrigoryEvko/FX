  ...
  | intro context generator rule args params level0 level1 flag isIntro sideHolds premisesHold
      ...
      · match args, params with
        | .childCons body .childNil, .childCons carrierCode .childNil =>
          ...
          -- substitution does not INCREASE it at the freshest binder (the lift fixes `var 0` and the
          -- deeper substituents are weakened), so `appScaled (subst body) 0 ≤ appScaled body 0 ≤ one`.
          have binderGradedSubst :
              UsageGrade.le
