...
/-! `lmaxFold`, `lmaxAll`, `TypingRuleDesc`, `universeFormerOutput`, `nullaryFormerOutput`, and
`typingRuleDescOf` are the PURE-SYNTAX formation rule descriptor — they mention no `HasTypeDesc`.
They live in the grown-free `FX1Poly.Typed.Engine.RuleTables.TypingRuleSpec` (imported above), so the
union's formation rule-table layer reaches them without importing this grown engine. -/
...
/-! The pure-`rfl` per-generator metadata lemmas `typingRuleDescOf_piTyCode` / `_sigmaTyCode` /
`_listCode` / `_optionCode` / `_unitCode`, and the non-variable witness
`formationRuleImpliesNotVariable`, are pure-syntax facts about `typingRuleDescOf`; they live in the
grown-free `TypingRuleSpec` module (imported above). -/
