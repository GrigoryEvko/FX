...
/-! # FX1Poly/Typed/BoundedNeutralMember
    — a variable is a bound-reducible member of any bound-reducible type (toward open strong normalization)

The member-side LEAF of the neutral/identity environment that extends the closed route
(`HasTypeDescPi.closedStronglyNormalizing`) to arbitrary contexts.
...
`noStep_var` refutes every alleged reduct.  No fuel-stability, no renaming-closure — just CR3 at a
no-reduct neutral.
...
vacuous (`noStep_var`).  The member leaf the neutral/identity environment cons-feeds at every context
position. -/
