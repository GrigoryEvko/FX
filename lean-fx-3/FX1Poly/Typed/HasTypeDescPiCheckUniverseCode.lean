...
/-! # FX1Poly/Typed/HasTypeDescPiCheckUniverseCode
    — the UNIVERSE-CODE case of the bidirectional grown-engine checker, in CHECK mode

The UNIVERSE-CODE case of the decidable checker (alongside the variable case,
`HasTypeDescPiCheckVariable`), the simpler of the two leaf positions.  Deciding
...
targetType`:
  ...
  * the principal type's OWN typehood is *also* direct — `Type@(level+1, flag)` is typed at `Type@(level+2,
    flag)` by a second `universeFormation`, so (unlike the variable case) NO `IsTypeDesc` typehood decision is
...
Two `universeFormation` constructors (inference + its principal type's typing) feeding the COMPARE step
+ universe-code inversion.  No `match`, no typehood decision, no `axiom`, `sorry`, `propext`, `Quot.sound`,
...
no `IsTypeDesc` typehood decision is required.  The target's typehood `targetTyped` is threaded as input (CHECK
mode). -/
