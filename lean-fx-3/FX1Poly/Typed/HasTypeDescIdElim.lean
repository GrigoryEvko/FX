...
ι-COMPUTATION.  The substrate's `gen_idJ` is the SIMPLIFIED two-child J (`idJ(baseCase, witness)`, no explicit
motive child — the motive lives in the PROFILE layer, per `Step.lean`), so on a `refl` witness its ι SELECTS the
base case:

  * `idJ(baseCase, refl(x)) ↝ baseCase`  (`Step.iotaIdJRefl`)
...
judgment), consuming `HasTypeDescIdIntro` (DI-2d) for the witness premise.

  * `idJCell` — the `gen_idJ` cell `idJ(baseCase, witness)` (arity 2, `[0, 0]`).
  * `HasTypeDescIdElim` — the judgment: `idJ(baseCase, witness) : C` from a witness typed at a reflexive identity
    type `Id(A, x, x)` (by the id-intro engine — so it is a `refl`) and a base case `baseCase : C` (by the grown
    engine).  The non-dependent J (result type = the base case's type).
...
/-- The identity eliminator cell `idJ(baseCase, witness)` — `gen_idJ` (arity 2, `binderShifts = [0, 0]`; the
motive is a profile-layer concern, not a substrate child). -/
def idJCell {scope : Nat} (baseCase witness : RawTerm scope) : RawTerm scope :=
  .mkGen .gen_idJ () (.childCons baseCase (.childCons witness .childNil))

/-- **The identity eliminator judgment.**  A standalone layer typing the simplified `idJ`:
`idJ(baseCase, witness) : C` when the witness is typed at a reflexive identity type `Id(A, x, x)` (by the
id-intro engine — so it is a `refl`) and the base case is `baseCase : C` (by the grown engine).  The result type
is the base case's type (the substrate's J is non-dependent; the motive is profile-layer). -/
inductive HasTypeDescIdElim (profile : PolyProfile) :
  ...
  | idJIntro {scope : Nat} (context : TypingContext profile scope)
      ...
      HasTypeDescIdElim profile context (idJCell baseCase witness) resultType
...
theorem HasTypeDescIdElim.idJOfUniverseCodesTyped {profile : PolyProfile} (flag : UniverseFlag) :
    HasTypeDescIdElim profile (TypingContext.empty : TypingContext profile 0)
      (idJCell (universeCodeCell LevelExpr.lzero flag) (reflCell (universeCodeCell LevelExpr.lzero flag)))
...
`idJ(baseCase, witness)`.  Free-index single-arm `cases`. -/
theorem HasTypeDescIdElim.subjectIsIdJ {profile : PolyProfile} {scope : Nat}
    ...
    ∃ (baseCase witness : RawTerm scope), subject = idJCell baseCase witness := by
  cases derivation with
  | idJIntro baseCase witness _typeCode _endpoint _resultType _witnessTyped _baseCaseTyped =>
...
theorem idJReflIotaComputesTyped {profile : PolyProfile} {scope : Nat}
    ...
    (baseCaseTyped : HasTypeDescPi profile context baseCase resultType) :
    HasTypeDescIdElim profile context (idJCell baseCase (reflCell witness)) resultType ∧
    Step (idJCell baseCase (reflCell witness)) baseCase ∧
    HasTypeDescPi profile context baseCase resultType := by
  refine ⟨?_, Step.iotaIdJRefl, baseCaseTyped⟩
  exact HasTypeDescIdElim.idJIntro context baseCase (reflCell witness) typeCode witness resultType
