...
NATIVE-08 (`BridgeEndpointNativeSubjectReduction`) closed the endpoint-β subject reduction on the two
SHIPPED fragments — the dimension-constant body (reduct GROWN-typed via `subst0_weaken`) and the
identity-path body at a bare endpoint (reduct DATA-INTRO-typed) — and recorded the remaining residual: a
GENERAL body's endpoint-β reduct `body[ε]` needs the grown single-substitution lemma to be typed.

This is that residual.  `HasTypeDescPi.substituteUnderBinding` (the grown β-engine) transports a derivation
under a binder along a grown-typed argument.  The endpoint-β redex inversion already gives the body
GROWN-typed under the interval binder at the redex's own (weakened) classifier; supplying a GROWN-typed
interval argument lets the grown substitution lemma fire, and `subst0_weaken` collapses the substituted
classifier back to the original — so the reduct is GROWN-typed at the original classifier, for an ARBITRARY
body (not just the constant or identity-path special cases).
...
This completes the wall-falls picture by classifying the endpoint-β reduct by ARGUMENT:
  ...
  * argument a bare endpoint `interval0`/`interval1` → reduct DATA-INTRO-typed (NATIVE-08, the wall case).
...
## What ships
  ...
  * **`endpointBetaGeneralArgumentNativelyTyped`** — the corollary into the NATIVE-08 combined predicate:
    the same reduct is `EndpointBetaReductNativelyTyped` via the `ofGrown` disjunct.  So the constant
    fragment (NATIVE-08), the identity-path-at-endpoint fragment (NATIVE-08), AND every grown-argument
    fragment (this file) all speak the ONE preservation predicate.
  * **`endpointBetaConstantBodySubsumed`** — the headline SUBSUMES NATIVE-08's constant fragment: a
    constant body `weaken constantBody` is one instance of the general body, recovering the
    reflexivity-bridge reduct typing through the SAME grown substitution lemma.
...
The substitution-transport residual NATIVE-08 deferred — now discharged for every body shape. -/
...
NATIVE-08's `EndpointBetaReductNativelyTyped` (the `ofGrown` disjunct): the constant fragment, the
...
/-- **The general theorem subsumes NATIVE-08's constant fragment.**  A dimension-constant body
...
path applied to a bare ENDPOINT (NATIVE-08) reaches only the data-intro engine.  The endpoint-β reduct's
