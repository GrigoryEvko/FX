...
from `HasTypeDesc.subjectStronglyNormalizing` (the formation subject is non-stepping by its own
...
normalizing.  Delegates to `HasTypeDesc.subjectStronglyNormalizing`, proved directly on the formation
engine's structure. -/
theorem HasTypeDesc.isStronglyNormalizing {profile : PolyProfile} {scope : Nat}
  ...
  typed.subjectStronglyNormalizing

/-- A description-engine type is strongly normalizing.  The `IsTypeDesc` witness is a
`HasTypeDesc` derivation whose SUBJECT is the classifier, so `HasTypeDesc.subjectStronglyNormalizing`
...
theorem IsTypeDesc.isStronglyNormalizing {profile : PolyProfile} {scope : Nat}
  ...
  exact classifierTyped.subjectStronglyNormalizing
