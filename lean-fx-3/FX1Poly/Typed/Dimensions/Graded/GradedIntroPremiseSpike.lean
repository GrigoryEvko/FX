...
the affine path abstraction is now realized ONLY by the uniform `HasTypeUnion.intro` builder at
`pathLamIntroRule` (selected by `introRuleOf` at `gen_pathLam`), whose `sideCondition` is this same
