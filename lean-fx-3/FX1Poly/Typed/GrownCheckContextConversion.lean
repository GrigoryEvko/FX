...
/-! # FX1Poly/Typed/GrownCheckContextConversion — GrownCheck structural helpers

The `GrownCheck` relation's structural toolbox for the strengthening campaign: context conversion
under pointwise-`Conv` contexts (with the Conv-related-binders cons condition and the binder-swap
corollary — the reflection's swap-the-floating-binder ingredient), the target-side Π EXPOSURE, and
the λ/app soundness REASSEMBLY shapes.

  * `convContextCondition_consConv` — the context-condition extends under CONV-RELATED bindings
    (generalizes `convContextCondition_cons`, which fixes the same binding on both sides).
  * `GrownCheck.convContext` / `GrownCheckTelescope.convContext` — a check survives replacing the
    context by a pointwise-`Conv`-related one, at the SAME target.  Because the relation is raw
    (no typehood premises) the proof is purely structural: the var leaf absorbs the lookup change
    by `Conv.trans`, every other arm passes through, binders extend the condition via
    `convContextCondition_cons`.  Contrast `HasTypeDescPi.convContextUnderWf` (#1133), which needs
    target well-formedness and produces only a `Conv`-equal classifier — the relation's exactness
    here is the payoff of keeping `Conv` at the leaves.
  * `GrownCheck.convBinder` — the corollary the β-redex reflection case consumes: a check under a
    consed binder survives replacing that binder by a `Conv`-equal one.
  * `HasTypeDescPi.piTargetExposure` — a typed target `Conv` a Π-code `StepStar`-reduces to a
    Π-code whose components are `Conv` the originals AND carry their own typings
    (`Conv.reducesToPiTyCode` + `subjectReductionStar` + `invertPiTyCode`; wf-conditional through
    SR).  The λ-arm soundness ingredient, also applied at the app arm's function Π target.
  * `GrownCheck.lamSoundGivenBodyTyped` / `appSoundGivenComponentsTyped` — the engine-side
    reassembly shapes: `piIntro`/`piElim` + the grown `conv` through the relation's leaf.  The
    STR-5 soundness induction discharges its λ/app cases by `piTargetExposure` → transport the
    body check (`convBinder` + `absorbConv`) → IH → these reassemblies.

## Zero-axiom verification

The mutual pair is structural recursion mirroring `HasTypeDescPi.convContextUnderWf`'s idiom;
`piTargetExposure` composes three shipped zero-axiom results; the reassemblies are single
rule-applications.  No `axiom`, `sorry`, `propext`, `Quot.sound`, `Classical`, `native_decide`,
