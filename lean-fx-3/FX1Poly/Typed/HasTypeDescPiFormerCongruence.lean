...
results (`invertPiTyCode`/`invertSigmaTyCode`, `classifierIsTypeDescPi`, the `conv` rule).  No `axiom`,
...
`piFormationViaGenArm` rebuilds at `Type@(lmax …)`; `classifierIsTypeDescPi` + `conv` returns to
