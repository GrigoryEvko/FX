...
`HasTypeDesc.classifierIsTypeDesc` (`WfContextDescValidity.lean`), then the type-level SN projection
...
`classifierIsTypeDesc` (formation validity) ∘ `IsTypeDesc.isStronglyNormalizing` (the
...
Native validity (`classifierIsTypeDesc`) turns the classifier into an `IsTypeDesc`, then
