...
The Π/Σ-formation rules are level-tight for the grown engine `HasTypeDescPi`, the companion to
`GrownUniverseFormationStrictness` (grown universe strictness) — together they make the grown engine's whole
formation family (universe + Π + Σ) level-strict.

`HasTypeDescPi.invertPiTyCode` / `invertSigmaTyCode` EXTRACT the component levels existentially and report the
classifier as `Conv`-equal to `Type@(lmaxAll [domainLevel, codomainLevel])`.  Since `lmaxAll [a, b]` reduces
definitionally to `LevelExpr.lmax a b` (an `lmax`-headed level, never `lzero`), a grown Π/Σ-type code is NEVER
classified by the bottom universe `Type@0` — in ANY context, with ANY components: the rejection discards the
component typings entirely and refutes the level equation structurally.
...
(`@[cumulUpMarker]`), the statement becomes a `≤`-bounded floor.
