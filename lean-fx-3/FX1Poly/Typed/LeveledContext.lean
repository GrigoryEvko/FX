...
/-- **Every entry of a leveled context is a universe-classified type — in the FULL context.**  By the
leveled-context recursor (clean, like `allLevelsPositive`); the per-position `Fin` split is a propext-clean
`⟨0,_⟩` / `⟨k+1,_⟩` structure match.  The head entry's prefix typing (`bindingTyped`) and each tail entry's
typing (the recursor IH) are weakened into the full context by `HasTypeDescPi.weakenUnderBinding`; the
classifier `universeCodeCell ..` is rename-invariant (`rename_universeCodeCell`, rfl), and the lookup is
unfolded by `lookup_cons_zero` / `lookup_cons_succ` (both rfl) to exactly the weakened term.

This is the substrate the term-FT recursor's `var` and `conv` arms read: it supplies the
`context.lookup index = universeCodeCell ..` evidence (here as an existential over the level/flag) that the
`conv`-with-pinned-reclassifier bridge arm (`validTypingBridgeConvPinnedReclassifier`) and the var arm
require to classify each looked-up context variable. -/
