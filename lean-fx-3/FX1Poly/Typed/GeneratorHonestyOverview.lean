...
honest semantic scope of the 197-generator table — how many generators are statically typed, operationally
reducing, semantically live (either), and how many are genuinely RESERVED (bare names, no encoded meaning).  It
keeps the gap visible — and the conspicuous "RESERVED 159" line in front of us — so the honesty work does not
quietly stall.

  * `allGenerators` — all 197 generators, enumerated via the total tag-inverse `Generator.fromTag` over the tag
    range `0..196` (`(List.range 197).filterMap Generator.fromTag`).
...
over the zero-axiom classifiers and are themselves zero-axiom.  (A machine-checked `… = 159` count theorem is
HON-9 / the count pin — deferred, as kernel-reducing a 197-element filter is heavyweight; the `#eval` reports the
count by compiled execution.)
...
-- ★ The build-time honesty overview.  Prints the live/reserved scope of the 197-generator table on every build
