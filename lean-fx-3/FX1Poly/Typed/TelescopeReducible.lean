...
/-- **A `listCode` former's children spine is `consecutiveShifts`-indexed.**  The one-child data-former twin of
`gen_piTyCode_binderShifts_eq`: `gen_listCode.binderShifts = [0] = consecutiveShifts 0 1` definitionally — the
bridge the `genFormationPi` arm's `gen_listCode` branch uses to feed the list former's one-child telescope into
the relation. -/
