...
/-! # FX1Poly/Typed/BoundedBindingTypeReducible
    — a universe-typed subject is bound-reducible-as-type under a reducible env
...
cons-feeds at every context position — the companion to `IsReducibleMemberAtBounded.ofVariable`
(the variable-membership leaf).  Together they discharge the `OpenStronglyNormalizing.lean` residual toward
unconditional open strong normalization.

## The five-step assembly

  1. `HasTypeDescPi.fundamentalAtBoundedSucc` on the derivation + budget + env gives a bound-reducible
  ...
  3. `belowBound_of_reducibleUniverse` recovers the gate `denote levelExpr env < bound` from the member's
     universe candidate.
...
the telescope induction, where it belongs.

## Zero-axiom verification

A linear composition of shipped zero-axiom results (FT + decode + cumulativity) with the universe-gate inversion and one
...
bound-reducible-as-type at `bound`.  FT → `subst_universeCodeCell` → below-bound gate → decode → cumulativity.
The type-side leaf the reducible-closing-environment builder (`reducibleEnvOfWfContext`) cons-feeds. -/
