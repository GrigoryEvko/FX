...
/-! # FX1Poly/Typed/GrownPinnedReflection — THE CONDITIONAL GROWN MASTER reflection
     (route-H reflection brick 8)

The full pinned reflection over the grown engine, assembled: mutual induction over
`HasTypeDescPi`/`DescTelescopePi` in the pinned motive (`PinnedReflectionConclusion`), with every
arm discharged by the shipped brick kit EXCEPT piElim, which is the explicit residual hypothesis.

  * `PinnedReflectionPiElimResidual` — THE remaining open core of the strengthening campaign: an
    in-image application with a pinned OUTPUT classifier, given reflections for the premises.  The
    function's Π classifier is the float — the output pin does not pin it.
  * `HasTypeDescPi.retypeAtUniverse` — the grown twin of the formation re-pin move.
  * `pinnedReflectionOfFormationArm` — the formation master reflection is PIN-FREE; wrap in
    `ofFormation` (the pin premises are simply unused).
  * `pinnedReflectionConvArm` — re-pin the premise through the conversion (`converts.trans pinned`,
    SAME pin base, same base typing) and convert the output back.
  * `HasTypeDescPi.pinnedReflectionConditional` / `DescTelescopePi.pinnedReflectionTelescopeConditional`
    — the conditional master: ofFormation/conv/piIntro/genFormationPi all discharged
    (piIntro by `pinnedReflectionPiIntroArm`; the grown telescope leg pins its heads with the
    rename-invariant universe code, typed by `universeFormation`, and re-pins via
    `retypeAtUniverse`); `piElim` consumes the residual with both premise reflections.

Instantiating the residual closes grown strengthening (STR-9 #1178): at `ρ := weaken` with
`ContextReflectsRename.ofWeakenCons`, the master reflects `Γ.cons B ⊢ weaken t : weaken T` to
`Γ ⊢ t : T'` with `Conv (weaken T) (weaken T')`, and `Conv.reflectWeaken` finishes.

## Zero-axiom verification

Every arm is a shipped zero-axiom brick; the assembly adds none.  No `axiom`, `sorry`, `propext`,
