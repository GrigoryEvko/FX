...
`∃ bound, BoundExceeds env bound d` is provable by induction on `d` (CONSTRUCTING the budget, taking `max` of
...
1`.  The FT arms (BFT-11) then INVERT `BoundExceeds (ctor …)` — which needs the index-equality-witness recipe
(`cases` at a specific index fails because `genFormation`'s `rule.outputType` index is opaque).

## Zero-axiom verification

A mutual inductive `Prop` family indexed by `HasTypeDesc` / `DescTelescope` derivations; constructors mirror the
typing constructors, the `universeFormation` constructor carrying the `belowBound` gate.  Inductives introduce no
axioms (checked: depends on no axioms).  Per-declaration gated in `FX1PolyAudit/AuditTyped.lean`.
-/
...
`bound` exceeds the denoted `lsucc`-level of every `universeFormation` leaf in the derivation `d`; conv and
genFormation carry the sub-derivations' budgets by construction.  Mutual with `BoundExceedsTelescope`. -/
mutual
