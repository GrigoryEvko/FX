...
Membership is tested with the `Bool`-valued `List.contains` (matching the
`isRound` idiom below) rather than the propositional `∈`, whose `Decidable`
instance on lists is proved with `propext`. -/
def OrientedGradedPoset.faces (poset : OrientedGradedPoset)
  ...
  poset.elements.filter fun candidate =>
    (poset.inputCofaces candidate).contains element ||
    (poset.outputCofaces candidate).contains element
...
def OrientedGradedPoset.point : OrientedGradedPoset where
  ...
  -- `List.pairwise_singleton` is proved `by simp` (propext + Quot.sound); the
  -- raw `Pairwise.cons`/`Pairwise.nil` constructors keep this axiom-free.
  elementsUnique :=
    List.Pairwise.cons (fun _ memberWitness => nomatch memberWitness)
      List.Pairwise.nil
  inputCofaces := fun _ => []
  outputCofaces := fun _ => []
  inputCofacesDimension := fun _ _ h => nomatch h
