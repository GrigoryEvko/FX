import FX1Poly.Shape.OrientedGradedPoset
/-!
...
The membership reasoning destructures `List.Mem` constructors directly rather
than routing through `List.mem_cons` / `List.mem_filter`, whose `Iff` shapes
are proved with `propext`.  `closedSingleton` reduces definitionally to the
one-element list here, so `List.Mem.head` and a `cases` on the membership
witness suffice. -/
def RegularDirectedComplex.point : RegularDirectedComplex where
  ...
  regularity := fun element memberWitness => by
    have elemIsZero : element = ⟨0, ⟨0, Nat.zero_lt_succ 0⟩⟩ := by
      ...
      | head => rfl
      | tail _ remainingMembership => exact nomatch remainingMembership
    subst elemIsZero
    refine IsMoleculeWeak.point ⟨0, ⟨0, Nat.zero_lt_succ 0⟩⟩ rfl
      (closedSingleton OrientedGradedPoset.point ⟨0, ⟨0, Nat.zero_lt_succ 0⟩⟩)
      ?memberInClosedSingleton ?allMembersEqual
    case memberInClosedSingleton =>
      exact List.Mem.head _
    case allMembersEqual =>
      intro other otherMembership
      cases otherMembership with
      | head => rfl
      | tail _ remainingMembership => exact nomatch remainingMembership
