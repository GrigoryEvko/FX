...
/-! # the raw kernel layer — extrinsic well-scoped MLTT kernel for FX.

The second kernel.  Coexists with `LeanFX.Syntax.Term` (KernelV1, the
intrinsic kernel) until the raw kernel layer reaches feature parity, at which
...
the raw kernel layer works around this by adopting the **well-scoped extrinsic**
...
## Architectural commitments
  ...
  * `Ctx` lives outside the mutual block.  In the raw kernel layer, `Ctx` is a
...
end LeanFX.Syntax.Raw

/-! ## Substitution machinery (renaming layer).
